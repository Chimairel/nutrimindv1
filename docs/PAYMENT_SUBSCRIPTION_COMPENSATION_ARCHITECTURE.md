...
The raw-body boundary, TEST signature/replay verification, durable inbox, leased processing, payment projection, balanced ledger/grant writes, and concrete read-only Checkout reconciliation adapter are implemented in source. Provider acceptance used one authentic signed TEST paid event and one later authenticated `GET /v1/checkout_sessions/{id}` for the same existing session. The event fixture in the projection acceptance was explicitly reconstructed from the accepted event/session identities and is not a second provider delivery. A server-composed processing lifecycle now invokes that processor only when `BILLING_PROCESSING_WORKER_ENABLED=true` and both TEST webhook ingestion and reconciliation are validly enabled. It uses bounded batches, concurrency, a conservative provider-call upper bound, jitter, infrastructure-failure backoff, durable database claims, cancellation-aware retrieval, and graceful stop. The admin-only aggregate status route exposes queue counts, age, recent outcomes, issue count, and process-local worker state without record-level finance or user data. The two authorized checkout/projection migrations are applied to the approved shared development target. Rollback-only shared acceptance exercised the production repository and lifecycle worker across expired-lease recovery, exact replay, retry recovery, dead-letter visibility, balanced posting, time-bounded entitlement, privacy-safe aggregates, and graceful stop with zero committed fixture rows and zero provider network calls. Deployed always-on operation and alert delivery remain unaccepted. Rollback: keep the worker, checkout, webhook, and reconciliation switches false, stop new collection, and retain immutable accepted finance evidence wherever it has been applied.
...
The next payment phase requires separate authorization for **deployment-topology scheduling, alert delivery, and incident-response acceptance**. It should apply no schema change and create no checkout or charge. It must prove one approved TEST worker owner, restart behavior under the selected host, alert routing for backlog/retry/dead-letter thresholds, graceful deployment shutdown, and operator response using `BILLING_PROCESSING_RUNBOOK.md`. All independent switches remain false until that approval. Recurring collection, cancellation, refunds, commercial pricing, live mode, and production deployment remain later gates.
