...
| REQ-018 | Safety intake must persist a backend-authoritative structured intersection of conditions, allergies, intolerances, and avoided ingredients with controlled aliases, original wording/provenance, review-before-save, history, and fail-closed unsupported states | Clinical/privacy/data integrity | Owner-approved September roadmap | Must | Strict preview/save contracts; separate catalogues; supported separators without splitting spaces; forged fields/codes denied; durable entries and revision; idempotent recheck; onboarding and Health profile UX; integration/browser acceptance | TEST-073; INT-009; E2E-007 | Implemented, migrated, and accepted for persistence plus the bounded compatible-plan/grocery path; incompatible active-plan replacement was not repeated in this phase | ADR-016, REQ-003/010/011, RISK-002/019 |
| REQ-021 | Payment collection, subscription entitlements, financial audit, and nutritionist compensation must use separate server-authoritative, idempotent, auditable domains without weakening free safety controls or rewarding review approval volume | Financial/security/privacy/data integrity | Owner-approved product roadmap and September 5 architecture phase | Must | Sandbox-first provider integration; verified webhook/reconciliation authority; immutable event and ledger evidence; time-bounded entitlements; Free retains all safety and privacy rights; work credits arise from valid completed actions and feed capped workload bands rather than per-approval cash; production gates cover provider, legal, tax, security, and operations | TEST-078 through TEST-089 and TEST-100 through TEST-104; future phased tests | Additive foundation is migrated; one authentic TEST paid event and one read-only provider reconciliation have been accepted into one local non-renewing paid-period projection; public triggering, shared-database application, UI, recurring/refund flows, deployment, and live money remain absent | ADR-017/020/021, RISK-022/025, UNC-016/018 |
| REQ-022 | Budget estimates must use dated, location-aware, attributable official price evidence with exact unit/mapping provenance, partial-coverage reporting, and clinical safety precedence | Product/data integrity/clinical | Owner-delegated September 6 budget roadmap phase | Must | No FNRI timeless price; integer PHP centavo ranges; explicit geography/dates/source/original unit; exact versus uncertain mapping; safe conversions only; missing/stale/locality gaps reduce coverage/confidence; Gemini cannot supply prices; clinically blocked/review meals never win budget ranking | TEST-090 through TEST-099 and TEST-130 through TEST-136 | Additive price migrations are accepted; bounded official price ingestion and the empty conversion foundation are locally verified; no shared price/factor row, runtime repository, endpoint, UI, or plan integration exists | ADR-018/019/024, RISK-023/024, UNC-017 |
| REQ-023 | PSA/OpenSTAT ingestion must be checksum-pinned, attributed, geography-exact, deterministic, idempotent, append-only, and restricted to manually reviewed FNRI mappings without inventing missing cells | Product/data integrity/licensing | Owner-authorized September 6 PSA ingestion phase | Must | Official CC BY 4.0 source evidence; exact City of Cebu code; preserved request/CSV bytes and hashes; integer centavos; exact/ambiguous mapping states; duplicate replay creates zero rows; later snapshots supersede rather than mutate; non-loopback application blocked pending a separate gate | TEST-099 | Implemented and locally integration-tested for one six-matrix/eight-commodity January-August 2026 snapshot; shared development remains empty | ADR-019, RISK-024, CHG-20260906-05 |
| REQ-024 | PayMongo sandbox checkout creation must be server-side, TEST-only, owner-scoped, idempotent across uncertain retries, auditable, response-allow-listed, and incapable of granting Premium without later verified webhook/reconciliation evidence | Financial/security/privacy/data integrity | Owner-authorized September 6 Phase 3B sandbox acceptance | Must | Independent fail-closed checkout/webhook switches; native bounded HTTPS; immutable demo price; durable lease and provider key; replay/collision/reclaim states; sanitized provider codes; one provider call; no payment or financial/entitlement projection | TEST-100 | Implemented and accepted locally with one card-capable TEST checkout; runtime defaults remain disabled and no shared database was accessed | ADR-020, RISK-025, UNC-018 |
| REQ-025 | PayMongo TEST payment authority must enter through exact raw-body signature verification and an immutable idempotent inbox before business processing | Financial/security/privacy/data integrity | Owner-authorized September 6 TEST webhook acceptance | Must | Exact TEST signature slot and timestamp tolerance; one-event allow-list; atomic event/work insert; duplicate/hash-conflict distinction; no raw body, signature, PII, card, or secret retention | TEST-101 | Implemented and provider-accepted for one authentic paid TEST event and exact replay; public listener and webhook registration were removed after acceptance | ADR-020, RISK-025, UNC-018 |
| REQ-026 | Paid checkout projection must require an exact local checkout/event binding plus reconciled TEST payment evidence and create one atomic, balanced, time-bounded, non-renewing entitlement projection | Financial/security/privacy/data integrity | Owner-authorized September 6 local projection phase | Must | Claim lease/retry/quarantine; session/reference/amount/currency/status/timestamp agreement; one subscription/invoice/attempt/transaction/grant; two balanced ledger postings; deterministic replay; browser return has no authority | TEST-102, TEST-103 | Implemented and locally accepted with injected reconciliation evidence; production provider retrieval remained a later gate at that point | ADR-020, RISK-025, UNC-018 |
| REQ-027 | Concrete PayMongo reconciliation must use one fixed authenticated read-only TEST retrieval, retain only allow-listed payment facts, fail closed on contradictory/provider-live data, and remain inaccessible from routes and schedulers | Financial/security/privacy/data integrity | Owner-authorized September 6 provider reconciliation acceptance | Must | Independent false-by-default switch; fixed `GET /v1/checkout_sessions/{id}`; Basic TEST-secret authentication; no redirects; bounded timeout/body/content type; sanitized errors; exact one-request acceptance; replay performs no retrieval | TEST-104 | Implemented and provider/local accepted for the existing paid TEST checkout; runtime composition has no public or scheduled trigger and shared development remains untouched | ADR-021, RISK-025, UNC-018 |
| REQ-030 | Purchased-to-edible, raw-to-cooked, and household-measure conversions used for budget estimates must be authoritative, licensed for the intended use, exact to food identity and preparation, append-only, bounded, and independently reviewable | Product/data integrity/licensing/clinical | Owner-delegated September 6 conversion-evidence phase | Must | Separate AP/raw/cooked bases; exact source/FNRI/commodity identities and preparation; rational factor ranges; source version/geography/uncertainty/license/checksum; review and supersession; no reverse/extrapolated transfer; clinical-first evaluation; visible 51-meal coverage | TEST-130 through TEST-136 and TEST-149 | Empty normalized schema is applied to shared development and the pure evaluator, authoritative metadata review, and exact catalogue report are locally verified; no numerical factor, runtime adapter, endpoint, UI, or ranking integration exists | ADR-024/026, RISK-024/027, UNC-017 |
| REQ-031 | Nutritionist compensation administration must use transactionally earned outcome-neutral work credits, locked versioned policy/period statements, independent maker-checker decisions, append-only corrections, owner-scoped nutritionist reads, and manual off-platform payout evidence separate from subscription revenue | Financial/security/privacy/data integrity | Owner-delegated September 6 compensation phase | Must | One credit per eligible completed action; no claim/timeout/abandoned credit; base retainer plus capped bands and approved signed adjustments; deterministic snapshots; separate policy/period/statement/adjustment/payout actors; server-authoritative payout amount; no payout-account data or provider movement | TEST-137 through TEST-142 and TEST-149 | Implemented and locally integration/browser verified; its additive schema is applied empty to shared development, while commercial/legal amounts and every real work-credit/statement/payout action remain unapproved | ADR-025/026, RISK-022/026/027, CHG-20260906-14/16 |
| REQ-032 | Shared-development schema changes must apply only an explicitly authorized pending set after exact target, history, checksum, empty-state, disposable rehearsal, preservation, and postflight gates | Operational/security/data integrity | Owner-delegated final capstone schema gate | Must | Sanitized approved target; 21 clean parents; exactly two pending names; committed checksum agreement; local 23-migration parity and constraints; normal deploy only; 64 table hashes unchanged; new tables empty; no-op replay/current status; exact cleanup | TEST-149 | Passed for the conversion-evidence and compensation-workflow migrations on shared development | ADR-026, RISK-027, CHG-20260906-16 |
| REQ-033 | Every condition rule must bind an immutable source revision, exact supporting location, population, required inputs, evaluation scope, caveats, and explicit result authority before policy impact analysis or activation | Clinical/data integrity/governance | Owner-approved clinical source-of-truth implementation | Must | Missing, withdrawn, or incomplete evidence fails closed; rules retain inactive draft state until existing dual-RND governance; public explanations distinguish formulas, daily/meal scope, and mandatory review | TEST-223 | Implemented, shared-schema applied, seeded as inactive drafts, and source/API/UI verified; no clinical policy activated | ADR-028, CHG-20260925-09, DOC-069 |
...
| ADR-016 | Persist structured safety entries additively while synchronizing existing enum/custom fields as a compatibility projection; backend catalogues alone assign canonical codes and support states | Accepted; migrated and runtime-accepted | 2026-09-05 | Preserves current consumers and legacy profiles; unsupported and unmapped entries remain review-gated | REQ-018, REQ-003/010/011, RISK-002/019, TEST-073, INT-009, E2E-007 |
| ADR-017 | Use PayMongo first as a sandbox collection adapter; keep collection, durable entitlement grants, and nutritionist compensation as separate auditable domains; defer provider disbursements and all live-money use behind explicit readiness gates | Accepted; foundation migration, Phase 3A boundaries, and Phase 3B TEST checkout acceptance complete | 2026-09-05 | Supplies a bounded 3-versus-6 swap entitlement and auditable persistence foundation while preserving common safety; webhook delivery/processing, UI, reconciliation, provider production approval, and legal/tax decisions remain pending | REQ-021/024, RISK-022/025, UNC-016/018, TEST-078 through TEST-089 and TEST-100 |
| ADR-018 | Keep retail-price evidence separate from FNRI nutrition; use append-only source/publication/geography/commodity/mapping/observation records and conservative partial-range policies before any public budget feature | Accepted; additive foundation and hardening migrations applied to shared development with all price tables empty | 2026-09-06 | Prevents timeless/invented prices and unsafe substitutions; requires a licensed bounded source basket, explicit geography, repository, API, and UI gates | REQ-022, RISK-023, UNC-017, TEST-090 through TEST-097 |
| ADR-019 | Commit one unchanged, checksum-pinned CC BY 4.0 PSA/OpenSTAT Cebu City snapshot with exact request bodies and use a loopback-only importer that inserts or verifies immutable evidence by stable identity | Accepted; bounded snapshot and importer implemented and locally accepted | 2026-09-06 | Makes source evidence reproducible and reviewable; preserves missing/ambiguous states and snapshot supersession; requires later shared-import and exact conversion-factor gates before runtime estimates | REQ-022/023/030, RISK-023/024, TEST-099/130-136 |
| ADR-020 | Connect PayMongo checkout through an explicitly enabled TEST-only native HTTPS adapter and lease-based Prisma repository while keeping webhook capability independently disabled and browser returns non-authoritative | Accepted; Phase 3B locally and provider-sandbox accepted | 2026-09-06 | Exact retries reuse one provider key; session evidence and sanitized failures are durable; checkout cannot create entitlement, ledger, payment, invoice, or subscription state | REQ-021/024, RISK-022/025, UNC-016/018, TEST-100 |
| ADR-021 | Retrieve a known PayMongo Checkout Session through a fixed read-only TEST-only adapter, reduce the response to exact payment evidence, and compose the existing projection processor only behind an independent internal switch | Accepted; one provider retrieval and local projection accepted | 2026-09-06 | Closes the concrete reconciliation-adapter gate without adding routes, schedulers, recurring collection, refunds, shared-database writes, or production enablement | REQ-021/026/027, RISK-022/025, UNC-016/018, TEST-102 through TEST-104 |
...
| ADR-028 | Treat KAINARA's versioned database evidence registry as the operational policy source of truth while preserving issuing-organization documents as immutable evidence revisions | Accepted | 2026-09-25 | Runtime rules require current bound evidence and explicit scope; external pages are not queried during meal generation; new editions receive new source codes; citation never implies activation, clearance, endorsement, or partnership | REQ-033, TEST-223, CHG-20260925-09 |
...
| RISK-022 | A payment/subscription implementation could grant access from untrusted client redirects, duplicate or lose financial effects through webhook failure, expose payment or health data, misstate refunds/receipts, or create approval-volume incentives and unaudited nutritionist payouts | Financial/security/privacy/data integrity | High without the planned controls | Critical | Critical | ADR-017/020; raw-body signature verification; immutable idempotent checkout/webhook evidence and financial ledger; server reconciliation; durable period grants; data minimization; maker-checker refunds/compensation; capped workload bands; sandbox and production gates | Open; foundation and TEST checkout controls are verified, while payment authority, webhook delivery/processing, reconciliation, and production governance remain absent |
| RISK-023 | Price evidence can become stale, geographically misleading, unit-incompatible, falsely mapped to FNRI foods, or appear more complete than its coverage, causing misleading budget rankings or unsafe meal choices | Product/data integrity/clinical | High without provenance and fail-closed policy | High | High | ADR-018 append-only evidence; exact mapping; safe unit families; source/locality/freshness labels; range and missing-item reporting; clinical compatibility filter before budget ranking; no Gemini prices | Open; bounded official data and local ingestion now exercise the controls, but shared import, runtime integration, user geography, and UI remain absent |
| RISK-024 | PSA retail observations price purchased raw commodities, while managed-meal quantities often represent cooked edible grams; treating them as the same basis would understate cost and overstate coverage | Product/data integrity | High if observations are connected directly to meal quantities | High | High | Explicit AP/raw/cooked evidence nodes; exact commodity/FNRI/preparation binding; reviewed bounded rational chains; no unauthorized reverse or extrapolation; complete missing-evidence reporting; no runtime adapter | Reduced structurally: conversion evidence is now defined and fail-closed, but zero real factors are loaded and 37 currently priced catalogue rows remain conversion-blocked |
| RISK-025 | A created checkout session could be mistaken for payment, duplicated after an uncertain response, or leak credentials/provider detail through persistence and errors | Financial/security/privacy/data integrity | Critical without server authority and durable idempotency | Critical | Critical | TEST-only config; fixed origins; bounded HTTPS; stable provider key with 23-hour reclaim limit; signed immutable inbox; exact local/provider reconciliation; atomic balanced projection; allow-listed response; sanitized failure codes | Reduced for the single accepted TEST paid checkout and local projection; public worker hosting, shared-database application, recurring/refund behavior, live mode, and production remain blocked |
| RISK-026 | Incorrect compensation actors, duplicate work, mutable snapshots, approval-volume incentives, client-controlled amounts, or payout-account collection could create unfair or unauditable reviewer compensation | Financial/security/privacy/data integrity | High without workflow controls | High | High | Outcome-neutral keyed credits in the review decision transaction; append-only reversals; capped bands; locked policy/period snapshots; strict schemas; distinct actors; server-derived payout remainder; manual reference only; reconciliation and own-only reads | Reduced in source, disposable-local acceptance, and empty shared schema application; real contracts/amounts, tax/legal treatment, dispute operations, retention, and any payout automation remain blocked |
| RISK-027 | A shared-development migration can target the wrong database, accept historical drift, partially apply unexpected work, or mutate existing domain evidence | Operational/security/data integrity | High without a fail-closed gate | Critical | High | Fixed sanitized fingerprint/database/schema/TLS classification; exact pending names and checksums; privacy-safe pre/post table hashes; local migration/datamodel parity; normal deploy only; no-op replay; empty target tables; no persistent shared fixtures | Mitigated for the exact 21-to-23 application by TEST-149; future schema or data changes require a new gate |
...
| DEF-031 | `20260831090000_production_workflow_hardening` and shared development contain `MealPlan_highRisk_review_idx` and `MealPlan_revalidation_status_idx`, but Prisma schema/generated metadata omitted both, causing datamodel comparison to propose removing valid physical indexes | Medium | Canonical migration blob; schema and generated metadata; Git history; sanitized read-only shared catalog; exact local 19-migration reconstruction | Add exact mapped Prisma declarations with the migration-defined names and column order; create no duplicate or removal migration | Resolved by CHG-20260906-04; local migration-derived and read-only shared datamodel parity both report no difference |
| DEF-032 | The verified-library reuse acceptance script hard-coded a 30-row catalogue after the managed catalogue grew to 51 and used a non-reserved fixture domain | Medium | Disposable setup failed at 51 versus 30 before reaching its intended assertions | Derive the expected row count from `COMMON_MEAL_CATALOGUE` and use `example.invalid` synthetic identity | Resolved by CHG-20260906-15; TEST-147 and the repaired disposable setup/verify/cleanup path pass |
| DEF-033 | Expected nutritionist review claim contention and replay failures were returned as HTTP 500 even though the service correctly rejected them | Medium | Concurrent claim produced 200/500; repeated completed review produced 500 | Preserve the service rejection while map known claim, ownership, state, and replay conflicts to HTTP 409 | Resolved by CHG-20260906-15; live contention now produces 200/409 and TEST-145 covers the mapping policy |
...
| UNC-016 | Will PayMongo approve NutriMind's exact business model and required subscription/payment capabilities, and what final commercial, tax-invoicing, refund, retention, compensation, and possible disbursement policies apply? | These answers determine whether production collection is permitted, what customers are charged and issued, how records are retained, and whether any payout automation is lawful and operationally appropriate | Written PayMongo approval/capability confirmation and commercial terms; owner price/refund decisions; Philippine accounting/legal advice; approved nutritionist contracts and compensation policy | Owner/provider/accountant/legal adviser | Open; blocks production and live money but not the bounded additive schema-and-pure-policy phase |
| UNC-017 | Which user geography, freshness windows, source precedence, high-coverage basket, package/edible-quantity mappings, conversion sources, DA/DOST-FNRI reuse terms, and minimum coverage thresholds should govern a public budget estimate? | These choices determine whether an estimate is locally meaningful and honest enough to show or rank | Owner product decision; bounded PSA and exact 51-meal coverage measurement; RND safety review; source-terms confirmation; reviewed exact mappings/conversions and user-facing copy | Owner/engineer/RND/legal reviewer | Open; exact coverage now quantifies 0 complete/0 partial/51 unpriced meals and a specific DOST-FNRI artifact request, but blocks shared factor import, runtime ranking, endpoint/UI exposure, and any Premium claim |
| UNC-018 | Does the sandbox/live account support Maya and the later recurring-subscription/refund lifecycle, and what permanent webhook/worker hosting will be approved? | One accepted card payment and local projection do not prove broader methods, lifecycle capabilities, or production operations | Explicit Maya/subscription/refund capability checks; owner-approved permanent HTTPS ingress and worker hosting; production governance gates | Owner/provider/engineer | Partially resolved: authentic TEST card payment, signed webhook delivery/replay, read-only retrieval, and local non-renewing projection are proven; Maya, recurring subscriptions, refunds, permanent hosting, and every live capability remain unverified |
...
| `MealPlan`, `MealIngredient` | per-slot plan and ingredient records | meal ingredients have optional FNRI link, data source, quantity, and free-text unit; Prisma maps both production-hardening `MealPlan` indexes by their exact database names | older/AI rows may lack exact quantities or FNRI links; actionability policy remains separate |
| `MealLibrary`, `MealLibraryFlag` | approved reusable meals and cross-RND flags | lifecycle/origin/declaration-state/cross-contact/revision/reviewer/version/invalidation columns and lifecycle/reviewer indexes | legacy rows remain explicitly incomplete until certified through current evidence workflow |
| `MealLibraryIngredient`, `MealLibrarySafetyDeclaration`, `MealLibrarySafetyReview` | stable library-owned ingredient provenance, explicit declarations, and append-only review audit | present; ingredient position unique per meal; restrictive evidence relations; certification workflow requires current exact evidence | unit strings remain free text and FNRI linkage proves nutrition identity, not retail-price identity |
...
| `GroceryList`, `GroceryItem` | generated checklist | optional quantity/unit and source-meal count; unknown contributing quantity is preserved as unknown | no price/provenance/coverage contract; regeneration can reset progress |
...
| `IngredientPriceSource`, `IngredientPricePublication`, `IngredientPriceGeography`, `IngredientPriceCommodity`, `IngredientPriceCommodityMapping`, `IngredientPriceObservation` | official source provenance, immutable snapshots, geographic scope, original/normalized commodity units, exact-or-uncertain FNRI mapping, and dated PHP centavo ranges | both additive migrations applied to shared development; bounded PSA snapshot/importer locally accepted; restrictive FKs; append-only triggers; explicit supersession | shared tables remain empty; no database query adapter, endpoint, UI, or runtime estimate; purchased/raw versus edible/cooked conversion remains unresolved |
| `IngredientConversionSource`, `IngredientConversionEvidence` | attributable purchased/raw/cooked conversion evidence with exact identity, preparation, rational range, review, and supersession | additive migration applied empty to shared development; restrictive FKs/checks, source-use gate, and append-only triggers | no numerical factor is accepted or loaded; runtime price conversion and budget ranking remain absent |
| `NutritionistWorkCredit`, `CompensationPolicy`, `CompensationPeriod`, `CompensationStatement`, `CompensationStatementWorkCredit`, `CompensationAdjustment`, `CompensationPayout`, `CompensationPayoutEvent` | outcome-neutral completed-work evidence and maker-checker statement/manual payout record | hardening migration applied to shared development; all tables empty; actor, lifecycle, append-only, and immutable-snapshot controls present | no real policy, credit, statement, adjustment, payout, contract amount, provider transfer, or money movement is authorized |

Schema changes are authorized only in their recorded phase. The ingredient-price migrations added by ADR-018 and the conversion/compensation hardening migrations accepted by ADR-026 passed disposable rehearsal and shared-development acceptance; they do not authorize source-data ingestion or operational compensation records. Before future uniqueness migrations over existing data, inspect duplicate counts without exposing personal data, approve deterministic remediation, and define recovery.
...
| TEST-073 | Structured multi-value safety intake policy and source boundary | Backend pure-policy/schema tests; Prisma validation/client generation; backend/frontend no-emit/build; frontend lint | Documented separators preserve multi-word terms; controlled aliases/deduplication/domain separation; invalid/vague/forged inputs deny; unsupported/custom entries remain review-gated; full legacy projection retains all domains; source compiles | Pass: 262 registered, 261 pass, 0 fail, 1 clinical TODO; Prisma schema valid; both production builds pass; frontend lint has zero warnings | CHG-20260905-01; REQ-018; ADR-016; INT-009/E2E-007 |
| TEST-078 | Payment/subscription/compensation architecture research and documentation sanity | Official-provider documentation review; repository/model/route inventory; Markdown internal-link and official-link checks; identifier, scope, diff, branch, and whitespace checks | ADR distinguishes implemented facts from proposals; cites only official PayMongo evidence with access date and uncertainties; covers collection, entitlement, immutable webhooks, financial ledger, refunds, compensation, failures, reconciliation, tests, gates, and rollback; changes documentation only and touches no protected runtime/database/provider boundary | Pass for architecture/documentation scope on 2026-09-05; exact command evidence and commit recorded in section 38; implementation/runtime/production verification not run | CHG-20260905-04; REQ-021; ADR-017; RISK-022; UNC-016 |
| TEST-079 | Billing persistence schema and additive migration audit | Prisma format/validate with a non-routable local placeholder URL; deterministic schema-to-schema diff; source/static SQL assertions | Required billing, entitlement, immutable webhook, financial, work-credit, statement, and payout aggregates exist; money uses integer minor units plus ISO currency; provider/idempotency uniqueness and preservation FKs exist; checked-in SQL starts with exact generated diff and contains no destructive DML/DDL or alteration of existing domain tables | Pass: 22 additive tables, 42 foreign keys, 43 explicit checks; exact generated prefix and additive audit pass; migration not applied | CHG-20260905-05; REQ-021; ADR-017; RISK-022 |
...
| TEST-090 | Canonical ingredient-price units and quantity conversion | Backend pure-policy unit | Recognized mass/volume/count aliases convert only within family; package labels, cross-family conversion, unknown units, and invalid/non-positive quantities fail closed | Pass: 3 deterministic cases | `backend/tests/ingredient-price-policy.test.ts`; CHG-20260906-01; REQ-022; ADR-018 |
| TEST-091 | PHP centavo, exact mapping, and freshness policy | Backend pure-policy unit | Money is safe integer centavos; only exact evidence-backed FNRI mappings qualify; current/stale/future/invalid dates classify deterministically | Pass: 3 deterministic cases | `backend/tests/ingredient-price-policy.test.ts`; CHG-20260906-01; REQ-022; ADR-018 |
...
| TEST-097 | Shared-development ingredient-price migration acceptance | Sanitized configured-target classification; read-only canonical history/checksum/pending/table preflight; exact local 17/19-migration PostgreSQL reconstructions; privacy-safe 56-table snapshots; normal production deploy; exact SQL catalog and empty-table postflight; no-op redeploy/status; proportional regression and cleanup audit | Apply exactly the rehearsed foundation and strict-normalization migrations to the owner-authorized pooled Neon `neondb`/`public` development schema, preserve all old data/schema, create only the declared empty price objects, and leave no local audit resource | Pass: 17 parent migrations and only two price migrations pending; migration-derived preflight parity; exactly two applied; 19 successful checksum-matched rows; 6 empty tables, 8 enums/34 values, 15 declared plus 6 PK indexes, 10 restrictive FKs, 14 checks, 6 append-only triggers; all 56 count/content hashes unchanged; no-op redeploy/status and post-migration parity; local audit teardown | CHG-20260906-03; REQ-022; ADR-018; DEF-031; DOC-037 |
| TEST-098 | MealPlan index datamodel reconciliation | Canonical migration/schema/generated-metadata/Git-history inspection; focused exact-name/column-order and query-shape tests; sanitized read-only shared catalog; task-owned PostgreSQL 16.4 reconstruction of all 19 migrations; migration-derived and shared datamodel diffs; no-op local deploy; cleanup audit | Resolve the contradictory index description, represent both existing physical indexes exactly in Prisma, add no migration, and preserve zero live writes | Pass: migration and read-only shared catalog both contain the exact two indexes; corrected schema/generated metadata map both names and ordered fields; revalidation query evidence is direct and review priority is in memory; local 19-migration and shared read-only comparisons each report no difference; second local deploy is empty; 365 tests registered, 364 pass, 0 fail, 1 existing TODO; clean teardown | `backend/tests/meal-plan-index-schema.test.ts`; CHG-20260906-04; DEF-031; DOC-038 |
| TEST-099 | PSA/OpenSTAT snapshot and importer acceptance | Official source/license/API/geography preflight; checksum-pinned request/CSV fixtures; strict parser/mapping/coverage unit tests; task-owned PostgreSQL 16.4 with all 19 migrations and FNRI seed; dry-run, first import, exact replay, local-only supersession/replay probe, row-state audit, schema parity, full regression/build/audit, cleanup | Preserve exact Cebu City official evidence, reject malformed/substituted/guessed data, create deterministic immutable rows, and leave shared development untouched | Pass: 6 matrices/8 commodities; 40 available and 24 missing cells; 7 exact/1 ambiguous mappings; first import created 1 source/1 geography/6 publications/8 commodities/8 mappings/40 observations; exact replay created zero; synthetic successor probe created 40 linked successors and replay created zero; 375 tests registered, 374 pass, 0 fail, 1 existing TODO; build, scripts compile, offline audit, migration no-op, and parity pass | `backend/tests/psa-openstat-price-ingestion.test.ts`; CHG-20260906-05; REQ-023; ADR-019; RISK-024; DOC-039 |
| TEST-100 | PayMongo Phase 3B sandbox checkout acceptance | Official-doc recheck; deterministic config/gateway/route/schema/transport tests; task-owned PostgreSQL 16.4 migration and repository scenarios; one TEST provider request plus exact local replay; persistence/redaction/projection audit | Prove TEST-only card checkout creation, one provider call, durable owner/idempotency evidence, replay/collision/reclaim behavior, sanitized failures, and zero entitlement/finance effect | Pass: provider returned one non-live hosted checkout; exact replay made no second provider call; wrong role/unknown user denied; retry reused provider key; zero subscription/invoice/attempt/transaction/ledger/grant rows; full final counts recorded in section 48 | `backend/tests/paymongo-*.test.ts`; `backend/scripts/paymongo-sandbox-checkout-acceptance.ts`; CHG-20260906-06; REQ-024; ADR-020; RISK-025; UNC-018; DOC-040 |
| TEST-101 | PayMongo TEST webhook durable-inbox acceptance | Exact raw-body/signature policy and repository tests; task-owned PostgreSQL; temporary minimal HTTPS ingress; one authentic provider delivery plus exact in-memory replay | Prove a paid TEST event is authenticated before storage, persisted once with one pending work item, replayed idempotently, and reduced to a sanitized envelope | Pass: event `evt_aF8YduaEpB1vLvC1NRM9rc8z` for session `cs_2c030de77711f9285b90547f` produced `INSERTED` then `DUPLICATE`; one event and one work row; raw body/signature/provider detail absent; temporary resources removed | `backend/tests/paymongo-webhook-boundary.test.ts`; `backend/tests/prisma-webhook-inbox.repository.test.ts`; CHG-20260906-07; REQ-025; ADR-020; RISK-025; UNC-018; DOC-041 |
| TEST-102 | PayMongo payment reconciliation and atomic projection policy | Pure policy and processor/repository integration tests with injected provider evidence and task-owned PostgreSQL | Prove exact event/checkout/price/payment agreement, bounded claim retry/quarantine, one atomic non-renewing paid-period projection, balanced ledger, and idempotent replay | Pass: success and month-end ranges, all required mismatch classes, concurrent convergence, retry recovery, conflict rollback, replay, no-overlap, half-open entitlement, and zero cancellation/refund rows | `backend/tests/paymongo-payment-reconciliation.test.ts`; `backend/tests/paymongo-payment-projection.service.test.ts`; CHG-20260906-08; REQ-026; ADR-020/021; RISK-025; DOC-042 |
| TEST-103 | PayMongo one-time projection schema and migration constraints | Static Prisma/migration assertions plus migration-derived PostgreSQL parity | Prove claim lease, reconciliation issue, one-time source binding, balanced posting identity, and restrictive preservation constraints exist in canonical schema/history | Pass: all 21 migrations current; second deploy empty; database/datamodel comparison reports no difference | `backend/tests/paymongo-payment-projection-schema.test.ts`; CHG-20260906-08; REQ-026; ADR-020/021; RISK-025; DOC-042 |
| TEST-104 | PayMongo concrete provider reconciliation acceptance | Fixed-adapter unit/security tests; runtime reachability assertions; task-owned PostgreSQL with all 21 migrations; exactly one authenticated read-only provider GET; post-projection database audit | Prove TEST-only fixed-origin retrieval, allow-listed parsing and sanitized failure classes, exact accepted checkout agreement, one projection, replay without a second request, and no unrelated lifecycle state | Pass: one `GET /v1/checkout_sessions/cs_2c030de77711f9285b90547f`; paid payment `pay_3PVKHghbxy3jcYLDDbRwfFPW`, succeeded intent `pi_U7FX3pA65Y8kv4Y8pfNQt4fK`, PHP 19,900 minor, non-live; projection counts 1/1/1/1/2/1; replay `NO_WORK` at one request; ledger balance zero; unrelated customer/cancellation/refund/compensation/payout counts zero | `backend/tests/paymongo-reconciliation-gateway.test.ts`; `backend/scripts/paymongo-provider-reconciliation-acceptance.ts`; CHG-20260906-09; REQ-027; ADR-021; RISK-025; UNC-018; DOC-043 |
| TEST-130 | Exact conversion composition and clinical precedence | Backend pure-policy unit with synthetic factors | Clinical `BLOCK`/`REVIEW` returns before malformed evidence; reviewed exact AP-to-raw and raw-to-cooked rational edges compose with canonical mass units | Pass: 3 deterministic cases | `backend/tests/ingredient-conversion-evidence.test.ts`; CHG-20260906-13; REQ-030; ADR-024 |
| TEST-131 | Conversion eligibility, lifecycle, direction, ambiguity, and graph safety | Backend pure-policy unit with synthetic factors | Food identity/preparation/basis/unit mismatch, unauthorized reverse, draft/rejected/future/stale/superseded evidence, duplicate conflicts, ambiguous paths, cycles, and repeated edges fail closed | Pass: 4 deterministic cases | `backend/tests/ingredient-conversion-evidence.test.ts`; CHG-20260906-13; REQ-030; ADR-024 |
| TEST-132 | Bounded conversion arithmetic and household measures | Backend pure-policy unit with synthetic factors | Ranges propagate exactly; household measures require exact food/preparation rows; safe integer-centavo costs round outward; unsafe quantities fail closed | Pass: 3 deterministic cases | `backend/tests/ingredient-conversion-evidence.test.ts`; CHG-20260906-13; REQ-030; ADR-024 |
| TEST-133 | Authoritative conversion-source review | Strict metadata schema and committed review artifact | Source/version/URL/access/geography/method/food/preparation/meaning/uncertainty/license/import decision are explicit; the pilot is provably empty | Pass: 1 deterministic case | `backend/data/ingredient-conversion-evidence/source-review-2026-09-06.json`; `backend/tests/ingredient-conversion-evidence.test.ts`; CHG-20260906-13; REQ-030 |
| TEST-134 | Exact 51-meal conversion coverage | Checksum-verified PSA snapshot plus managed catalogue | Count all 51 meals/195 rows through source, mapping, current price, quantity/unit, same identity without a basis bridge, conversion evidence, and complete/partial/unpriced layers; list every missing row | Pass: 1 deterministic case; 0 complete, 0 partial, 51 unpriced | `backend/data/ingredient-conversion-evidence/coverage-2026-09-06.json`; CHG-20260906-13; REQ-030; RISK-024 |
| TEST-135 | Additive conversion-evidence schema | Static Prisma/migration assertions | Two normalized models, explicit bases, constraints, restrictive FKs, append-only triggers, source-use gate, and no shortcut `FoodItem` conversion/price field | Pass: 3 deterministic cases | `backend/tests/ingredient-conversion-schema-migration.test.ts`; CHG-20260906-13; REQ-030; ADR-024 |
| TEST-136 | Disposable conversion migration rehearsal | Task-owned PostgreSQL 16.4; all canonical migrations; rollback-only valid graph and invalid-operation probes; second deploy/status/diff; preservation and cleanup audits | Apply the one additive migration empty, prove SQL constraints/source-use/immutability and no drift, preserve prior tables, and remove every task resource | Pass: 22 migrations current; checksum `cf6c15f3…c24af2a1`; 2 empty tables, 6 enums/25 values, 8 indexes, 5 restrictive FKs, 10 checks, and 3 triggers; 7 expected SQL failures plus valid-chain audit pass; all 64 parent-table hashes match; no-op deploy/status and datamodel parity pass; exact cleanup | `backend/scripts/ingredient-conversion-migration-rehearsal.sql`; CHG-20260906-13; REQ-030; ADR-024 |
| TEST-137 | Compensation request validation | Backend schema unit | Strict PHP policy, periods, signed adjustments, decisions, payout idempotency/evidence, and reversal bodies reject unknown, malformed, zero, decreasing, or cross-currency input | Pass | `backend/tests/compensation-admin-contract.test.ts`; CHG-20260906-14; REQ-031 |
| TEST-138 | Compensation role and amount authority | Static route contract | ADMIN middleware precedes all administration; nutritionist read derives identity from eligible authenticated profile; payout request contains no client amount | Pass | `backend/tests/compensation-admin-contract.test.ts`; CHG-20260906-14; REQ-031 |
| TEST-139 | Completed-review work-credit wiring | Static transactional caller contract plus disposable integration | Approve, reject, and high-risk escalation hooks use the same decision transaction; claim path has no award; action keys deduplicate; high-risk second review is distinct | Pass | `backend/tests/compensation-admin-contract.test.ts`; `backend/scripts/compensation-local-acceptance.ts`; CHG-20260906-14; REQ-031 |
| TEST-140 | Additive compensation workflow migration | Static migration checks plus PostgreSQL deploy/status/diff | Add actor/status fields, one-active-policy, maker-checker constraints, append-only credit/link/event evidence, and immutable approved statement snapshots without changing accepted migrations | Pass: 23 migrations current; checksum `56ab3fa1…8c332232`; no-op redeploy/status and datamodel parity | `backend/prisma/migrations/20260906235900_compensation_admin_workflow/migration.sql`; CHG-20260906-14; REQ-031; ADR-025 |
| TEST-141 | Compensation role UI contract and browser acceptance | Source assertions and authenticated in-app browser at default desktop and 390x844 mobile | Admin sees separated ledger/reconciliation and controlled actions; nutritionist sees only own credit/statement/adjustment/payout evidence; both layouts avoid overflow/errors; cross-role routes deny | Pass: both roles, both viewports, zero console errors, 390/390 mobile width | `frontend/src/app/(admin)/admin/compensation/page.tsx`; `frontend/src/app/(nutritionist)/nutritionist/compensation/page.tsx`; CHG-20260906-14; REQ-031 |
| TEST-142 | Full disposable compensation lifecycle | Task-owned PostgreSQL 16.4 with all 23 migrations and production Prisma services | Prove 4 deduplicated valid awards across outcomes/stages, no claim credit, linked reversal, policy/period/statement actor separation, idempotent adjustment/payout replay, two own-only statements, exact reconciliation, append-only guards, and no provider row/call | Pass: 4 awards, 2 statements, PHP 230,500 minor gross/paid, zero mismatch, manual method, zero provider calls | `backend/scripts/compensation-local-acceptance.ts`; CHG-20260906-14; REQ-031; ADR-025 |
| TEST-143 | Unified weight boundary | Pure policy plus live local API/database | Reject non-number, non-finite, below-30, and above-300 kg inputs consistently | Pass: policy matrix and live 10,000 kg request returned 400 | `backend/tests/full-system-e2e-audit.test.ts`; CHG-20260906-15; DEF-034 |
| TEST-144 | Atomic weight profile/history write | Static service contract plus live local API/database | Profile and weight history change together once, notes are trimmed and bounded, and invalid requests write nothing | Pass: invalid count/weight unchanged; valid 58.5 kg created one trimmed entry and updated profile | `backend/tests/full-system-e2e-audit.test.ts`; CHG-20260906-15; DEF-034 |
| TEST-145 | Nutritionist review HTTP conflict semantics | Pure policy plus concurrent/replay API probes | Claim contention, stale ownership, completed-review replay, and same-reviewer second-review denial return 409 | Pass: concurrent claim converged at 200/409; stale/replay/same-reviewer attempts returned 409 | `backend/tests/full-system-e2e-audit.test.ts`; CHG-20260906-15; DEF-033 |
| TEST-146 | Test-only mail capture boundary | Backend unit/filesystem | OTP, reset, and invitation evidence can be captured locally only under `NODE_ENV=test` with an absolute path and no SMTP connection | Pass: three reserved-domain JSONL records; production and relative-path attempts rejected | `backend/tests/full-system-e2e-audit.test.ts`; CHG-20260906-15 |
...
| TEST-223 | Versioned clinical evidence registry and fail-closed condition-rule governance | Prisma validation/migration/deploy/status, source/rule postflight, pure backend tests, public API/HTTP checks, frontend data/UI tests, full lint/build/regression suites | Exactly one additive migration; source revision and rule application metadata required; withdrawn/missing source excluded; all seeded policies/rules remain inactive; no ruleset clearance created; public policy map names formulas and review boundaries | Pass: migration checksum `7d06ed8e…e995de7`; 67 migrations current; 14/14 sources current; 7 nutrient + 9 ingredient rules complete; 0 active policies/rules/ruleset clearances; evidence API and `/sources` HTTP 200; backend 502 pass/0 fail/1 existing TODO; frontend 248/248; both builds and lints pass | `backend/tests/clinical-evidence-source-catalog.test.ts`; `backend/tests/condition-policy-evidence.test.ts`; `frontend/src/data/evidence-sources.test.ts`; CHG-20260925-09; REQ-033; ADR-028 |
...
| DOC-069 | Record the versioned clinical evidence registry, rule-to-source bindings, scope and authority metadata, public condition calculation map, shared migration/seed postflight, and inactive-policy boundary | Completed source, shared-development schema/data, API, HTTP, regression, and build evidence; no policy approval, clinical validation, or production deployment claim | REQ-033; ADR-028; TEST-223; CHG-20260925-09 |
...
## 35. Structured multi-value safety intake — migrated and bounded runtime acceptance complete (2026-09-05)

**Requirement ID:** REQ-018

**Decision ID:** ADR-016

**Change ID:** CHG-20260905-01

**Verification ID:** TEST-073; INT-009; E2E-007

### Implemented source behavior

- Added separate version-controlled condition and food-safety catalogues. Every recognized entry has a stable code, display name, controlled aliases/search terms, support state, and policy reference. The five persisted states are `SUPPORTED`, `RECOGNIZED_UNSUPPORTED`, `NEEDS_CLARIFICATION`, `PENDING_REVIEW`, and `INVALID`.
- Added deterministic parsing for commas, semicolons, slashes, and line breaks. Ordinary spaces remain part of multi-word terms. Input is NFKC-normalized, trimmed, bounded, deduplicated by domain and canonical identity, and mapped only through explicit aliases. No Gemini call diagnoses or validates an entry.
- Strict preview/save request schemas accept only `domain`, raw `value`, and `provenance`; unknown properties, forged classifications, unconfirmed saves, invalid predefined codes, vague phrases, separator-only input, and oversized entries fail closed.
- Added an additive `SafetyProfileEntry` model retaining canonical code, original wording, normalized comparison text, provenance, support state, and policy reference. Saves replace the complete structured set transactionally, write one `STRUCTURED_SAFETY_UPDATED` history snapshot, invalidate stale nutrition-report acknowledgement only on a real change, and skip both revision and recheck work for an identical submission.
- Existing `HealthCondition`, `Allgy`, `UserProfile.otherConditions`, and `UserProfile.otherAllergies` records remain synchronized as a compatibility projection. Conditions, allergies, intolerances, and avoided ingredients all remain in that projection; unsupported/unmapped items therefore continue to force the existing compatibility policy to review rather than disappearing. Legacy write endpoints now enter the same structured service and preserve domains they do not edit.
- Legacy profiles are not destructively backfilled. The UI projects their current enum/custom values into the structured editor until the first confirmed save. This preserves the original rows and avoids inventing historical provenance.
- Replaced onboarding condition/allergy free-text controls and the editable Health profile safety form with one responsive keyboard-operable review flow. Predefined and custom entries share the same collection; food allergies, intolerances, and avoided ingredients retain separate domains; preview chips show classification and original wording; removal is labeled; unsupported/pending entries visibly explain the individual-review boundary.

### Schema and verification evidence

- Owner authorization was recorded before the configured shared Neon database was changed. The additive migration `backend/prisma/migrations/20260905120000_structured_safety_intake/migration.sql` was applied on 2026-09-05 and registered as the sixteenth migration with SHA-256 checksum `ce7a50c07ca4fe5c428143264202a76c259dda19564f2410675da700835bfb1e`. Runtime introspection confirmed the 12-column `SafetyProfileEntry` table, primary/unique/support-state/user-domain indexes, cascading user foreign key, three enums, and additive revision value.
- Managed-shell `prisma migrate status` and `prisma migrate deploy` again stopped at the repository's documented Windows schema-engine boundary before applying a migration. The exact nine checked-in SQL statements were therefore executed under one PostgreSQL transaction with the Prisma advisory lock, and the exact file checksum was recorded in `_prisma_migrations` in the same transaction. A runtime read then proved the finished, non-rolled-back migration record and schema objects. No seed, catalogue row, or unrelated migration was applied.
- Slow Neon round trips exposed Prisma's five-second interactive-transaction default in structured saving and grocery generation. Those two multi-query transactions now use a bounded 30-second timeout; no query semantics changed.
- TEST-073 remains **262 registered: 261 pass, 0 fail, 1 pre-existing clinical-policy TODO**. Cases cover separators, multi-word values, mixed sources, aliases and controlled misspellings, case-insensitive duplicates, domain separation, invalid/vague/unsupported/unknown states, oversized and separator-only values, `NONE` contradictions, forged client fields/codes, catalogue evidence metadata, full legacy intersection projection, and deterministic input preservation. Prisma validation/client generation, backend no-emit/build, frontend no-emit/lint/build all pass; lint has zero warnings and the frontend generates all 37 routes.
- INT-009 passed through the guarded acceptance command. Seven mixed restrictions survived profile reload; `high blood pressure` resolved to `HYPERTENSION`; legacy enum/custom fields remained synchronized; a semantic change added exactly one revision and cleared report acknowledgement; identical resubmission added no revision. A certified still-compatible active plan remained approved/actionable and its derived grocery-list identity and item count remained unchanged. The fixture's `finally` cleanup left zero reserved users, safety entries, meal plans, or grocery lists.
- E2E-007 signed in with an exact reserved `example.invalid` account and exercised onboarding conditions, onboarding food safety, and editable Health profile. The browser covered comma/semicolon/slash separation, multi-word terms, predefined/custom mixing, canonical aliases, unsupported and pending states, a visible `heart problem` clarification error, chip removal, keyboard checkbox/button/input interaction, confirmation, save, report invalidation, and reload. Default desktop and 390x844 mobile layouts were usable, and browser logs contained zero warnings or errors. Exact cleanup removed the browser fixture, 10 safety entries, 3 revisions, 2 sessions, and left zero user/entry/revision/session/plan/grocery rows.

### Decision and remaining boundary

- ADR-016 accepts additive structured rows plus a synchronized legacy projection for Phase 1. This keeps current generation/swap/review callers stable while making raw wording and classification durable. The backend catalogue is authoritative; client suggestions never authorize a code or support state.
...
**Verification IDs:** TEST-074, TEST-075, INT-010; E2E-008 public/guard-only
...
- Backend deterministic suite: **268 registered, 267 pass, 0 fail, 1 pre-existing clinical-policy TODO**. Prisma validation, backend build, Phase 2 script type-check, frontend lint, and frontend production build pass; lint has zero warnings and the build generates all 37 routes. Local browser inspection on port 3000 rendered the public landing page at the default viewport, rendered `/docs` at 390x844, and confirmed unauthenticated `/nutritionist/library` redirects to `/login`. The changed authenticated coverage/review/admin data surfaces were not claimed as browser-tested because this phase did not create or mutate an authenticated fixture. The public browser session recorded one expected stale-session `401` warning from `AuthContext`; no page-rendering error was observed.

### Guarded INT-010 runtime evidence

- The owner authorized exactly one configured shared-database run of `ALLOW_STRUCTURED_REPLACEMENT_ACCEPTANCE=true npm run test:acceptance:structured-replacement` on September 5, 2026. Preflight proved the clean feature branch and remote were both at `2f90c197922ebda0cc1206305abeb8edf5228bd4`, migration 16 (`20260905120000_structured_safety_intake`) was the latest completed migration with checksum `ce7a50c07ca4fe5c428143264202a76c259dda19564f2410675da700835bfb1e`, the structured table had its expected 12 columns, and the reserved namespace was empty.
- INT-010 passed in one execution. The active certified `Pandesal, Egg and Tomato Breakfast` (`cmtk0w6hn0009pqphabpfn40v`) had complete current evidence with `EGGS` present. Saving the structured egg restriction caused the production recheck to replace it with certified `Tofu Tomato Breakfast Rice Bowl` (`cmtk0weji0022pqphgqu2ms6d`). A read-only post-check proved the replacement still had complete evidence, an `EGGS` reviewed-absent declaration, matching safety/certified revision **6**, policy `NUTRIMIND_LIBRARY_SAFETY_V1`, and an `ALLOW` result from the production structured-profile evaluator.
- The fixture proved the replacement stayed approved and actionable, wrote `SAFETY_REPLACED` meal-log evidence, invalidated the previously acknowledged nutrition report, produced exactly one semantic structured-safety revision, replaced the prior grocery list, and projected the replacement plan's ingredient-name set. Repeating the identical save/recheck made no new revision, replacement, or grocery projection. The production path returned only after its replacement notification write completed; notification presence/content was source-path evidence rather than a separate pre-cleanup fixture assertion.
- Before writes, all 49 managed counter baselines were read. The one selected replacement received the production `usageCount +1`; cleanup restored only that selected increment through compare-and-set, independently deleted the exact reserved user and cascading owned records, and internally asserted zero selected-counter drift. The read-only post-check found both original and replacement counters at **0** and zero reserved users, structured entries, plans, meal ingredients, grocery lists, grocery items, revisions, notifications, and `SAFETY_REPLACED` logs.
- No Gemini, email, OAuth, payment, seed, permanent catalogue-content, deployment, or `main` write occurred. The exact reserved fixture was deleted before browser verification could reuse it, so authenticated replacement/grocery UI surfaces remain build/static evidence; creating another shared actor solely for browser coverage was outside the authorized mutation scope.
...
## 37. Diabetes + vegetarian + egg-free catalogue gap and live coverage (2026-09-05)

**Requirement ID:** REQ-020

**Change ID:** CHG-20260905-03

**Verification IDs:** TEST-076, TEST-077; INT-011

### Minimum source additions

- The 49-meal configured-database evidence in section 36 established one missing compatible lunch and one missing compatible dinner for the Diabetes + vegetarian + egg-free profile. The source catalogue adds exactly two distinct vegan, egg-free meals; no existing definition, compatibility threshold, clinical constant, migration, or schema changed.
...
- Both definitions inherit the existing vegan/vegetarian/omnivore/pescatarian and goal tags, contain none of the five supported allergen classes, remain below the unchanged **60 g carbohydrate** diabetes threshold, and have complete sodium evidence below the unchanged **600 mg** hypertension threshold. INT-011 certified the five supported allergens as reviewed absent through the established workflow. Catalogue validation now rejects exact duplicate ingredient/amount/category recipes in addition to duplicate names.
...
- The owner explicitly authorized the four-step INT-011 sequence against the configured shared Neon `neondb` target while the clean local and remote branch both pointed to `6a875998ee42808e85d3ac58360b3801cf2bfff9`. Read-only preflight resolved all **36** exact FNRI foods for all **51** definitions and found the existing configured reviewer Andrea Reyes, RND (`nutritionist@gmail.com`, `PRC-RND-NM-0001`) eligible. The exact operation projection was **2 creates, 0 updates, 49 skips**, with no meal-name collisions and no new or invented reviewer identity.
- The first authorized `npm run seed:meal-library -- --apply` created and certified only `Tokwa Ampalaya Rice Bowl` and `Tokwa Sayote and Sitaw Dinner Plate`, skipped all 49 current definitions, verified 51 complete current managed meals, and reconciled the existing reviewer's `totalVerified` counter to **55**. The immediate second apply-capable run reported **0 created, 0 certified, 51 already current** and left the profile counter unchanged, establishing live database idempotence.
- Read-only `npm run measure:structured-coverage` used production `isCertifiedLibraryMealCompatible` against all **51** managed certified meals. Diabetes + vegetarian + egg allergy is now **7/7/7** breakfast/lunch/dinner; hypertension + pescatarian + dairy allergy is **9/14/14**; diabetes + hypertension + gluten allergy is **9/14/11**. All three profiles are week-ready at the unchanged seven-per-slot requirement.
- No migration, deletion, temporary user or reviewer, Gemini, email, OAuth, payment, deployment, port 3030 interaction, or `main` write occurred. INT-011 made only the two authorized permanent catalogue additions, their established ingredient/review/declaration certification records, and the existing reviewer-counter reconciliation on the first run; the second run made no writes.

## 38. Payment, subscription, and nutritionist-compensation architecture (2026-09-05)

**Requirement ID:** REQ-021

**Decision ID:** ADR-017

**Change ID:** CHG-20260905-04

**Verification ID:** TEST-078

### Accepted architecture

- PayMongo is selected as the first sandbox collection adapter. Production remains blocked on written provider acceptance of NutriMind's exact business model, actual subscription/payment capability activation, final commercial terms, Philippine accounting/legal decisions, security review, operations evidence, and explicit owner go-live approval.
- Collection, durable entitlement grants, and nutritionist compensation are separate auditable domains. A browser redirect or client flag never grants Premium, and a user subscription payment never directly triggers reviewer pay.
- The sandbox MVP defines one monthly Premium test price, with PHP 199.00 allowed only as a demo placeholder. Free retains every safety, restriction, verification, weekly planning, logging, grocery/PDF, compatible-library, and account-export capability. The only MVP Premium behavior is a safety-bounded weekly swap cap of six instead of the existing three.
- Paid invoice periods create durable grants through verified webhooks or server reconciliation. Cancellation stops renewal while preserving the paid period; past-due grace is bounded to 72 hours; incomplete and unpaid states create no new grant; refund effects are explicit rather than silently inferred.
- A future raw-body PayMongo webhook route must precede the current global `express.json()` middleware. It verifies the environment-specific HMAC over the exact bytes, persists an immutable event and outbox item before 2xx, processes idempotently, does not assume delivery order, and reconciles missed or exhausted events independently.
- The proposed additive data design covers immutable products/prices, provider customers, subscriptions, invoices, payment attempts, financial ledger entries, refunds, entitlement grants, webhook envelopes/processing attempts, and reconciliation issues. No schema or migration was created in this phase.
- Nutritionist work credits are first-class records written transactionally with eligible completed review actions. Claims, mutable `totalVerified`, duplicates, outcome type, and raw approval volume are not payroll evidence. Compensation uses a contracted base retainer plus capped workload-band allowance plus independently approved adjustments, with period statements and maker-checker payout records. Capstone payouts remain manual/off-platform and contain no bank details.
- PayMongo merchant `Payouts` and wallet-funded `Disbursements` are distinct. Only Disbursements could be relevant to later worker/contractor payment, and it remains deferred behind a separate ADR, provider approval, legal/tax decisions, recipient-data controls, wallet funding, idempotency, and reconciliation.

### Research, scope, and evidence

- Official provider materials were reviewed on September 5, 2026. The abandoned payment architecture artifact and its claim-level citations were removed from the active branch with the September 22 product reversal and remain available in Git history.
- Provider documentation establishes cards and Maya for automatic subscriptions, separate account activation, immediate first invoice with a 24-hour completion window, once-daily retry for up to three attempts, signed raw-body webhooks, duplicate/retry behavior, test mode, and full/partial refund support. GCash subscription availability, refund test coverage, subscription-specific fees, NutriMind business eligibility, and the legal status of provider invoices/receipts remain explicit uncertainties.
- Repository inspection confirmed no existing billing/subscription/entitlement/compensation implementation; the mutable nutritionist counter and generic audit stream are unsuitable accounting sources; the swap cap is currently hard-coded as three; account export must remain a free privacy right; and the current middleware order requires a deliberate webhook raw-body boundary.
- This change adds and reconciles documentation only. It does not modify source, tests, package files, Prisma schema/migrations, provider accounts/configuration/credentials, the configured shared database, deployed environments, or any payment. It does not start or interact with ports 3000, 5000, or 3030.

### Phased gate and next work

- Phase 0 resolves provider/account, commercial, legal/tax, refund, retention, and compensation-policy prerequisites.
- The next single bounded coding phase is Phase 1 only: additive Prisma schema and migration files plus pure state, entitlement, money, idempotency, and work-credit policies with deterministic tests. The migration remains unapplied and no PayMongo call occurs.
- Later separately gated phases cover disposable/shared migration rehearsal, sandbox collection, immutable webhook processing and reconciliation, the 3-versus-6 swap entitlement UI, internal compensation statements/manual payout records, and finally production readiness.
- Every phase has a disable/rollback boundary. Financial and compensation evidence is append-only; rollback never deletes ledger, webhook, invoice, refund, statement, or payout history.

### Documentation verification

- TEST-078 verifies the new ADR's official-source citations, internal Markdown targets, current traceability IDs, required topic coverage, documentation-only diff, whitespace, branch isolation, and protected-boundary exclusions. All **20** unique external citation URLs use official `docs.paymongo.com` or `www.paymongo.com` hosts and were opened successfully during the September 5 research pass; relative links across all four edited documents resolve locally; the 19-term required-topic scan and `git diff --check` pass.
- The exact edited set is `README.md`, `docs/NEXT_FEATURES_HANDOFF.md`, this engineering record, and the new architecture document. Pre-commit isolation confirmed `feature/payment-subscription-architecture` at exact base `d3c73911a9e9a5e28de47e3fcde94fe2ef530eb2`, while local `main` and `origin/main` both remained `d17b30482398472a02eefd505950f8b42af6b223`. No application test or runtime claim is attached to this architecture-only phase.

## 39. Billing, entitlement, webhook-idempotency, and work-credit foundation (2026-09-05)

**Requirement ID:** REQ-021

**Decision ID:** ADR-017

**Change ID:** CHG-20260905-05

**Verification IDs:** TEST-079, TEST-080, TEST-081, TEST-082

**Documentation ID:** DOC-031

### Additive persistence foundation

- Added Prisma enums and 22 new models for products and immutable versioned prices; provider customers; user subscriptions and cancellation requests; invoices, payment attempts, successful transactions, refunds, and append-only financial ledger entries; immutable provider webhook envelopes with separate mutable processing state; durable entitlement grants and reconciliation issues; nutritionist work-credit awards/reversals; versioned compensation policies and periods; statements with immutable credit snapshots and signed adjustments; and payout records/events.
- All monetary values use integer minor units plus a three-letter ISO currency field. No card number, CVC, OTP, provider secret, bank account, e-wallet credential, or health field is present. Provider resource IDs and outbound operation keys have environment-scoped uniqueness where applicable. A stable opaque billing-subject key allows user relations to be cleared without deleting financial evidence.
- Finance/user ownership uses `ON DELETE SET NULL` only where an identity may later be pseudonymized; invoices, transactions, ledger evidence, webhook evidence, nutritionist work credits, statement snapshots, and payout events use restrictive relations. Existing `User`, `NutritionistProfile`, review, meal, safety, catalogue, and auth columns are unchanged.
- The generated migration `backend/prisma/migrations/20260905180000_billing_foundation/migration.sql` is additive and intentionally unapplied. Its generated portion exactly matches a local schema-to-schema Prisma diff from the `bc07d9a` baseline; 43 explicit PostgreSQL checks add money/currency, time-range, webhook environment/hash, ledger sign, grant, reversal, compensation, payout-method, terminal-reference, and maker-checker invariants that Prisma cannot express.

### Pure policy foundation

- `billing-entitlement.policy.ts` resolves Free or Premium only from valid durable grants. A paid-invoice grant requires `PAID` invoice evidence; an active provider subscription alone provides nothing; cancellation preserves an already-paid period; the end instant is exclusive; revocation ends access; unpaid and unverified invoice evidence fail to Free; and past-due grace is linked to the last paid period and capped at 72 hours. The returned weekly swap cap is exactly three for Free and six for Premium.
- `billing-state.policy.ts` defines explicit transition matrices for subscriptions, invoices, payment attempts, and refunds. Same-state delivery is idempotent, older provider evidence is stale, and terminal/out-of-order regressions are invalid. Failed payment attempts remain terminal so retry uses a new attempt; a failed refund may be resubmitted through its same durable request.
- `billing-idempotency.policy.ts` validates provider/environment/event ID and lowercase SHA-256 payload identity, classifies insert, exact duplicate, or same-ID/different-payload conflict, and creates deterministic bounded outbound operation keys. It performs no signature verification or provider I/O; those belong to the later webhook/adapter phase.
- `billing-money.policy.ts` requires safe integer minor units within PostgreSQL integer bounds, uppercase ISO currencies, bounded cumulative refunds, explicit ledger signs, and overflow-safe sums.
- `nutritionist-compensation.policy.ts` gives each eligible work kind fixed relative units, independent of approve/reject/escalate outcome. Duplicate source actions receive no second award; correction uses a signed append-only reversal linked to one original award. Compensation uses a base retainer, capped net workload units, fixed allowance bands, and approved adjustments. Payouts cannot exceed an approved statement or cross currencies, and preparation/review/approval/submission actors must differ.
- These policies are not wired into the current swap, review, API, or admin workflows. Existing behavior is unchanged; `NutritionistProfile.totalVerified` remains present but is not treated as compensation evidence by the new foundation.

### Deterministic evidence and boundary

- Prisma format and validation pass with a process-local non-routable placeholder `DATABASE_URL`; no configured database connection is required or attempted. The checked-in migration begins with the exact deterministic schema-to-schema diff, contains **22 `CREATE TABLE` statements, 42 foreign keys, and 43 check constraints**, contains no `DROP`, `DELETE`, `UPDATE`, `INSERT`, or `TRUNCATE`, and does not alter any existing domain table.
- TEST-079 through TEST-082 add **32** database-free cases. The complete backend suite reports **303 registered, 302 pass, 0 fail, and 1 pre-existing clinical-policy TODO**. Backend production build passes and rewrites 42 compiled aliases; generated `dist` output remains ignored/unmodified in Git.
- No frontend/shared type changed, so frontend lint/build was not rerun. No Prisma client generation claim is made; schema syntax and relations are established by Prisma validation, while generated-client recognition remains part of a later safe migration rehearsal if needed.
- No migration was applied. No database process or configured/shared/Neon connection, seed, fixture, fake finance row, PayMongo API/account/key, webhook route, Express middleware, UI, server, email, OAuth, Gemini, deployment, or money operation was used. Ports 3000, 5000, and 3030 were untouched.
...
## 40. Disposable billing-migration rehearsal (2026-09-05)
...
### Runtime and exact isolation target

- The rehearsal resumed from exact clean commit `4ef1db827d64e0ba8ae8c88c6062ecfff5e5a373` on `feature/billing-migration-rehearsal`; that branch itself starts from billing-foundation commit `753be3d7609e6b43f249893e587c7c6a7c7bf4cf`. Local `main` and `origin/main` both remained `d17b30482398472a02eefd505950f8b42af6b223`.
- Docker Desktop client/server 29.7.2 was already running. Initial inventory was zero containers, zero images, zero volumes, and only Docker's built-in `bridge`, `host`, and `none` networks. Host port 55439 was confirmed unused before creation.
- The task created only container `nutrimind-billing-rehearsal-20260905-4ef1db8-pg`, network `nutrimind-billing-rehearsal-20260905-4ef1db8-net`, volume `nutrimind-billing-rehearsal-20260905-4ef1db8-data`, primary database `nutrimind_billing_rehearsal_20260905`, and shadow database `nutrimind_billing_rehearsal_shadow_20260905`. The three Docker resources carried the exact `com.nutrimind.rehearsal=nutrimind-billing-rehearsal-20260905-4ef1db8` label.
- PostgreSQL 16.4 ran from task-pulled image `postgres:16.4-alpine`, digest `sha256:5660c2cbfea50c7a9127d17dc4e48543eedd3d7a41a595a2dfa572471e37e64c`, bound only to loopback port 55439. No application server started; ports 3000, 5000, and 3030 were untouched. Port 3000 remains reserved for NutriMind and the unrelated Antigravity project remains on 3030.

### Migration, preservation, and schema evidence

- An isolated temporary migration directory first contained exactly the 16 committed prebilling migrations through `20260905120000_structured_safety_intake`. Normal `prisma migrate deploy` applied those 16 in order. The billing tables were absent at this baseline.
- Five synthetic local-only baseline rows covered `User`, `NutritionistProfile`, `HealthCondition`, and legacy-mapped `Allgy`. Every one of the 34 prebilling domain tables received a deterministic row-count/content-hash snapshot before the billing migration.
- The exact repository billing SQL and isolated copy both had SHA-256 `a3e8b56ca5f3ea640ecd6fb619230038e26654422c387628792a05f728779563`. A second production-path deploy then applied only `20260905180000_billing_foundation`; `_prisma_migrations` contained 17 completed, non-rolled-back migrations and the stored billing checksum matched the committed file.
- The database moved from 34 to 56 domain tables, 34 to 55 enums, 77 to 166 indexes, 45 to 87 foreign keys, and 21 to 64 checks. Those exact deltas equal 22 billing tables, 21 enums, 67 migration-declared indexes plus 22 primary-key indexes, 42 foreign keys, and 43 validated check constraints. Parsed migration names exactly matched the live table, index, foreign-key, and check inventories, including both partial unique indexes; the migration-directory no-drift comparison also covered enum types and values.
- All 34 prebilling row-count/content hashes were byte-for-byte unchanged after the billing migration. A second `prisma migrate deploy` reported no pending migrations, `prisma migrate status` reported the database current, and migration-directory-to-database diff with the isolated shadow database reported no difference.

### Constraint, regression, and cleanup evidence

- Added `backend/scripts/billing-migration-rehearsal.sql`, a reusable local-only psql audit that runs entirely inside one transaction and ends with `ROLLBACK`. It constructs a valid synthetic collection, webhook, ledger, entitlement, work-credit, statement, adjustment, and payout graph; verifies environment-scoped duplicate identifiers; verifies nullable user ownership after deletion; and verifies restrictive evidence relations.
- Eighteen invalid operations were rejected with their exact expected SQLSTATE and constraint name. Coverage includes positive integer money and ISO currency, one active price/current subscription, invoice/refund bounds, one transaction per attempt, provider-event uniqueness/environment/hash, signed ledger entries, entitlement ranges, work-credit reversal shape, distinct compensation actors, payout method/reference shape, and restrictive parent deletion. Nine explicit postconditions verify environment scoping, evidence relations, and ownership nulling.
- The audit transaction rolled back and left zero billing rows. The five baseline rows were then deleted by exact synthetic IDs, leaving zero matching rows. Prisma schema validation passed. The full backend suite remained **303 registered, 302 pass, 0 fail, and 1 pre-existing clinical-policy TODO**; the backend production build passed and rewrote 42 path aliases. No frontend source changed, so frontend checks were not repeated.
- Both exact databases were dropped before container teardown. Label checks preceded removal of the exact container, network, and volume. Because the PostgreSQL image did not exist before the task, that exact pulled image was also removed. The temporary credential file and migration workspace were deleted after absolute-path verification. Final Docker inventory returned to zero containers, zero images, zero volumes, and only the same built-in `bridge`, `host`, and `none` network names/drivers. The Docker-managed `bridge` ID rotated from `878124901b99` to `6b5c04ec9197` during the session while the `host` and `none` IDs remained stable; Docker returned no retained network events for the 45-minute window. No unrelated user-defined container, image, volume, or network existed before or after the rehearsal.
- Every database-capable command resolved to the explicit loopback target. The first baseline Prisma invocation printed its standard `.env`-load notice, but its process-local `DATABASE_URL` override and reported datasource were `127.0.0.1:55439`; no configured connection string was displayed or contacted. Subsequent Prisma operations ran from the isolated temporary workspace. No shared/Neon database, PayMongo/provider API/account/credential, money operation, seed, Gemini, email, OAuth, deployment, UI, or application server was used.
...
## 43. Budget and ingredient-price foundation (2026-09-06)
...
**Change ID:** CHG-20260906-01

**Verification IDs:** TEST-090, TEST-091, TEST-092, TEST-093, TEST-094, TEST-095

**Documentation ID:** DOC-035

### Verified source and repository boundary

- Official Philippine sources were reviewed on September 6, 2026. PSA's current 2018-based/new-geographic-code Retail Price Survey collection exposes eleven monthly OpenSTAT matrices (`0042M4ARN01.px` through `0042M4ARN11.px`) across national, regional, provincial, and selected-city geography. Its official API supports structured anonymous queries and multiple formats under a documented 10-request-per-10-second limit; PSA pages state CC BY 4.0 unless otherwise noted. A metadata-only call succeeded, but no price cells or bulk dataset were downloaded or imported.
- DA Bantay Presyo publishes weekly average retail-price PDFs for named NCR markets; the index was current through August 24-30, 2026. Rows retain commodity, specification, unit, and market-group context, but no structured API or stable commodity identifier was documented and exact reuse terms were not found on the reviewed page/PDF. Automated ingestion and reuse remain unverified.
- DTI's reviewed February 1, 2025 BNPC bulletin supplies nationwide-unless-qualified suggested prices for branded/package-specific goods. It is a policy guide rather than an observed-price series, offers no documented API/stable SKU ID, and its former latest-bulletin URL returned 404. The site says public-domain unless otherwise stated, but per-bulletin machine-readable terms were absent. Automated ingestion remains unsupported by current evidence.
- Code/schema inspection confirms FNRI `FoodItem` rows contain nutrition only. Meal/library/grocery ingredients use optional quantities and free-text units; missing/legacy quantities remain possible. FNRI linkage proves nutrition identity, not retail commodity, preparation, package, geography, or freshness equivalence. Full findings and links are in `docs/BUDGET_PRICE_FOUNDATION.md`.
...
- TEST-090 through TEST-095 add 19 deterministic cases. The full backend suite reports **362 registered, 361 pass, 0 fail, and 1 pre-existing clinical-policy TODO**. Backend production build, Prisma format/validation against a process-local non-routable placeholder, the schema-to-schema generated-prefix comparison, static migration audit, offline production-dependency audit, and diff/credential/link checks pass.
...
- The shared-development acceptance for exactly `20260906120000_ingredient_price_foundation` and `20260906150000_harden_ingredient_price_normalization` is now recorded in section 45. It applied no price data.
...
## 45. Shared-development ingredient-price migration acceptance (2026-09-06)
...
**Change ID:** CHG-20260906-03

**Verification ID:** TEST-097

**Documentation ID:** DOC-037

### Authorization and read-only preflight

- The owner explicitly authorized exactly the two rehearsed price migrations from clean commit `7fd21bfcd8781086e90a3e67162ffc1f9e9bc3da` on `feature/budget-price-shared-migration`. Sanitized configuration and a live identity read classified the destination as the previously accepted TLS-required pooled Neon development endpoint, database `neondb`, schema `public`, with hostname fingerprint `6f48da70b1ce`. No credential, username, connection string, or complete hostname is retained in this record.
- Read-only history inspection found exactly 17 finished, non-rolled-back migrations through `20260905180000_billing_foundation`. Every stored checksum matched the unchanged canonical SQL content; historical Windows checkouts use LF, CRLF, or unchanged raw byte serialization, so validation accepted only a stored hash of one exact byte form of the same committed file. The only pending directories were `20260906120000_ingredient_price_foundation` and `20260906150000_harden_ingredient_price_normalization`.
- The foundation and hardening files matched exact SHA-256 values `54ee88a482cb6bfaa001126cff31c29092efeab1e5ac80c0505176aab210ff7d` and `f3b92e5cc7fb4291c344096275e100083a298ec444af663254a616d5aefd4f11`. All six price tables were absent. Privacy-safe count plus content hashes were captured for all 56 existing domain tables, together with exact tables, columns, indexes, constraints, enums, and triggers; no row content was emitted or stored.

### Baseline drift decision

- Direct comparison to the Prisma datamodel reported two `MealPlan` indexes that `20260831090000_production_workflow_hardening` creates and shared development retains, but `schema.prisma` omitted: `(highRiskReviewRequired, reviewApprovalCount, status)` and `(requiresSafetyRevalidation, status)`. The diff's “removed index” direction described transforming the physical database into the incomplete datamodel. DEF-031 records this metadata mismatch and section 46 records its later code-only correction.
- The bounded price-migration stop condition was cleared through the authorized migration-derived method. A task-owned PostgreSQL 16.4 database reconstructed all 17 canonical migrations, and read-only database-to-database comparison against shared development returned `No difference detected`. The price deployment therefore started from the exact schema produced by accepted migration history; DEF-031 was neither changed nor silently folded into that phase.

### Exact deployment and postflight

- The normal production path, `prisma migrate deploy`, applied exactly `20260906120000_ingredient_price_foundation` followed by `20260906150000_harden_ingredient_price_normalization`. No other migration or SQL path ran against shared development.
- Immediate read-only postflight found exactly 19 finished, non-rolled-back migration rows. The two new stored checksums exactly matched their required SHA-256 values. The catalog contained precisely the six price tables, eight price enum types with 34 values, 15 declared indexes plus six primary-key indexes, ten foreign keys all using restrictive deletion, 14 checks including both strict normalization-pair constraints, and six append-only triggers.
- Every new price table contained zero rows. All 56 pre-price table count/content hashes matched the preflight set; the aggregate snapshot-set hash remained `128c5621fd98d171e59878eb7adfac318354b1162eed71fd1ac89980f2eefc57`. Pre-price columns, indexes, constraints, enums, and triggers also matched their exact preflight inventory.
- A task-owned local database then reconstructed all 19 migrations. Database-to-database comparison against shared development again returned `No difference detected`. A second shared `migrate deploy` reported no pending migrations, final status reported the schema up to date, and a second read-only audit produced the same migration-history, 56-table snapshot, and full schema-inventory hash `0a4fdf033fc4d233b9c66be99959028fef690db5ebd14d909001fe5bf89eb71a` as the first postflight.

### Preserved boundaries and cleanup

- The full backend suite reports **363 registered, 362 pass, 0 fail, and 1 unchanged clinical-policy TODO**. Backend production build, Prisma validation against a process-local non-routable placeholder, offline production-dependency audit, documentation whitespace/scope checks, and migration hash checks pass. No frontend source changed.
- No synthetic/destructive constraint probe, seed, fixture, source, publication, geography, commodity, mapping, observation, price value, or other domain row was written to shared development. Shared writes were limited to the two authorized additive migrations and their Prisma history rows.
- No PSA/DA/DTI import, invented price, PayMongo/provider call or credential, Gemini, email, OAuth, deployment, application server, browser, or port 3000/5000/3030 was used. Frontend and runtime source were unchanged.
- The exact local baseline database, labeled container, network, volume, initially absent PostgreSQL image, generated local credential, and verified temporary audit directory were removed. Final Docker inventory contains zero containers, volumes, and images; only built-in `bridge`, `host`, and `none` networks remain, and local port `55442` is free.

### Next bounded gates

- After explicit source-license and product-scope acceptance, the next price phase is one small internal PSA/OpenSTAT basket drawn from exact ingredients used most often by the 51 managed meals, one explicit geography, and current monthly observations with safe bases. Require reviewed exact FNRI mappings, full publication/geography/unit provenance, append-only ingestion, and measured meal-slot plus grocery item-count coverage before expansion.
- DA/DTI ingestion, public endpoints, frontend estimates, scheduled fetching, budget ranking integration, user geography, package-to-edible-weight mappings, and any Premium claim remain separately gated. PayMongo Phase 3B remains paused indefinitely.
- DEF-031 is resolved in section 46 by exact mapped Prisma declarations; no database migration or shared write was required.

## 46. MealPlan index datamodel reconciliation (2026-09-06)

**Defect ID:** DEF-031

**Change ID:** CHG-20260906-04

**Verification ID:** TEST-098

**Documentation ID:** DOC-038

### Contradiction resolution

- Primary evidence resolves the earlier conflicting reports. Canonical migration `20260831090000_production_workflow_hardening` creates `MealPlan_revalidation_status_idx` on `(requiresSafetyRevalidation, status)` and `MealPlan_highRisk_review_idx` on `(highRiskReviewRequired, reviewApprovalCount, status)`. Git history shows both were introduced in commit `098f71d1d3fb1b1affe200a66ab0d3d80e2d8b20`; its simultaneous Prisma model update added the three fields but omitted both `@@index` declarations, and no later schema commit added them.
- Before correction, generated Prisma metadata mirrored the omission. A sanitized read-only query against the accepted pooled Neon `neondb`/`public` development schema returned exactly both named btree indexes with the migration-defined ordered columns. The prior price-acceptance diff was interpreted backward: “removed index” meant that transforming the physical database into the datamodel would drop the indexes, so it proved physical presence rather than absence.

### Smallest code-only correction

- Added `@@index([requiresSafetyRevalidation, status], map: "MealPlan_revalidation_status_idx")` and `@@index([highRiskReviewRequired, reviewApprovalCount, status], map: "MealPlan_highRisk_review_idx")` to `MealPlan`. The explicit `map` values preserve exact historical database names and prevent Prisma from proposing duplicate default-named indexes.
- No migration was created or edited, and no shared database write was performed. The correction describes existing accepted physical state in Prisma schema and generated metadata.
- Current query evidence differs by index. `scripts/enqueue-legacy-plan-revalidation.ts` directly filters `requiresSafetyRevalidation = true` and `status = APPROVED`, matching the revalidation index's leading equality fields before its scheduled-date predicate. The nutritionist queue fetch currently filters `status = PENDING_REVIEW`, orders by creation time, then prioritizes `highRiskReviewRequired` plus `reviewApprovalCount` in memory. The high-risk index therefore has no direct full-prefix query claim in current code; it remains declared because it already exists in canonical migration history and shared development. Any later removal requires a separately authorized database migration and workload decision.

### Verification and preserved boundaries

- TEST-098 checks both exact schema mappings against the immutable production-hardening migration and distinguishes the direct revalidation query from in-memory high-risk review priority. Prisma format/validation and client generation reproduce both mapped declarations in generated metadata.
- A task-owned PostgreSQL 16.4 database applied all 19 canonical migrations. It contained exactly both named indexes, 19 completed migration rows, and zero `MealPlan` rows. Comparing that migration-derived database with the corrected datamodel reported `No difference detected`; a second local deploy reported no pending migrations. A separate read-only comparison of shared development to the corrected datamodel also reported `No difference detected`.
- The full backend suite reports **365 registered, 364 pass, 0 fail, and 1 unchanged clinical-policy TODO**. Backend production build passes with 47 aliases rewritten, and the offline production-dependency audit reports zero vulnerabilities.
- The exact local database, labeled container, network, volume, initially absent PostgreSQL image, generated local credential, and verified temporary directory were removed. Final Docker inventory contains zero containers, volumes, and images; only built-in networks remain and port `55443` is free.
- No shared migration or other write, domain data read, EXPLAIN/ANALYZE/load against shared development, historical migration edit, PSA/DA/DTI import, price data, PayMongo/provider call or credential, Gemini, email, OAuth, deployment, application server, browser, frontend change, or ports 3000/5000/3030 was used.

## 47. PSA/OpenSTAT Cebu City ingestion foundation (2026-09-06)
...
**Change ID:** CHG-20260906-05

**Verification ID:** TEST-099

**Documentation ID:** DOC-039

### Official source, license, and geography preflight

- The official PSA Retail Price Survey page states that it collects retail prices at pre-selected major trading centers and links the eleven category series. Its footer licenses site data/content under Creative Commons Attribution 4.0 International unless otherwise stated. The OpenSTAT matrix metadata reports `Copyright: No` and identifies PSA as the source.
- The official OpenSTAT API guide documents anonymous JSON POST access, CSV and other response formats, and a maximum of 10 requests per 10 seconds. The current 2018-based/new-geographic-code collection lists matrices `0042M4ARN01.px` through `0042M4ARN11.px`, monthly through 2026.
- Live metadata for all eleven matrices contains the exact selected-city entry `....City of Cebu` with external code `072217000`, distinct from province `....Cebu` (`072200000`). This phase therefore uses City of Cebu directly and does not substitute Region VII, Cebu province, or national data.
- Six bounded API POSTs returned unchanged CSV subsets for City of Cebu, January-August 2026. Exact request bodies, raw CSV bytes, retrieval timestamp, source URLs, CC BY 4.0 attribution, and SHA-256 checksums are committed under `backend/data/psa-openstat/2026-09-06-cebu-city`. PSA's `..` missing marker is preserved verbatim.

### Basket and manually reviewed mappings

- Frequency was calculated from the 51-definition managed catalogue: 195 ingredient occurrences and 36 distinct names. The bounded basket selects well-milled rice, carrot, potato, green munggo, cucumber, tomato, chicken breast, and tilapia. Their catalogue targets account for 93/195 ingredient rows (47.69%) and occur in 44/51 meals (86.27%).
- Seven source commodities have manually reviewed exact raw FNRI identities: `Rice, well-milled`, `Carrot`, `Potato`, `Cucumber`, `Tomato`, `Chicken breast`, and `Tilapia`. Green munggo remains `AMBIGUOUS`, with dried-green and fresh-green FNRI candidates retained because the PSA label does not declare preparation. No fuzzy or label-only automatic mapping is used.
- Catalogue exact-identity coverage is intentionally lower because rice, carrot, potato, and munggo recipes use separate boiled FNRI foods. Tomato, cucumber, chicken breast, and tilapia provide 49/195 exact same-identity rows (25.13%) across 37/51 meals (72.55%). Current source cells exist only for tomato and cucumber among those four, leaving actual available-cell identity coverage at 43/195 rows (22.05%) across 35/51 meals (68.63%).
- The snapshot contains 40 positive observations across five commodities and 24 explicit missing cells. Available period ends span January 31 through August 31, 2026; the importer records those dates and applies no implicit freshness threshold. Munggo, chicken breast, and tilapia have no available City of Cebu value in any selected month. Missing observations are reported and not written as zero, inferred, regional, or national prices.

### Deterministic ingestion implementation

- `psa-openstat-price-ingestion.ts` strictly validates the versioned manifest, HTTPS/source/license fields, unique matrix/commodity identities, exact checksums, request format, RFC-style CSV structure, exact geography/commodity labels, monthly headers, positive decimal prices, and explicit missing markers. Decimal strings become integer PHP centavos without binary floating-point money arithmetic.
- Coverage reporting separates basket selection, exact catalogue identity, and actually available source cells. Counts are checked against the live source-controlled catalogue so catalogue drift fails the snapshot preflight.
- `psa-openstat-price-ingestion.service.ts` resolves every named FNRI food exactly once, preserves ambiguous candidate IDs, inserts or verifies stable immutable identities, and rejects conflicting source/geography/commodity/publication/mapping/observation evidence. Later snapshot mappings/observations create append-only successor links rather than updates.
- `import-psa-openstat-prices.ts` defaults to a database-free dry run. `--apply` rejects non-loopback database hosts until a separately authorized shared-import gate. It adds no route, public endpoint, UI, scheduled fetch, or runtime price repository.

### Disposable acceptance evidence

- A task-owned PostgreSQL 16.4 container applied all 19 canonical migrations and seeded 1,537 current FNRI rows. The committed snapshot dry-run passed, then first apply created exactly 1 source, 1 City geography, 6 publications, 8 commodities, 8 mappings, and 40 PHP point observations. Exact replay classified every row as existing and created zero rows.
- A temporary, clearly synthetic local correction copy changed one cell and retrieval identity only to exercise successor logic. It created 6 new publications and 40 successor observations; every new observation linked its prior active row. Exact replay of the probe created zero rows. The probe is not committed, is not official source evidence, and is removed with the disposable environment.
- Post-probe audit found 12 publications, 80 observations, 40 current rows, 40 rows with predecessor links, 7 exact mappings, 1 ambiguous mapping, zero non-PHP observations, and zero ranges with unequal endpoints. The second migration deploy reported no pending migration, status was current at 19, and datamodel comparison reported `No difference detected`.
- The full backend suite reports **375 registered, 374 pass, 0 fail, and 1 unchanged clinical-policy TODO**. Backend build passes with 49 aliases rewritten. Script compilation passes, and the offline production-dependency audit reports zero vulnerabilities.

### Preserved boundaries and next gate

- Shared Neon was not accessed or written. No migration/schema change, historical migration edit, DA/DTI/commercial/crowdsourced source, AI/Gemini price, provider/PayMongo call, email, OAuth, deployment, application server, browser, frontend change, or ports 3000/5000/3030 was used.
- The official observations price purchased raw commodity kilograms. Managed-meal quantities often describe cooked edible grams, and FNRI edible portions do not alone establish cooking yield or retail waste conversion. RISK-024 therefore blocks connecting these observations to meal/grocery cost calculations until reviewed conversion evidence exists.
- A separate shared-development data-import gate is warranted. It should re-check shared price-table state, immutable source metadata, hashes, exact FNRI resolution, expected row counts, and idempotent/rollback behavior. Public/runtime use remains a later phase even if shared import is accepted.

## 48. PayMongo Phase 3B TEST checkout acceptance (2026-09-06)
...
**Change ID:** CHG-20260906-06

**Verification ID:** TEST-100

**Documentation ID:** DOC-040

### Official contract recheck

- Official PayMongo material was accessed on September 6, 2026. The current Hosted Checkout guide and quick start recommend server-side `POST /v2/checkout_sessions`, Basic authentication with the TEST secret as username and an empty password, `data.attributes.checkout_url` as the redirect, and signed `checkout_session.payment.paid` evidence as payment authority. The testing guide confirms TEST Hosted Checkout support. Idempotency guidance limits keys to 255 characters, retains an executed result for 24 hours, replays exact retries, and rejects changed parameters.
- The official payment-method resource names `card` and `paymaya`, while the go-live checklist makes actual method activation account-specific. This acceptance requested only `card`; the sandbox API accepted it. Maya, GCash, Subscription API, refunds, and live capabilities were not inferred or tested.
- The legacy create-session reference still shows a `/v1` URL while recommending `/v2`. The current quick start and actual accepted response match the `/v2` adapter. Official checkout pages do not provide a complete checkout-specific account-capability error taxonomy, so only a bounded machine-readable provider code is retained when present; provider detail text is discarded.
- Claim-level primary links and the documentation discrepancy are recorded in ADR-017 section 3.7. No provider credential or authorization header is retained in documentation.

### Server-side checkout slice

- `PAYMONGO_INTEGRATION_ENABLED` remains false by default and controls checkout only. `PAYMONGO_WEBHOOK_ENABLED` is a separate false-by-default capability requiring its own signing secret and version. Both modes remain TEST-only and production enablement fails closed. Explicit configured methods accept only `card` and/or `paymaya`; this phase used card only after the sandbox request was accepted.
- `NodeHttpsBillingTransport` uses native TLS, follows no redirects, enforces the existing five-second timeout and 64 KiB response ceiling, and emits no logs. `PaymongoGateway` fixes the `/v2` API and hosted-checkout origins, sends no user or health data, accepts only session ID, checkout URL, and `livemode: false`, and reduces provider failures to stable internal categories plus an optional bounded machine code.
- The additive migration creates `BillingCheckoutRequest` and `BillingCheckoutAuditEvent` plus two enums. A request is scoped to one user and immutable price, binds its price/method/return configuration into a SHA-256 request hash, holds a one-minute claim lease, reuses one opaque provider idempotency key and reference across retries, and refuses automatic provider retry after a conservative 23-hour safety window. State-shape, TEST-only, identifier, hash, attempt, URL, failure-code, uniqueness, and preservation constraints are database-enforced.
- The Prisma repository distinguishes create, replay, collision, in-progress, retryable reclaim, terminal failure, and provider-idempotency expiry. Raw claim tokens are returned process-locally while only their SHA-256 hashes are stored. Success stores the provider session ID and hosted URL; failures store only sanitized codes. Audit events append claims, reclaims, replays, collisions, in-progress denials, session creation, and failure classifications.
- The acceptance-only local catalogue contains one exact `PREMIUM` monthly TEST price at PHP 199.00. It is explicitly a sandbox/demo placeholder and is neither inserted by the migration nor approved as a commercial price. Runtime eligibility also requires an active USER who is not suspended. No frontend, public catalogue endpoint, entitlement resolver, payment projection, scheduler, or production price was added.

### Provider and persistence acceptance

- One and only one PayMongo API request created a provider-hosted TEST checkout with `livemode: false`. No payment was attempted or completed. Replaying the exact user/idempotency request returned the stored session, and the counting transport remained at one provider call.
- Repository scenarios denied an ADMIN and an unknown user, rejected a changed-hash collision, converted a synthetic transient failure to `RETRYABLE`, reclaimed it with a new local lease token, and reused the exact provider idempotency key and reference. A terminal synthetic provider classification could not be retried.
- The disposable database contained three checkout requests representing the collision, retry/terminal, and actual success scenarios, with nine append-only audit events. One actual request was `SUCCEEDED`; no secret occurred in the persisted request/audit serialization. Four direct malformed probes for LIVE environment, invalid hash, impossible success shape, and zero attempts were rejected by database checks.
- Checkout creation and browser return produced exactly zero `UserSubscription`, `BillingInvoice`, `PaymentAttempt`, `BillingTransaction`, `FinancialLedgerEntry`, and `EntitlementGrant` rows. Premium therefore remains unavailable without future verified webhook and reconciliation evidence.

### Verification, isolation, and next gate

- Prisma format, validate, and generate pass. All 20 migrations deploy to task-owned PostgreSQL 16.4; the second deploy is empty, migration status is current, and database-to-datamodel comparison reports `No difference detected`.
- The backend suite reports **381 registered, 380 pass, 0 fail, and 1 unchanged clinical-policy TODO**. The production build passes with 49 alias rewrites, script compilation passes, and the offline production-dependency audit reports zero vulnerabilities.
...
## 51. Concrete PayMongo TEST retrieval and local projection acceptance (2026-09-06)
...
**Change ID:** CHG-20260906-09

**Verification ID:** TEST-104

**Documentation ID:** DOC-043

### Provider contract and implementation boundary

- Official PayMongo material was rechecked on September 6, 2026. Checkout creation guidance currently recommends `POST /v2/checkout_sessions`, while the authenticated retrieval reference specifies `GET /v1/checkout_sessions/{id}`. The [retrieve Checkout Session reference](https://docs.paymongo.com/re/reference/get_checkout_sessions) and [Checkout Session resource](https://docs.paymongo.com/reference/checkout-session-resource) state that secret-key retrieval includes payment details; Checkout Session status itself remains `active` or `expired` and is not paid-state authority. The adapter therefore requires one `paid` Payment and its matching `succeeded` [Payment Intent](https://docs.paymongo.com/reference/the-payment-intent-object), in addition to the already authenticated `checkout_session.payment.paid` inbox event.
- `PAYMONGO_RECONCILIATION_ENABLED` is an independent false-by-default capability. It can run only in TEST mode, rejects production enablement, and requires the existing TEST secret without exposing it. `PaymongoReconciliationGateway` fixes the origin to `https://api.paymongo.com`, path to `/v1/checkout_sessions/{id}`, method to GET, Basic authentication, no request body, no redirects, a five-second timeout, a 64 KiB response ceiling, and JSON content type.
- The parser requires exact session identity, `livemode: false`, a bounded merchant reference, exactly one paid payment, one matching succeeded payment intent, exact positive integer PHP amounts on both objects, and valid provider timestamps. It returns only session/reference, payment/payment-intent IDs and normalized statuses, amount/currency, TEST mode, paid time, and provider update time. PII, card/payment-method data, line items, billing details, provider response bodies, authorization headers, and secrets are discarded.
- Authentication, not-found, unsupported response, unpaid/processing, live-mode, session-mismatch, malformed, oversize, and transient failures become stable sanitized codes. Only timeout/rate-limit/server/transport failures are retryable. The existing projection processor uses these classes without weakening its binding, lease, quarantine, amount, currency, timestamp, ledger, or entitlement policies.
- Runtime constructs the processor only when the reconciliation switch is enabled. No route, application bootstrap, cron service, scheduler, or public command invokes it. The committed acceptance script is an explicit operator-only harness and enforces at most one provider request per run.

### Exact provider and local acceptance evidence

- The local event used for processing was explicitly reconstructed as `LOCAL_ACCEPTANCE_RECONSTRUCTION_NOT_PROVIDER_DELIVERY` from previously accepted identities: authentic event `evt_aF8YduaEpB1vLvC1NRM9rc8z`, session `cs_2c030de77711f9285b90547f`, and merchant reference `nmco_ca452ab1fbdb50918247b039f9a99a0b`. This reconstruction is not claimed as a second authentic webhook delivery; it contains no raw prior payload or signature.
- Exactly one authenticated read-only PayMongo request retrieved that existing session. The allow-listed response reported payment `pay_3PVKHghbxy3jcYLDDbRwfFPW` as paid, intent `pi_U7FX3pA65Y8kv4Y8pfNQt4fK` as succeeded, PHP 19,900 minor units, `livemode: false`, paid at `2026-09-06T05:43:27.000Z`, and provider-updated at `2026-09-06T05:43:28.000Z`. No checkout, webhook, customer, subscription, cancellation, refund, payout, or other provider mutation was made.
- The real gateway and real Prisma processor projected exactly one non-renewing `ONE_TIME_ACCESS_PERIOD` from `2026-09-06T05:43:27.000Z` through the exclusive end `2026-10-06T05:43:27.000Z`. Persistence contains one `NON_RENEWING` subscription, one paid PHP 19,900 invoice, one succeeded attempt, one transaction, one Premium grant, and two PHP 19,900 ledger entries in one batch. `CASH_CLEARING` debit and `DEFERRED_REVENUE` credit balance to zero. Provider customer, provider subscription, and provider invoice IDs are null.
- Immediate processor replay returned `NO_WORK`; the counting transport remained at exactly one request. Premium resolved immediately before the exclusive end and Free at the end. A browser return with no grant resolved Free. Provider customers, cancellations, refunds, nutritionist work credits, compensation statements, and compensation payouts all remained at zero; no reconciliation issue was opened.

### Verification, isolation, and remaining gates

- TEST-104 covers the fixed GET request, Basic TEST authentication, URL/identifier safety, redirects, timeout/body bounds, content type, strict allow-list, live/mismatch/unpaid/contradictory/malformed responses, sanitized terminal/transient classes, independent configuration, and absence of route/scheduler reachability. The full backend suite reports **402 registered, 401 pass, 0 fail, and 1 unchanged clinical-policy TODO**. The backend production build and script compilation pass.
- A task-owned PostgreSQL 16 container on loopback port 55448 applied all 21 canonical migrations. A second deploy reported no pending migrations, status was current, and database-to-datamodel comparison reported `No difference detected`. Post-acceptance database reads confirmed the exact projection, completed work item with one attempt, balanced ledger, and zero reconciliation issues.
...
- The historical payment-processing runbook documented the safe disabled state and recovery limits. It was removed from the active branch with the September 22 product reversal and remains available in Git history.
...
## 55. Ingredient conversion-evidence foundation and exact catalogue coverage (2026-09-06)

**Requirement IDs:** REQ-022, REQ-030

**Architecture decision:** ADR-024

**Risk ID:** RISK-024

**Change ID:** CHG-20260906-13

**Verification IDs:** TEST-130 through TEST-136

**Documentation ID:** DOC-047

### Authoritative source decision

- DOST-FNRI's official 47th Seminar Series abstract reports 173 Philippine yield factors derived by weighing selected Metro Manila foods before and after defined cooking methods. The public abstract does not contain the row-level factors, exact item/preparation protocol, dispersion, or applicable reuse terms. It is recorded as `LICENSE_REVIEW_REQUIRED` metadata and supplies no number.
- USDA FNS Food Buying Guide rows define purchased-to-edible relationships for exact product forms and trim, peel, cut, cook, and drain procedures. USDA ARS Release 2 defines cooking yield as cooked hot weight divided by raw weight for exact meat/poultry cuts and methods. These authoritative US rows do not exactly match the current Philippine PSA commodity plus FNRI catalogue preparation pairs and are recorded as metadata only.
- FAO/INFOODS unit-conversion and food-matching guidelines support explicit denominator, density, identity, edible-portion, and preparation controls. They are methodological references rather than NutriMind factor data, and their publication terms still require a use review.
- The committed strict review artifact contains five sources, one shared access timestamp, source/version/geography/method/factor-meaning/food/preparation/uncertainty/license fields, an explicit omission reason for every source, and `pilotEvidence: []`. No blog, retailer, search snippet, AI output, or inferred ratio is present. The acquisition workflow requests DOST-FNRI's full 173-row artifact, protocols, uncertainty, and written reuse terms before any import.

### Additive evidence model and policy

- Migration `20260906234500_ingredient_conversion_evidence` adds `IngredientConversionSource` and `IngredientConversionEvidence` plus six enums. The source records exact provenance, use status, attribution, version/date/geography/methodology, and artifact hash. Evidence records an exact row/version between explicit `PURCHASED_AS_SOLD`, `RAW_EDIBLE`, or `COOKED_EDIBLE` nodes, direction, source and FNRI identities, preparation codes, units, rational lower/upper factors, uncertainty, effective/observation dates, review, and supersession.
- No price or conversion shortcut was added to `FoodItem` or `IngredientPriceCommodity`. Relations are explicit and restrictive. SQL rejects malformed hashes/URLs, source-use claims without license and artifact checksum, non-positive or inverted factors, invalid kind/basis/unit shapes, missing identity bindings, incomplete review evidence, invalid date order, self/branched supersession, and factors sourced from metadata-only or license-review records. Both new tables reject update and delete operations; corrections require append-only revisions. The migration inserts no row.
- The pure evaluator has no Prisma, network, AI, route, or UI dependency. Clinical exclusion runs first. Eligible paths require reviewed, current, non-superseded exact identity/preparation evidence. Canonical mass/volume conversion does not cross unit families. Forward-only rows cannot be reversed. Duplicate conflicts, cycles, repeat use, multiple valid paths, incompatible bases, malformed/plausibility failures, stale/future evidence, and identity/preparation extrapolation return stable unavailable reasons.
- Bounds compose with integer rational arithmetic. The converted-cost helper consumes safe integer quantities and PHP centavos, calculates the purchased range conservatively, and rounds the lower centavo bound down and upper bound up. Household measure-to-mass evidence must be an exact food and preparation row; no generic cup/spoon density is inferred.

### Exact 51-meal coverage result

- The deterministic report revalidates the checksum-pinned PSA snapshot, source review, and all 51 managed meals/195 ingredient rows as of September 6, 2026. The source basket covers 93 rows/44 meals. Exact raw FNRI mapping covers 86 rows/43 meals. A current exact-mapped price and valid gram/kilogram basis cover 80 rows/43 meals.
- Cucumber and tomato share the same named food identity at both boundaries for 43 rows/35 meals, but the retail basis is still `PURCHASED_AS_SOLD` and the catalogue basis is `RAW_EDIBLE`. They require an explicit edible-yield bridge. Thirty-seven current priced rows for boiled rice, carrot, and potato require a purchased-to-cooked chain. The reviewed conversion-evidence and conversion-usable layers both contain 0 rows/0 meals.
- Whole-meal status is therefore 0 complete, 0 partial, and 51 unpriced. The report lists all 195 missing ingredient rows: 102 lack a source-basket commodity, 7 lack an exact FNRI mapping, 6 lack a current price observation, 43 lack reviewed purchased-to-raw evidence, and 37 lack a reviewed purchased-to-cooked chain. Missing evidence remains visible rather than becoming zero cost or guessed coverage.

### Disposable database, regression, and cleanup evidence

- A task-owned PostgreSQL 16.4 container on loopback port 55453 first reconstructed the 21-migration parent. Privacy-safe count/content hashes were captured for all 64 parent domain tables. Applying the one new migration produced exactly 22 completed migrations. Its file and stored checksum matched at `cf6c15f3f6de31b9b4b7120f3fd716e846cccb0d003a5bffa7b13b5fc24af2a1`.
- The conversion inventory contained 2 empty tables, 6 enums with 25 values, 8 indexes including primary keys, 5 restrictive foreign keys, 10 checks, and 3 triggers. A rollback-only graph inserted a valid source and two valid synthetic conversion rows, then proved seven expected failures for source-use rights, factor positivity/plausibility, review evidence, uniqueness, and both append-only tables. Rollback restored both tables to zero rows.
- All 64 parent-table hashes matched the pre-migration snapshot hash `3747c0740b58c2824570e1a0bcc0505865af3a31e1c75d7c006b9c19e9e171c9`. A second deploy was empty, migration status was current, and database-to-Prisma datamodel comparison reported `No difference detected`.
- The full backend suite reports **447 registered, 446 pass, 0 fail, and 1 unchanged clinical-policy TODO**. Backend Prisma format/validate/generate, production build, script compilation, conversion coverage generation, and offline production dependency audit pass. Frontend lint, production build, and offline production dependency audit also pass with no frontend source change. Both audits report zero vulnerabilities.
- The task-owned container, anonymous volume, newly pulled PostgreSQL image, loopback port, and verified temporary directory were removed. Shared Neon, provider/PayMongo, Gemini, email, OAuth, browser automation, deployment, application servers, and ports 3000/5000/3030 were untouched. `main` and `origin/main` remained at `d17b30482398472a02eefd505950f8b42af6b223`.

### Remaining gate

- No numerical conversion factor is accepted. Before a shared or runtime import, obtain a row-level artifact, explicit intended-use rights, unchanged checksum, and an exact source-commodity/FNRI/preparation match with named review. Shared price import, conversion import, repository/API/UI integration, minimum public coverage, user geography, source precedence, scheduled retrieval, and any Premium budget claim remain separate decisions.

## 56. Nutritionist compensation administration and manual payout evidence (2026-09-06)
...
**Change ID:** CHG-20260906-14

**Verification IDs:** TEST-137 through TEST-142

**Documentation ID:** DOC-048

### Work evidence and calculation boundary

- Work began from exact accepted commit `0b649daf57dcc02ceb519ae34e94741034ecac87` on `feature/nutritionist-compensation-admin`. `main` and `origin/main` remained at `d17b30482398472a02eefd505950f8b42af6b223`.
- A successful ordinary approval, ordinary rejection, or first high-risk escalation writes one `ORDINARY_PLAN_REVIEW` credit inside the same serializable transaction as the completed review decision. An independent high-risk second decision writes `HIGH_RISK_SECOND_REVIEW`. Stable source-action keys make exact replays idempotent. Opening or expiring a claim does not call the credit writer; abandoned and invalid decisions do not commit a credit.
- Corrections append one signed negative `REVERSAL` linked to the original award. Work-credit and statement-credit rows reject update/delete operations. `NutritionistProfile.totalVerified`, subscription payment state, and current mutable meal state are not calculation inputs.
- A period locks one independently activated policy version. The deterministic calculation snapshots all unstatemented period credits, applies the base retainer, caps net workload units before selecting the fixed allowance band, and includes only independently approved signed adjustments. Approved statement amounts and identity fields are immutable.

### Administration and role surfaces

- ADMIN endpoints and the responsive `/admin/compensation` workspace support strict policy drafting and independent activation, non-overlapping period opening and independent closure, statement calculation/review/approval, adjustment approve/reject decisions, reconciliation, append-only credit reversals, and manual payout preparation/approval/evidence recording.
- Policy creator/activator, period opener/closer, statement preparer/reviewer/approver, adjustment creator/checker, and payout submitter/checker constraints are recorded in services and PostgreSQL. The payout request accepts only an idempotency key; its PHP amount is the remaining approved statement amount calculated by the server.
- `GET /api/nutritionist/compensation` derives ownership only from `req.nutritionistProfileId`. `/nutritionist/compensation` shows the signed credit timeline, locked policy/version and period statement, adjustments, and payout status/reference for that profile only. It does not accept another profile ID and returns no user billing records.
- Subscription revenue and compensation are explicitly separate in both role surfaces. Capstone payout records use `MANUAL_OFF_PLATFORM`, store a bounded evidence reference only, and contain no bank/e-wallet destination, provider transfer, PayMongo call, or money movement.

### Migration, integration, and browser evidence

- The one additive migration `20260906235900_compensation_admin_workflow` adds workflow actors, adjustment status/rejection evidence, one-active-policy enforcement, lifecycle/maker-checker checks, append-only triggers, and approved-statement snapshot protection. No accepted migration changed. Its SHA-256 is `56ab3fa1c8d631282383d5a27dc62dba9471796d58ad1c64f442b0ae8c332232`.
- A task-owned PostgreSQL 16.4 container on loopback port 55439 applied all 23 migrations. TEST-142 created four unique eligible awards spanning approve/reject/escalate and high-risk second-review outcomes, replayed one award without duplication, appended one reversal, created two statements, independently approved one adjustment, and completed two manual payout records. Exact approved gross, committed payout, and recorded-paid totals were PHP 230,500 minor units with zero mismatch; provider-bound payout rows and provider calls were zero.
- The database rejected direct credit mutation and approved-statement amount mutation. A second migration deploy was empty, migration status was current, and database-to-Prisma datamodel comparison reported `No difference detected`.
- The full backend suite reports **452 registered, 451 pass, 0 fail, and 1 unchanged clinical-policy TODO**. Prisma format/validate/generate and the backend production build pass. The frontend production build generates both new routes; its first run reported two explicit-`any` and one unused-import warning in the new admin page, which were corrected before final checks.
- Authenticated in-app browser acceptance used local ports 3000 and 5000 after confirming they were free. ADMIN and NUTRITIONIST pages rendered their synthetic records at default desktop and 390x844 mobile viewports with no browser console errors and no horizontal overflow. Each role was redirected to `/unauthorized` from the other role's route. The servers were stopped immediately after acceptance.
- The task-owned container and port 55439 were removed. Shared Neon, provider/PayMongo, bank/e-wallet data, Gemini, email, OAuth, deployment, and real payout operations were untouched. Port 3030 and the unrelated project were untouched.

### Remaining gates

- The additive schema is now applied empty to shared development under TEST-149. Compensation policy amounts in disposable acceptance are synthetic and do not establish a contract, salary, employee/contractor status, tax treatment, payout schedule, dispute procedure, retention requirement, or production approval.
- Creating any work credit, policy, period, statement, adjustment, payout evidence, administrator staffing for maker-checker stages, accounting/legal approval, operational dispute and reversal procedures, deployment monitoring, and any future provider disbursement require separate authorization and evidence. Existing PayMongo capabilities remain disabled and unrelated to this manual record flow.

## 57. Integrated three-role adversarial local audit (2026-09-06)

**Change ID:** CHG-20260906-15

**Defect IDs:** DEF-032 through DEF-036

**Verification IDs:** TEST-143 through TEST-148

**Documentation ID:** DOC-049

### Scope and isolation

- Work began from exact commit `9c4e2497053c4c429093d1edc7e4b13d56950157` on `audit/full-system-e2e`. `main` and `origin/main` remained at `d17b30482398472a02eefd505950f8b42af6b223`.
- A task-labelled PostgreSQL 16.4 container exposed only loopback port 55441. Its main database plus dedicated Premium and compensation databases each applied all 23 canonical migrations. A second labelled container on loopback port 55447 hosted only the payment-projection acceptance database required by its existing safety guard.
- All accounts, applications, health entries, meals, reviews, payment records, compensation records, OTPs, invitations, and payout references were synthetic and used `example.invalid` identities. Ports 3000 and 5000 were used only for the bounded local browser/API session. Port 3030 was checked, left untouched, and never bound.
- PayMongo checkout, webhook, provider reconciliation, billing worker, Gemini, Google OAuth, SMTP, deployment, and live-money switches remained disabled. The payment-projection acceptance used an injected in-memory fake gateway; its seven counted invocations were local fixture lookups, not network/provider calls. Premium and compensation acceptance each reported zero provider calls. The local mail-capture seam avoided SMTP and rejects use outside `NODE_ENV=test` or with a relative path.
- One diagnostic Node command accidentally inherited the ignored backend `.env` before the explicit local URL guard was applied. It issued a count and one synthetic-email `findUnique` against the shared database, found no matching row, failed before its proposed update, and performed no shared write. No further shared access occurred. This was an audit-procedure violation and is retained here rather than omitted from the evidence.

### Three-role and adversarial results

- Authentication exercised whitespace/Unicode normalization, malformed email and password input, duplicate registration races, wrong/expired/resend-cooldown OTPs, successful captured OTP verification, invalid access tokens, concurrent refresh rotation/replay, and rate limiting. The double registration committed one account; concurrent refresh returned one success and one denial; invalid and replayed credentials remained sanitized.
- USER onboarding rejected contradictory goals, separator-only safety input, forged structured fields, and a vague unsupported condition. A mixed condition/allergy/intolerance/avoidance save retained six structured entries, required review, replayed idempotently, and blocked normal features until report acknowledgement. The 51-meal certified library generated a 21-slot, seven-day plan with zero Gemini calls.
- USER runtime checks covered current/detail/cross-owner access, planned-meal idempotency, invalid status, outside-meal missing input, water bounds, grocery projection, progress/history, account export, the Free three-swap cap, Premium six-swap cap, and concurrent cap reservation. A forged checkout-return query remained non-authoritative and left the Free account unchanged.
- NUTRITIONIST checks covered verified-license access, claim ownership/expiry/contention, stale/repeated decisions, rejection, ordinary credits, and two-person high-risk approval. Claim contention now converges at HTTP 200/409; a first high-risk reviewer cannot perform the second approval; completed actions create the expected outcome-neutral work credits.
- The full disposable compensation acceptance proved deduplicated completed-action awards, no claim credit, append-only reversal, policy/period/statement maker-checker separation, signed adjustment, idempotent payout preparation, two owner-scoped nutritionist statements, exact PHP minor-unit reconciliation, immutable approved snapshots, manual off-platform evidence, and zero provider-bound payout rows or calls.
- ADMIN checks covered analytics, user/professional/application lists, strict staged application review and call URL validation, invitation activation/replay, operations/audit views, bounded search/pagination, self-suspension denial, target suspension session invalidation, reinstate/login recovery, billing operations, and compensation controls.
- PostgreSQL outage simulation preserved `/health` at 200 while `/ready` changed 200 to 503 and returned to 200 after recovery. A frontend API outage left the loading gate within seconds and returned to sign-in; the recovered API accepted login again.

### Corrections and regression boundary

- DEF-032 replaces the stale fixed `30` acceptance expectation with `COMMON_MEAL_CATALOGUE.length` and moves the fixture to `example.invalid`.
- DEF-033 centralizes expected review-conflict classification and returns HTTP 409 for ownership, claim, state, second-review, and replay contention instead of HTTP 500.
- DEF-034 introduces one finite 30–300 kg policy, bounded and normalized notes, aligned check-in/legacy endpoints, and transactionally coupled profile/history changes. The pre-fix disposable probe accepted 10,000 kg; the post-fix probe returned 400 with both profile and log count unchanged.
- DEF-035 removes the public statement that budget shapes every plan because the accepted conversion report still has zero priced meals.
- DEF-036 normalizes invalid page inputs, caps list limits at 100, and bounds trimmed search at 200 characters inside admin and nutritionist service boundaries.
- TEST-143 through TEST-148 add deterministic regressions for those boundaries and for the test-only mail capture. They require no database or network.

### Browser and verification limits

- Authenticated desktop browser coverage at 1280x720 traversed all current USER, NUTRITIONIST, and ADMIN navigation routes plus landing, registration, nutritionist application, docs, billing return, and unauthorized routes. Every inspected page reported zero horizontal overflow. Theme change and keyboard Tab focus worked, role-crossing routes resolved to `/unauthorized`, malformed registration showed field-level errors, and no new browser console error occurred. The expected network warning during the deliberate API outage was recorded.
- This full audit did not dynamically re-run every page at 390x844. The connected in-app browser exposed no viewport-resize control and no Chrome browser surface was available. TEST-141 retains prior 390x844 evidence for both compensation pages; responsive classes and production compilation cover the remaining pages statically. A complete 390px interactive rerun remains a browser-environment limitation rather than a passing claim.
- Live Gemini output, OAuth, SMTP delivery, PayMongo/provider traffic, real payment/payout, shared-database mutation, deployment, production monitoring, accessibility conformance, and clinical review were not tested and remain outside this local audit.
- Final teardown removed both labelled containers, their two exact anonymous volumes, the newly pulled PostgreSQL image, the local server wrapper, and the synthetic mail-capture file. Ports 3000, 3030, 5000, 55441, and 55447 were all free after cleanup; no task-labelled container, task volume, temporary file, or image remained.

## 58. Final shared-development conversion and compensation schema gate (2026-09-06)
...
**Change ID:** CHG-20260906-16

**Verification ID:** TEST-149

**Documentation ID:** DOC-050

### Authorization and target preflight

- Work began from exact clean commit `a3cdb225ac3e72a3990d841f3de5667f3392860d` on `feature/final-shared-schema-acceptance`. `main` and `origin/main` remained at `d17b30482398472a02eefd505950f8b42af6b223` and were not checked out or modified.
- Authorization covered only `20260906234500_ingredient_conversion_evidence` and `20260906235900_compensation_admin_workflow` on the existing shared Neon development schema. It did not cover a seed, price/factor import, compensation record, provider action, application server, external integration, deployment, or production/live operation.
- The fail-closed read-only target check matched hostname fingerprint `6f48da70b1ce`, database `neondb`, schema `public`, a Neon hostname class, and `sslmode=require`. All four billing capability switches resolved disabled. No URL, credential, provider key, or personal row value was retained in evidence.
- Two exhaustive preflight snapshots were identical. Each found exactly 21 successful, non-rolled-back, one-step migrations; only the two authorized names pending; all 21 stored checksums matching either exact Git-blob or Windows working bytes; exactly 64 existing domain tables; all 24 billing/finance/compensation tables empty; and every target conversion/compensation object absent. The baseline data/schema snapshot hash was `4cb54281570c28686c77e7873f87480e79e39f19915350ce3ccd8cbfae5d4faf` across 5,169 existing rows.
- Baseline catalog hashes were tables `e9427dea4b71e1de745375bbef4a0af448f532dae1aca8294d335e0b96fff1ad`, columns `563bf8b1a6f672879ed23879a39688b44518b8290cd64f033a2acdd6f9761252`, enums `b5093a0e69338dc8a74f79ee6134dc15c823611f7009484a93531a864c28537e`, indexes `3e42d8e894d9cfd09d979f96e344175e3a3cd9bdde95760a2eb653bf1a9d0f9e`, constraints `6944d7a3f0a0936db4335e9553120be67ab864931be4e86f5b9b6a97ed67ca32`, triggers `ae2f9c905e8bbe1806f3aa4a7a520936b06118539865f0032cf56f115cbd2a43`, and routines `25c546aaf160c901bcafcf3c8ab4e6e4369c055a27c1e00f91829ed78fa2d0be`.

### Disposable reconstruction and constraint gate

- A task-labelled PostgreSQL 16.4 container on loopback port 55459 reconstructed a 21-migration parent and a separate full 23-migration database. Both second deploys were empty and both statuses were current. Shared preflight versus the local 21-migration parent, local 23-migration database versus migration history, and local database versus Prisma datamodel each reported no difference.
- The local parent snapshot contained 64 tables at aggregate hash `3747c0740b58c2824570e1a0bcc0505865af3a31e1c75d7c006b9c19e9e171c9`. After both migrations, all 64 named parent rows/counts/hashes were identical; only `IngredientConversionEvidence` and `IngredientConversionSource` were added. The 66-table local snapshot hash was `cbc82666e2240933e1ba66d91fb4c396f5a3c7d69aece769ce23241f3a7bdf78`.
- A rollback-only conversion graph accepted a rights-bearing source and reviewed evidence, rejected metadata-only factor use, a non-positive factor, source mutation, and evidence mutation, then returned conversion, price-source, commodity, and food fixture counts to zero. The dedicated compensation database passed TEST-142 with four eligible awards, two statements, exact PHP 230,500 minor-unit reconciliation, manual off-platform evidence, immutable-record rejection, and zero provider calls.
- The exact additive catalog delta is two conversion tables with 47 columns, six enums/25 values, eight indexes including primary keys, five restrictive foreign keys, ten checks, and three triggers; plus one compensation enum/three values, nine columns, eight indexes, six foreign keys, seven checks, and four triggers. Four supporting routines were added in total. The migrations insert no row.

### Shared application and postflight

- Normal `prisma migrate deploy` applied only the two authorized names. Stored checksums exactly match committed working bytes: `cf6c15f3f6de31b9b4b7120f3fd716e846cccb0d003a5bffa7b13b5fc24af2a1` for conversion evidence and `56ab3fa1c8d631282383d5a27dc62dba9471796d58ad1c64f442b0ae8c332232` for compensation administration. No migration file was edited and `db push` was not used.
- Exhaustive postflight found exactly 23 clean migrations, no pending migration, and 66 domain tables. Every one of the 64 pre-existing tables retained its exact row count and content hash. Both conversion tables and every billing, finance, compensation, provider, price, and audit-event target remained empty or unchanged. The postflight snapshot hash was `26ecc46e98aa3173c9cade523cbc26a066f3e4c602980665a206d1344e300e22`.
- Postflight catalog hashes were tables `8cfd2db10ba631e8240d5e902ce9a0d18f36a69273b8f633afff8d0e7593f12c`, columns `b2ab72c74127dbf508138024fac8def3823677a9a5fd040942626c4df3471608`, enums `05d73031f55bce3f8df99c045be1b819a07d2ef59333ed05ec961dafd04cffbe`, indexes `e45a5c138350f1b8595cea8460455d7fe259b83a48fb33cb7d3a1c858d5f45c0`, constraints `8246b9ef5221fdc808b672bc19bd7e26a0aebb764e3885046a614cb5c9b07cd2`, triggers `14624c2a5061483a4e45ed5a3761f0cd74e7d7011ebbebab5e8c7cdddd7065f5`, and routines `2dbac6841fcecd59f08f950f32a5cb76993ef6a94c85d47fa1708fb5e7a1e72c`.
- A second shared deploy reported no pending migrations, migration status reported current, and shared development compared exactly with the local 23-migration reconstruction and Prisma datamodel. The data-integrity preflight found zero planned-meal duplicate groups and zero daily-aggregate duplicate groups.
- Neon connectivity remained intermittent: read-only parity/preflight and no-op/status checks encountered bounded `P1001` or empty schema-engine failures before succeeding on retry. The authorized deploy itself returned success without a connectivity error. No failed attempt was treated as success; complete preflight or postflight evidence followed every uncertain boundary.

### Verification, cleanup, and remaining gates

- The full backend suite reports **458 registered, 457 pass, 0 fail, and 1 unchanged clinical-policy TODO**. Backend production build, 58-file alias rewrite, phase-2 script compilation, Prisma format/validate/generate, frontend no-emit type-check, lint with zero warnings, and the 42-route production build pass. Backend and frontend offline production dependency audits each report zero vulnerabilities.
...
- The first runtime probe correctly failed rather than being counted as a pass: the frontend standalone output was nested at `/app/app/server.js` because tracing starts at the repository parent, while the runtime command started from `/app`. The Dockerfile now copies public/static assets into that standalone application root and runs from `/app/app`.
- Fresh `nutrimind-web:latest` and `nutrimind-api:latest` images then ran as the unprivileged `node` user in uniquely named disposable containers bound only to loopback ports 13000 and 15000. The web root returned HTTP 200 with the NutriMind page, `/health` returned HTTP 200 with the stable API health contract, and Docker reported both containers healthy. The API probe used synthetic development-only JWT strings and a deliberately unused local database URL; it made no database or provider request. Both task-owned containers were removed afterward.
...
- The final source audit found no handwritten TypeScript or JavaScript module above the enforced 900-line ceiling. The largest remaining modules are bounded feature views, domain services, or cohesive declarative catalogues rather than the prior mixed-responsibility monoliths.
- The complete root `npm run check` gate passed after modularization: source architecture, Prettier, backend and frontend lint with zero warnings, **471 registered backend tests / 470 pass / 0 fail / 1 unchanged clinical-policy TODO**, **6 frontend tests / 6 pass**, backend production compilation with 67 alias rewrites, and the Next.js production build with all 42 routes.
...
## 74. Frontend evidence integrity and connected UI polish (2026-09-10)

**Change ID:** CHG-20260910-01

**Status:** Statically verified and component-tested; public browser-tested. Authenticated visual acceptance remains outstanding.

- Preserved the existing cockpit dashboard, brand tokens, role layouts, session-resource cache, API contracts, and initial Radix locality-control work. No backend business logic, schema, payment, authentication, dataset, provider assignment, or deployment configuration changed.
- Removed browser-invented clinical advice and false reviewer attribution from the dashboard guidance card. Its informational replacement explains per-meal evidence and links to Health Profile and Progress; sentinel allergy values can no longer become safety claims. Removed inferred whole-plan verification from the retained hero component.
- Grocery preview and full page share one current-meal approval check and response types. Pending counts remain authoritative; no approved current rows suppress stale projections. Preview errors remain visible and do not write fabricated safe cache state. Cached initial rendering is retained.
- Category illustrations prioritize structured primary-protein evidence and conservative meat/egg/vegetable ordering. Approved local photographs now match only the seven exact reviewed recipe identities, with creator/source/license metadata projected from the approved manifest. API image metadata takes precedence. Unassigned Admin rows disable local-photo substitution. Image URL changes recover from prior delivery errors, disclosure pills are separated, and loading motion respects reduced-motion preferences.
- Locality stops require valid Region and Province/HUC hierarchy entries rather than nonempty free text. Locked prerequisites are explained and attached to the slider, keyboard End clamps to the highest available level, and existing NATIONAL/REGIONAL/LOCAL enum values are unchanged.
- Mixed approved/pending dashboard days retain pending previews. Native sibling buttons separate detail navigation from logging. Incomplete outside-meal nutrition is disclosed; water shows the logged quantity rather than an unsupported daily target.
- Shared focus/reduced-motion behavior, mobile navigation scrolling/current states, Admin image search/empty states, nutritionist source/error wording, and Meals section selection semantics received bounded fixes. The landing sample explicitly identifies itself as an illustrative demo rather than live user data.
- Verification: `git diff --check`, source architecture (all modules <=900 lines), Prettier, frontend lint (0 errors / 0 warnings), full frontend suite (**19 files / 68 tests passed / 0 failed**), frontend production build (45/45 static generation steps), and backend no-emit type check pass. Local Prisma client generation passed without database access. The existing public Chromium suite passed **2/2** against the production build. Intermediate formatting and unused-state warnings were corrected before delivery.
...
## 76. Current three-actor journey audit (2026-09-10)

- Current code and disposable PostgreSQL integration evidence supersede historical completion claims. The [system journey audit](SYSTEM_JOURNEY_AUDIT_2026-09-10.md) records the implemented flow, sixteen prioritized defects/gaps, product recommendations, exact verification boundaries and repair order.
- Reproduced profile-to-plan diet inconsistency, differing weight/target paths, stale report re-acknowledgement, missing library calorie compatibility, same-day swapping, bypassable swap warnings, and stale groceries after a successful swap. These product issues remain open; no clinical or business-policy fixes were silently applied.
- New guarded `backend/scripts/current-journey-audit.ts` exercises registration/OTP/onboarding gates, nutritionist application/activation with locally captured email, real login/role isolation, seventeen role API reads, grocery PDF output, and seven current defect/gap observations. [Sanitized output](verification/system-audit-2026-09-10/journey-observations.json) contains no credentials or real user data. The probe is observational and must change as defects are repaired.
- All 29 migrations applied to disposable loopback PostgreSQL. Production integration, admin-data, outside-meal, compensation, Premium entitlement, and controlled payment-projection suites passed. A separate combined-library fixture failed its library-only expectation: all 21 slots for the first 1,900-kcal profile fell back to Gemini, whose key was intentionally absent. Live generated-plan/report output and authenticated browser completion remain unverified.
- Initial backend suite: 514 pass / 1 fail / 1 clinical TODO. The failure was an outdated Premium navigation source-location assertion. Updated it to follow shared navigation and the All tools entry point; final backend suite: 515 pass / 0 fail / 1 clinical TODO. Frontend: 73 pass. Public Chromium: 2 pass. Both production builds, lint, formatting, architecture, script types, Prisma validation and all three dependency audits pass.
- Testing used task-owned synthetic records and an ephemeral Express listener, leaving the existing backend/shared data untouched. The frontend dev server was restored on port 3000 after build verification. UI work remains paused pending functional repairs.

## 77. Journey repairs and purchase-aware meal planning (2026-09-10)

**Change ID:** CHG-20260910-03

- Implements the engineering repair directions in section 76 and the user's subsequent decision to retain informed same-day swaps. The [repair delivery record](AUDIT_REPAIR_IMPLEMENTATION_2026-09-10.md) maps AUD-01 through AUD-15 to the resulting behavior and explicitly preserves the external AUD-16 clinical-policy decision.
- Profile revision transactions invalidate affected current/future unconsumed meals and report acknowledgement; weight routes share calculations. Reports retain immutable version content and profile snapshots, expose history, and reject stale acknowledgement/PDF export. Daily retry and grocery stale flags preserve failed work for recovery.
- Swap confirmation is bound to current server evidence and purchases, enforces required warnings, is idempotent, and commits quota, meal, ingredients and groceries atomically. Purchased totals survive rebuilding; remaining quantities and additional-only notices implement the requested shopping behavior. Legacy lists cannot credit purchases to an unrelated cycle.
- User library ranking follows serving compatibility and selected-day targets; attribution identifies the actual safety reviewer. Premium supports one upcoming shopping cycle separately from the current plan. Nutritionist coverage reveals missing serving sizes. Admin composition changes retain source-backed draft/publication history and invalidate dependent certifications. Grocery costing exposes partial evidence rather than claiming budget compliance.
- Final verification: root `npm run check` (architecture, formatting, both linters, deterministic tests and both production builds); `npm --prefix backend run check:phase2-scripts`; Prisma schema validation; and `git diff --check`. Backend: **519 pass / 0 fail / 1 external clinical TODO**. Frontend: **73 pass across 20 files**. All **33 migrations** applied to fresh disposable PostgreSQL databases.
- Disposable journey acceptance now asserts repaired behavior, including warning rejection, injected grocery-failure rollback, replay, partial purchases, cross-cycle invalidation and source correction. Production integration, admin-data, outside-meal, Premium, compensation and controlled payment-projection acceptance also pass. [Sanitized acceptance observations](verification/audit-repairs-2026-09-10/journey-observations.json) retain the coverage boundaries.
- Chromium: **5/5 pass**, including a purchased-quantity swap and report history, plus nutritionist/admin desktop and 390 px mobile workspace checks. This is bounded authenticated browser coverage, not exhaustive testing of every screen or live provider journey. A cold-load login native-submit race found during verification was corrected by enabling fields only after hydration.
- Four migrations are supplied but were applied only to task-owned disposable data. Shared databases and the existing backend on port 5000 remain untouched. The review preview on port 3000 uses the disposable API on loopback 5012 and synthetic test accounts. Clinical sign-off, real reviewed serving content, live Gemini/SMTP/OAuth/payment delivery, production scheduling/deployment and complete price/conversion coverage remain external completion items. No provider credentials were used and no real messages or payments were sent.
...
- **Architectural & Design Implementation**:
  - Replaced the linear list with an interactive Intake Activity Matrix (`MealActivityCalendar.tsx`) inspired by the GitHub contribution calendar and styled under NutriMind's signature brand design system.
  - Aligned with NutriMind's theme palette: Level 0 crisp sage-gray neutral, Level 1 soft mint emerald (`#a7f3d0`), Level 2 signature forest pine (`#08705b`), and Level 3 electric lime glow (`#b8f45f`).
...
## 83. Profile report lifecycle repair (2026-09-17)
...
Preserved the latest Desktop UI changes, added the now-removed administrator test-access experiment, and clarified missing price-data diagnostics. Shared development had no price observations or commodity mappings; numerical cost coverage remained unavailable pending source-backed data work. The dedicated experiment audit was removed from the active branch on September 22 and remains available in Git history. Frontend at that point: 194 passing tests. Backend: 528 passing tests and one existing TODO.
...
## 90. Meal-generation architecture checkpoint 1 (2026-09-20)

- The authorized shared development database dry run identified exactly **751** `COMPLETE` Panlasang Pinoy rows whose append-only review history contained `CAPSTONE_SOURCE_BATCH`, `source = PANLASANG_PINOY`, and `fixtureReviewer = true`. All 751 had the fixture safety reviewer and a certified revision; none had explicit safety declarations.
- The guarded invalidation changed those 751 rows to `STALE`, removed the fixture verifier and safety reviewer, cleared certification pointers and compatibility arrays, reset condition/allergen/cross-contact states to unknown, and appended 751 `FALSE_PANLASANG_BATCH_CERTIFICATION` invalidation reviews. A repeat dry run matched zero rows. The command is retained as `meals:invalidate-false-panlasang:dry-run` / `:apply` and is idempotent because only `COMPLETE` false certifications match.
- `MealLibrary.safetyEvidenceStatus` now means **base recipe evidence completeness only**: approved operational state, current supported evidence revision, no invalidation, and a nonempty fully FNRI-linked ingredient snapshot. Reviewer eligibility, condition declarations, allergen declarations, and cross-contact findings no longer change that base result.
- Runtime library eligibility now applies the domains independently. Base evidence is always required. Allergen review, exact present/absent coverage, and cross-contact assessment are required only when the user has an allergy. Explicit condition clearance is required for every user condition only when conditions are present. Missing, malformed, custom/unmapped, contradictory, or partially covered relevant evidence remains excluded. A manually reviewed clearance can cover every current canonical condition; future hypertension ruleset provenance remains deferred to the approved condition-rule checkpoint.
- Deterministic policy regression coverage now includes base-only/no-restriction eligibility, condition-only coverage, allergen-only coverage, combined intersection behavior, explicit high-risk manual clearance reuse, exact conflicts, aliases, and fail-closed unknown evidence.

## 91. Meal-generation architecture checkpoint 2 (2026-09-20)

- Added one shared deterministic ingredient classifier for definite canonical allergen findings and conservative dietary compatibility. Unknown ingredients remain `NEEDS_REVIEW`; they never create vegetarian/vegan or reviewed-absent allergen claims. The same classifier now supports review warnings, safety rechecks, reusable-draft publication, and the later generated-meal validator.
- `MealLibrary` now persists per-recipe-serving sodium, sugar, fiber, potassium, phosphorus, and saturated-fat evidence as nullable fields, together with evidence source and serving description. Null means unavailable and is not converted to zero. The source file contains **1,994** recipes; the source-evidence backfill matched and updated all **1,949** Panlasang rows currently imported into the shared development database, with zero unmatched imported rows: 860 sodium, 837 sugar, 821 fiber, 858 potassium, and 847 saturated-fat values. The remaining 45 source recipes are absent from the curated database corpus. The source supplies no phosphorus values, so phosphorus remains null. Raw `SOURCE_RECIPE` ingredient provenance remains non-FNRI and cannot complete base evidence.
- Added governed `ConditionNutrientRule` and `ConditionIngredientRule` tables with threshold basis, operator, severity, rationale, source title/URL, policy version, RND approver, status, and active state. Database constraints prevent active drafts and prevent draft approval identities. The seed contains **7 nutrient rules and 9 pregnancy ingredient rules**, all `DRAFT`, inactive, and unapproved. Primary sources are NHLBI DASH, ADA Standards of Care 2026, NIDDK CKD guidance, AHA saturated-fat/sodium guidance, FDA pregnancy food safety, and ACOG pregnancy guidance. No universal potassium, phosphorus, sugar, or carbohydrate threshold was invented where guidance requires individualization.
- Condition evaluation is generic and fail closed across per-serving, per-1,000-kcal, daily-total, percent-energy, and body-weight daily bases. Only active, approved, RND-signed rules can evaluate. Hypertension may later receive reusable `APPROVED_RULESET` provenance; diabetes, kidney disease, heart conditions, and pregnancy still require manual RND condition clearance. Kidney disease and pregnancy reusable certification require the same evidence proposal from two different eligible RNDs.
- User-specific approval and reusable evidence are now separate decisions. Approving or replacing a user meal no longer creates a library row. The approved-review archive exposes an explicit reusable-draft action; it applies classification and creates or links a draft without certifying it. Certification still requires fully FNRI-linked stable ingredients, explicit present/reviewed-absent facts for all five supported allergens, cross-contact assessment, and current eligible RND authority.
- Added a stable SHA-256 recipe signature over normalized name, meal type, macros, and sorted ingredient identities/amounts. Explicit publication checks the unique signature before insert and handles concurrent unique collisions by linking the existing entry. User goals no longer participate in reusable dietary tags or eligibility.
- Removed the legacy common-catalogue shortcuts that derived diabetes/hypertension clearances from hardcoded per-meal cutoffs and stored user goals as reusable meal tags. The managed 51-meal fixture catalogue now retains base and allergen evidence while carrying zero condition declarations until governed ruleset or explicit RND evidence exists; outside-meal condition warnings also fail closed without those old numeric shortcuts.
- Generation, swap selection, compatible-library browsing, and profile safety rechecks now use bounded database retrieval. Queries prefilter operational/base status, slot, calorie range, dietary tag, required condition declarations, required allergen absence, conflicts, exclusions, and search, cap each read at 120 rows, then retain the existing deterministic in-process policy as a final guard.
- Applied migrations `20260920120000_checkpoint2_classification_rules`, `20260920121000_source_recipe_provenance`, `20260920122000_tighten_condition_rule_governance`, and `20260920123000_remove_legacy_catalogue_shortcuts` to the owner-authorized shared development database. The database reports 40 migrations and is current. Verification: backend **550 pass / 0 fail / 1 existing clinical TODO**, frontend **194 pass**, both builds and linters pass, Prisma validates, migration status is current, and script type checking passes.
- Checkpoint 3 remains unimplemented. Its accepted step 10 order is certified library lookup, then broader raw-corpus candidate sourcing, then from-scratch generation. Raw-corpus provenance will be trace-only and will grant no safety authority, queue priority, or review shortcut.

## 92. Meal-generation architecture checkpoint 3 (2026-09-21)

- Completed the accepted three-stage candidate flow: each slot first queries the certified library through bounded database filters, then searches the broader raw recipe corpus, and only then requests a new Gemini recipe. A raw-corpus match is treated exactly like a newly generated candidate: it receives FNRI reconciliation, deterministic validation, `PENDING_REVIEW` state, and the full applicable RND review path. Only a currently eligible certified-library result may enter a plan as `APPROVED`.
- Indexed the full Panlasang source corpus by exact normalized content signature. Of 1,994 source rows, 34 exact duplicates collapse and 1,960 distinct raw recipes remain available. This recovers the 11 genuine variants that the earlier title-level curation had removed while continuing to exclude the 34 actual duplicates. Corpus provenance is trace-only (`RAW_RECIPE_CORPUS` versus fresh generation) and supplies no safety clearance, review priority, or trust shortcut.
- Added one deterministic generated-candidate validator for diet compatibility, allergens, ingredient resolution, macro/calorie integrity, and duplicate signatures. Definite violations are rejected before the review queue and retry is bounded. Unknown or qualitative ingredient amounts remain unresolved review facts rather than being converted to zero or presented as FNRI certainty.
- Added explicit AI usage telemetry by operation and purpose, including certified selection counts, raw-corpus lookup/generation activity, validation outcomes, and a 30-day Gemini planning invocation rate. The administrator analytics endpoint and page expose these dimensions without treating raw recipe selection as certification.
- Formalized `BASE`, `STANDARD`, and `ENHANCED` assurance tiers. Hypertension may use a versioned, approved ruleset or one manual RND review; diabetes requires one manual RND review; kidney disease, heart condition, and pregnancy require two independent reviews and cannot be auto-cleared. Kidney and heart clearances remain user scoped until the profile contains sufficient structured clinical detail.
- Added `canLeadReview` to the existing nutritionist profile as an administrator-assigned capability. A Lead is required for the second `ENHANCED` decision, ruleset approval, dispute resolution, and evidence suspension/invalidation. The second reviewer must be a different eligible RND and submits blind; agreement publishes, disagreement creates `DISPUTED` evidence and blocks publication.
- Made reusable condition clearance a first-class meal-plus-condition record carrying recipe signature, evidence revision, condition, optional user scope, policy version, issuance tier, manual/ruleset provenance, immutable reviewer decisions, and lifecycle state. Every selected plan slot can reference the exact clearance that justified it. Suspension, evidence change, reviewer-eligibility lapse, policy withdrawal, deterministic conflict, safety report, or overdue audit fails closed and marks dependent plan slots for revalidation.
- Added Pending, Second Review, Audit, and Disputed views to the existing nutritionist review page. Audit ordering is server calculated: disputes/reports, ruleset impact, enhanced-tier evidence, distinct-user exposure, age, then a deterministic daily sampling key. The interface explains the priority reason and does not expose configurable policy controls.
- Added governed ruleset versions with two independent RND approvals, at least one Lead, immutable approval records, and a dry-run impact report before activation. Suspending a ruleset suspends derived clearances. The development database still contains only the 7 nutrient and 9 pregnancy ingredient draft rules across 5 draft policy versions; none is active or safety authoritative.
- Reclassified all 1,949 imported Panlasang library rows through the deterministic pipeline. All 1,949 now have complete classifier output and 2,292 explicit deterministic allergen declarations, while condition-clearance count remains zero. Raw source provenance and deterministic classifications do not certify these meals. Base evidence now also requires a real current eligible nutritionist review; this tightens and supersedes checkpoint 1's temporary statement that reviewer eligibility did not affect base completeness. Condition and allergen coverage remain independently enforced domains.
- Live generation acceptance created three plan slots: two raw-corpus candidates saved as `PENDING_REVIEW` and one certified-library candidate saved as `APPROVED`. Recorded provenance was two `RAW_RECIPE_CORPUS` and one `CERTIFIED_LIBRARY`; the raw candidates received no condition safety authority. Gemini returned several transient 429/503 responses during FNRI fallback estimates, but the configured model fallback recovered and the journey completed. Provider capacity and latency remain external operational risks.
- Live governance acceptance traversed `REVIEW_DUE` to `DISPUTED` to `ACTIVE` to `SUSPENDED`, verified blind independent review, dependency revalidation, two-person ruleset activation with a Lead, subsequent Lead suspension, and zero diabetes auto-clearances. Temporary acceptance fixtures and policies were removed afterward.
- Applied migrations `20260920150000_checkpoint3_clearance_governance` and `20260920151000_clearance_identity_constraint` to the authorized shared development database. Prisma reports 45 migrations and a current schema. Final verification passed: backend 556 tests with zero failures and one existing clinical-policy TODO; frontend 194 tests across 49 files; both linters, both production builds, architecture size checks, formatting checks, Prisma validation, and migration status passed. Removed the duplicate `frontend/public/icon.svg` route that conflicted with the identical App Router icon, rebuilt successfully, cleared the production/dev cache collision, and restarted `localhost:3000`; `/login`, `/icon.svg`, and the backend health endpoint return HTTP 200, and the login surface renders in a browser. Authenticated role UI click-through was not repeated; the user, RND, admin, generation, and governance journeys are covered by API/service acceptance and production compilation.
...
- Self-service deletion now supports either the current password or a freshly verified Google ID token whose subject or verified email matches the signed-in account. This fixes Google-created accounts, whose generated internal password is intentionally unknown to the user. The exact typed confirmation remains mandatory before either reauthentication option is enabled.
- Account export and deletion remain authenticated and restricted to the `USER` role, but no longer require completed onboarding, accepted terms, or an acknowledged nutrition report. The frontend route guard makes the same privacy page exception so an interrupted onboarding account can still export or delete its data.
- The database already cascaded most patient-owned records, including sessions, profile and restrictions, nutrition reports, meal plans and ingredients, logs, grocery data, hydration, check-ins, generation jobs, previews, and user-scoped clearances. Deletion now explicitly removes the restrictive clinical decision links attached to the user's plans and scoped clearances before deleting the user. Those links intentionally remain restrictive during ordinary clinical operations.
- Shared reference data and reusable `MealLibrary` recipes are preserved. Audit and finance records whose schema intentionally uses nullable ownership are preserved with the deleted user reference set to null. The deletion event records only the reauthentication method and becomes deidentified when the user row is removed.
- Added `test:acceptance:account-deletion`, which creates a disposable account with a session, profile, water and grocery records, an RND-reviewed approved plan, and a user-scoped condition clearance. The acceptance passed: all patient-owned records and restrictive review links were removed, the shared recipe remained, and the deletion audit remained with no actor user ID. Verification also passed backend build/lint/script checking, backend **556 pass / 0 fail / 1 existing TODO**, frontend **194 pass across 49 files**, frontend lint, and the isolated production build of all 55 routes.

## 95. ENNS food-group locality evidence (2026-09-21)

- Received and reviewed the DOST-FNRI 2018, 2019, and 2021 ENNS individual dietary public-use delivery: **234,293** respondent records, **17** historical region codes, **116** historical province/HUC domains, valid national and provincial sampling weights, and **20** nonoverlapping edible-portion food-group measures. The untouched 94 MB delivery and dictionary now live in a Git-ignored source directory with recorded SHA-256 fingerprints; no respondent identifiers are committed or persisted to PostgreSQL.
- Added a deterministic, reproducible aggregation command. It produces **2,620** weighted food-group rows across one national scope, all **18** current planning regions, and **112** directly comparable current province/HUC scopes. Each row carries weighted percent consuming, weighted mean grams per day, unweighted sample size, survey-year coverage, rank, and relative-to-national intake. The three historical domains that cannot be represented exactly are handled conservatively: Maguindanao, Pateros, and Isabela City contribute to current regional evidence but are not presented as exact current province/HUC evidence.
- Published active release `2018-2019-2021_FOOD_GROUP_LOCALITY_V1` from source `FNRI_ENNS_IFCS` to the authorized development database through additive migration `20260921153000_add_enns_food_group_locality_evidence`. The release fingerprint is `4d59d27ddd9512e0466e609d9868c49e10ef7595e2ee5caf5766845e0df8400f`; its publication audit explicitly records zero respondent rows persisted.
- Locality remains a post-safety familiarity preference. Certified-library candidates first pass all base, dietary, allergen, condition, and calorie gates, then receive food-group ranking. Raw-corpus sourcing now scans a bounded candidate set, ranks it by the same food-group evidence, and tells Gemini that locality is preference-only and never safety evidence. From-scratch generation receives the same provenance-backed ENNS group context. No locality record creates certification, ingredient resolution, nutrient composition, or clinical authority.
- The existing three-stop interface now has real evidence behind National → Regional → Province/HUC fallback. General Santos acceptance resolves the exact local scope; fish and fish products measure **1.296723×** the national weighted mean and the locality ranker places a fish candidate ahead of a comparable beef candidate. This validates the user's original fish-focused example without inventing biome labels.
- Verification: backend **562 pass / 0 fail / 1 existing clinical TODO**, build and lint pass. `test:acceptance:enns-locality` verifies the active release, 2,620 aggregate rows, zero persisted food-item/respondent rows, exact General Santos resolution, and food-group ranking. The raw survey's separate regional-estimate methodology remains a scientific limitation; this feature uses the weighted aggregate as a familiarity signal and makes no prevalence or clinical claim to users.

## 96. Explicit Google registration and deletion-session completion (2026-09-22)

- Split Google account access by user intent. `/api/auth/google/login` only authenticates an existing account and returns `ACCOUNT_NOT_FOUND` without provisioning when the verified identity is absent. `/api/auth/google/register` is the only Google path that creates an account; it treats Google's verified email as sufficient verification and routes the new user to onboarding without a password or application OTP. The legacy `/api/auth/google` endpoint remains as a login-only compatibility alias.
- Google account linking now persists the verified Google subject for both existing and newly registered accounts even when Google supplies no profile picture. An existing account linked to another Google subject fails closed instead of replacing its identity link. Additive migration `20260922001000_add_google_account_identity_constraints` enforces one account per provider subject and one provider link per user; the development preflight found zero duplicates.
- Successful self-service deletion now uses the deletion response's refresh-cookie expiry and completes local session teardown directly. It no longer calls authenticated logout after the user row has already been removed. Refresh interception is suppressed during deliberate logout/deletion, preventing the prior 401 → refresh → hard-navigation race.
- Post-authentication routing now uses one shared destination policy. Login and registration surfaces show a bounded redirect state; after eight seconds they expose retry and sign-out actions instead of displaying an indefinite spinner. Successful deletion returns to login with a visible confirmation message.
- Added database-backed `test:acceptance:google-auth-intent` plus frontend coverage for destination ordering, redirect recovery, and refresh suppression. External Google Cloud origin configuration remains a provider-console responsibility and must authorize the exact frontend origin used for browser testing.

## 97. Provider-aware password controls (2026-09-22)

- Added an explicit `User.passwordLoginEnabled` capability instead of trying to infer whether a stored bcrypt hash represents a user-selected password or the unknowable random value assigned to a Google registration. New Google registrations set the capability to false; email/password registrations retain the true default. The migration conservatively identifies historical Google-created accounts by their Google link and absence of email-verification-send evidence while preserving password access for accounts that completed the email OTP flow before linking Google.
- Live profile responses now expose bounded authentication capabilities (`password` and `google`). Google names remain the normal editable `User.name`; no extra authentication-time name confirmation was added.
- The Security page keeps the password form visible for Google-only accounts but disables every field and action, with a direct explanation that the account uses Google sign-in and has no KAINARA password. Account deletion presents only authentication methods actually attached to the account.
- Email/password login, password changes, reset-token issuance, reset-token redemption, and password-based deletion all enforce the password capability server-side. The public recovery response remains neutral; Google-only addresses receive no reset token or email and are directed to continue with Google without exposing account type to an unauthenticated requester.
- Applied migration `20260922002000_add_password_login_capability` to the authorized development database. Prisma reports 48 migrations and a current schema. Verification passed: backend 562 tests with zero failures and one existing clinical-policy TODO, frontend 200 tests across 52 files, both builds and linters, Google intent acceptance, account-deletion acceptance, and provider-aware Security UI tests.

## 98. Subscription system removal and universal user access (2026-09-22)

- The owner removed the subscription product from the active development branch. Deleted the user billing, checkout, payment projection, webhook worker, entitlement, test-access grant, and provider reconciliation routes, services, policies, scripts, tests, UI pages, hooks, environment settings, and dedicated runbooks. The admin can no longer grant a test access pass.
- All users now receive compatible meal swaps, grocery cost evidence when data exists, outside-meal Gemini estimation within the existing fair-use compute quotas, reports, and the Salakot avatar accent. The weekly swap limit and its UI counters were removed. `SwapLog.requestKey` remains the idempotent replacement audit; the obsolete `PlanSwapTracker` table and response fields were removed. Safety eligibility, calorie warnings, AI quotas, and RND review rules remain enforced.
- Removed paid advance-cycle behavior end to end: no next-plan route, current/next selector, next-grocery view, scheduled future-plan preparation, or compatibility cron wrappers remain. Current-cycle generation and idempotent rollover remain available when the cycle is active.
...
## 102. Profile revision, report acknowledgment, and cycle adaptation (2026-09-22)

**Change ID:** CHG-20260922-02

- Added explicit cycle adaptation state and exact requested/acknowledged profile and safety revisions without mutating immutable cycle snapshots. Ordinary changes leave the active and shopping-started cycles stable, gate unfrozen upcoming cycles until the exact report version is acknowledged, and then mark them for rebuild. Shopping-day acknowledgment supersedes the unfrozen dated identity.
- Safety changes fail closed across every current/future cycle, including shopping-started cycles. Report acknowledgment is necessary but insufficient: a cycle remains blocked until every expected slot has explicit current safety evidence.
- Profile saves are semantic and idempotent. One real multi-field save creates one revision and one audit entry; an identical repeat creates neither. Shopping day now uses the same revision/report transaction instead of bypassing it. Onboarding baseline report version 1 now receives its matching immutable `NutritionReportVersion` history row.
- Replaced the carb-level UI/API contract with explicit `NO_RICE`, `FLEXIBLE`, and `WITH_RICE` preferences. Existing records defaulted to `FLEXIBLE` with recorded `DEFAULTED` provenance; explicit choices store `USER_SELECTED`. Removed obsolete carb proxy columns after a preflight found 12 profile labels and 7 snapshot labels; calorie/macro targets and historical plan facts remain separately stored.
- Applied migrations `20260922190000_add_profile_cycle_adaptation`, `20260922200000_remove_legacy_carb_preference`, and `20260922210000_rename_rice_default_provenance` to the authorized development database. Prisma reports 58 migrations and a current schema.
- Verification passed backend build/lint/script checking, **461 pass / 0 fail / 1 existing TODO** across 462 backend tests, frontend build/lint and **201/201** tests, and disposable database acceptance for ordinary, identical, shopping-day, frozen-cycle, rice, acknowledgment, and safety-revalidation transitions.
- Full evidence and remaining batch boundaries are in [Batch 2 implementation evidence](BATCH_2_PROFILE_CYCLE_ADAPTATION_IMPLEMENTATION.md). Recipe rice roles remain Batch 3; consuming `REBUILD_REQUIRED` through preparation and reranking remains Batch 4.

## 103. Catalog metadata, favorites, and source-neutral candidates (2026-09-22)

**Change ID:** CHG-20260922-03

- Added normalized multi-slot applicability for certified and raw recipes while retaining singular `mealType` as the historical primary display slot. Request-time lookup, swapping, coverage, safety replacement, and raw candidate sourcing now use persisted applicability; title inference runs only during indexing/classification. Nutritionists can replace proposals with reviewed slot selections.
- Added explicit recipe rice roles with independent review state and optional included-rice grams. Deterministic roles remain proposals. Unknown included-rice portions stay unevaluable. Reviewed paired rice is composed only from an explicit cooked gram amount and governed FNRI per-100-g evidence.
- Added immutable plan serving components, base and composed signatures, composed-clearance scope, and grocery inclusion for an explicit cooked-rice component. Existing plans were backfilled with a base component without guessing rice. Automatic rice addition remains Batch 4.
- Added user-scoped, idempotent favorites with account/recipe cascade. Favorites carry no safety authority and survive temporary ineligibility. The user library exposes favorite and rice-role filters, authoritative server totals, opaque cursor pagination, and explicit page loading.
- Added a source-neutral candidate-provider contract and Panlasang adapter. Generation retains certified library → real corpus → Gemini order. Provider provenance exposes no safety authority.
- Reindexed all 1,994 source rows: 34 exact content duplicates collapsed and 1,960 variants remain. The development database contains 3,073 raw and 2,769 library applicability facts. Reclassification covered 1,949 imported Panlasang library records and created no condition clearances.
- Applied migrations `20260922223000_add_catalog_metadata_and_favorites` and `20260922224000_scope_composed_clearances`. Prisma reports 60 migrations and a current development schema.
- Database acceptance passed provider and eligible-library cursor pagination, authoritative total 50 for the unrestricted fixture query, favorite uniqueness/ownership/cascade, and favorite retention with fail-closed eligibility. Full implementation evidence is in [Batch 3 catalog evidence](BATCH_3_CATALOG_METADATA_IMPLEMENTATION.md).
- Final verification passed backend build/lint/script checking, **466 pass / 0 fail / 1 existing TODO** across 467 backend tests, frontend typecheck/lint and **201/201** tests across 53 files, the full 51-route production build, and the live Batch 3 database acceptance after all service integrations.

## 104. Upcoming preparation, ranking, and deadline-aware review (2026-09-23)

**Change ID:** CHG-20260923-01

- Added one idempotent upcoming-cycle preparation operation triggered by the daily scheduler, meal-cycle and grocery access recovery, and nutrition-report acknowledgment. It opens three Manila calendar days before shopping for `BASE`/`STANDARD` profiles and five days before shopping for `ENHANCED` profiles. Acknowledged profile/safety rebuild states regenerate the unfrozen upcoming cycle against the current revision.
- Preserved the sourcing order certified library → bounded raw provider → FNRI and deterministic validation → bounded from-scratch Gemini. Explainable ranking reasons are persisted after hard gates; cleared meals occupy early slots before unresolved later candidates. Reviewed rice-pairing meals may receive an explicit governed FNRI rice component only when the composed serving fits its slot.
- Added exact review-work identity and deadline metadata. Pending/second-review queues sort by shopping deadline and cook date, expose tier/stage/provenance/fallback information, and coalesce identical clinical work without dropping each dependent slot's decision history. Enhanced work retains independent Lead second review and different-reviewer enforcement.
- Rejection now supersedes only the affected slot and tries certified, then raw-corpus, then at most three deterministic-checked Gemini candidates. Deadline reconciliation uses a wider certified tolerance and otherwise cancels unresolved pending candidates, audits the gap, and records `INCOMPLETE_AT_DEADLINE` without publishing provisional meals.
- Tightened `READY_TO_SHOP`: every slot must be approved and revision bound, library and condition-clearance dependencies must still be current, manual candidates must carry the required distinct approval decisions, and a non-stale grocery projection must commit. Grocery aggregation failure leaves the cycle under review.
- Applied migration `20260922233000_add_upcoming_preparation_metadata` to the authorized shared development database. Prisma reports 61 migrations and a current schema. Verification passed backend build/lint/script checking, **471 pass / 0 fail / 1 existing TODO** across 472 backend tests, the Batch 4 development-database acceptance, frontend build/lint and **201/201** tests, and backend HTTP health. Full evidence and browser limits are in [Batch 4 implementation evidence](BATCH_4_UPCOMING_PREPARATION_IMPLEMENTATION.md).

## 105. Progressive current and next grocery workspace (2026-09-23)

**Change ID:** CHG-20260923-02

- Added one cycle-authoritative grocery workspace with universal current-week and next-week views. Upcoming quantities contain only evidence-cleared slots and expose cleared, expected, and unresolved counts. The interface labels partial data as a preview and explains that quantities may increase.
- Centralized grocery actionability on the server. Preparing/under-review, stale, revalidation, and unacknowledged incomplete projections cannot be checked or exported. Ready/current/shopping-started projections are actionable; accepted incomplete subsets export with an explicit incomplete marker and gap count.
- Grocery aggregation now consumes the exact same evidence-complete meal IDs as lifecycle readiness, so an `APPROVED` status alone cannot leak invalidated library evidence or missing condition clearance into shopping quantities.
...
## 107. Outside-meal capture, provenance, and revisions (2026-09-23)

**Change ID:** CHG-20260923-04

- Outside-meal search now prioritizes eligible certified recipes, labels other catalog/raw/FNRI matches by uncertainty or known conflict, and keeps custom entry. A selected library recipe snapshots its serving macros; changes record user-adjusted provenance. Measured FNRI, manual, AI-estimated, and unresolved items remain distinct.
- The capture flow collects consumed time, optional private note and image, and separate AI preparation context. Preview confirmation is request-key bound and idempotent. Current-profile safety findings are persisted and shown after the meal is recorded, never used to block a retrospective fact.
- All unconfirmed effective calories count in the estimated tracker segment; unresolved values remain excluded from explicitly partial totals. User corrections append revisions and recompute effective totals. Voiding appends revisions, removes the log from active totals, and retains history until account deletion.
- The additive migration `20260923160000_outside_meal_capture_revisions` was applied to the authorized development database. Backend build/lint/script checks and unit suite passed (**476 pass / 0 fail / 1 TODO**); frontend build/lint and **201/201** tests passed. The disposable development-database Batch 7 acceptance passed, including commit replay, corrections, voiding, image storage, and post-save allergen conflict. Details and limits are in [Batch 7 outside capture evidence](BATCH_7_OUTSIDE_MEAL_CAPTURE_IMPLEMENTATION.md).

## 108. Outside-meal estimate review and clarification (2026-09-23)

**Change ID:** CHG-20260923-05

- Added selective outside-log review entry for user requests, conflicts, macro-energy inconsistency, and high calorie uncertainty. Non-production environments explicitly queue all Gemini estimates as a capstone demonstration policy; ordinary manual logs remain outside the queue unless requested or flagged.
- Eligible RNDs can claim a specific item revision for 30 minutes, confirm an estimate, correct its macros, ask a bounded clarification question, or mark it unverifiable. User replies and edits append revisions and reopen review. Decisions, rationale, reviewer identity, before/after values, notifications, and tracker recomputation are persisted atomically. Confirmation does not certify a shared meal.
- User history exposes the actual review state and conversation. The RND screen shows context and a separately authorized private image. Queue/claim JSON omits image bytes, and ownership/claim checks guard requests.
- Applied additive migration `20260923190000_outside_meal_review_thread` to the authorized development database. Backend build/lint/script check and unit suite passed (**480 pass / 0 fail / 1 existing TODO** across 481 tests); frontend build/lint and **201/201** tests passed. Disposable database acceptance covers claim exclusivity, clarification, revision binding, correction, unverifiable totals, private-image access, and deletion cascade. Full evidence and limits are in [Batch 8 outside review evidence](BATCH_8_OUTSIDE_MEAL_REVIEW_IMPLEMENTATION.md).

## 109. Consented observed food references and raw candidates (2026-09-23)

**Change ID:** CHG-20260923-06

- Added explicit, revision-scoped user consent before any confirmed outside log can enter a shared observed-food pipeline. Image rights are recorded separately; private photos and notes are not copied. Eligible RNDs classify consented observations as food references or reproducible raw recipe candidates.
- Food references supply labeled, non-certified autocomplete estimates. Reproducible candidates share the existing raw-corpus selection and subsequent FNRI, deterministic-validation, pending-review, and normal RND safety gates. `USER_OBSERVED` provenance is traceable and has no clearance effect. Exact content signatures deduplicate observations while preserving serving and preparation variants.
- Edits, voids, renewed RND decisions, and reopened reviews withdraw outdated submissions; candidates with no active consent are retired from future lookup and blocked at RND plan approval. Account deletion nulls private source links and retains only deidentified admitted facts. Panlasang indexing is source scoped so it cannot retire observed entries.
- Migration `20260923230000_observed_meal_candidates` passed a rollback dry run and was applied to the authorized development database (65 migrations applied). A direct database-to-schema diff found only older unrelated objects/index/default differences, with no Batch 9 object drift. Backend build/lint/script checks and unit suite passed (**483 pass / 0 fail / 1 existing TODO** across 484 tests); frontend build/lint and **201/201** tests passed. The disposable database acceptance passed. Details and limits are in [Batch 9 observed corpus evidence](BATCH_9_OBSERVED_CORPUS_IMPLEMENTATION.md).

## 110. Integrated journey verification and historical Prisma parity (2026-09-23)

**Change ID:** CHG-20260923-07

- Reconciled the Prisma datamodel with the already-applied historical `ClinicalProfileReview` table, index names, and cycle defaults. No new migration or schema write was needed; `prisma validate`, migration status (65 applied), and the database-to-datamodel diff now pass with no difference detected.
- Added disposable email-auth and cross-role acceptance flows. The latter connects admin Lead assignment, patient outside logging, RND clarification/correction, observed-recipe consent and admission, raw-corpus lookup without certification, and privacy deletion. Existing Batch 1–9 and governance database acceptances also passed.
- Authenticated desktop/mobile browser smoke tests passed for user, RND, and admin workspaces. They exposed a login/registration redirect race while the profile was loading; the pages now wait for the authoritative profile before redirecting. The root architecture, format, lint, unit, build, and dependency audit gates pass.
- **Batch 10 remains in progress.** Full browser action journeys, observed-candidate plan-to-library reuse, live Google/Gemini provider checks, and timezone job execution remain unverified. The empty-database rehearsal was completed in §111. Exact evidence and limits are in [Batch 10 integrated journeys](BATCH_10_INTEGRATED_JOURNEYS.md).

## 111. Clean bootstrap migration reconciliation (2026-09-24)

**Change ID:** CHG-20260924-01

- A fresh disposable PostgreSQL 16.4 database applied all 65 existing migrations, then exposed two foreign-key update-action differences and one legacy geography index in a Prisma datamodel diff. New migration `20260923233000_reconcile_fresh_bootstrap_schema` conditionally aligns those objects. A clean 66-migration replay, second no-op deploy, migration status, and final diff all passed. No test container was retained.
- The authorized development target already had both desired update actions and no legacy index. Before deployment, a full PostgreSQL 17 custom-format backup was captured and its archive catalog verified (2,464,775 bytes; SHA-256 `B76A71054EB084FF6FD14CDCD0A9407FF9D97541D023BF5675ADAF5847D9244E`). A read-only preflight and postflight each found 0 composition revisions, 10 nutrition-report versions, and 12 user profiles. `prisma migrate deploy` recorded the one pending migration; all **66 migrations** are current and database-to-datamodel diff reports **No difference detected**.
- This closes Batch 10's clean migration lifecycle check. Connected browser actions, observed-candidate plan-to-library reuse, live-provider behavior, and timezone job execution remain open in [Batch 10 integrated journeys](BATCH_10_INTEGRATED_JOURNEYS.md).

## 112. Patient-to-nutritionist browser action journey (2026-09-24)

**Change ID:** CHG-20260924-02

- Extended the disposable three-role Playwright fixture with a connected browser journey. A patient records a manually measured outside meal, confirms the preview, requests nutrition estimate review, answers an RND clarification, and sees corrected confirmed macros. The RND claims the item, asks for information, then applies the correction. The admin browser also removes and restores the fixture RND's Lead capability. All four authenticated browser tests passed against the local frontend/backend at desktop/mobile sizes where applicable; the three fixture accounts and their private records were removed afterward.
- The connected outside-log path now has browser-level evidence in addition to the existing service acceptance. Remaining Batch 10 action journeys and provider/clinical limits are tracked in [Batch 10 integrated journeys](BATCH_10_INTEGRATED_JOURNEYS.md).

## 113. Observed candidate to certified library acceptance (2026-09-24)

**Change ID:** CHG-20260924-03

- Expanded the integrated role acceptance to carry a consented user-observed recipe through a pending plan linked to locally seeded test food items, patient-specific RND approval, an explicitly unreviewed reusable library draft, meal-type/rice-role review, RND evidence certification, and an eligible-library query. The raw candidate and draft remain ineligible until certification. The deterministic precheck accepted the omnivore recipe and rejected its vegan conflict. A freshly migrated disposable PostgreSQL 16.4 database passed this service journey; real FNRI reconciliation, provider selection/generation, full composition-service execution, and enhanced-condition second review were outside this fixture.
- Approval writes append-only nutritionist work credits. The acceptance script now requires a disposable loopback database, preventing further shared-development fixture credits. Three earlier fixture RND accounts on shared development retain their immutable credits and are suspended, unverified, and stripped of Lead capability. Their patient, observed-candidate, and library fixture records were removed; counts verified zero remaining. This limitation and the still-open Batch 10 journeys are documented in [Batch 10 integrated journeys](BATCH_10_INTEGRATED_JOURNEYS.md).

## 114. Fresh-account onboarding browser journey (2026-09-24)

**Change ID:** CHG-20260924-04

- Fixed the shared planning-location form so national planning does not require an empty Region field and Province/HUC is required only for that detail level. Component tests cover national, regional, and Province/HUC form validity.
- Added a fresh verified email-account browser fixture and exercised metrics, national food preferences, no-condition and no-food-restriction declarations with confirmation, shopping day, three consents, onboarding completion, dashboard entry, and baseline report history. A separate patient fixture with a synthetic persisted unacknowledged report exercised the login-to-report redirect, acknowledgment action, and subsequent dashboard access without a live Gemini call. The complete six-test Batch 10 role-browser file passed against the local frontend/backend. All five browser fixture accounts were removed. Report regeneration after a real profile change remains an open journey.

## 115. Profile edit and upcoming grocery consistency (2026-09-24)

**Change ID:** CHG-20260924-05

- Extended the connected profile-cycle acceptance with a prepared upcoming meal and its ingredient list. A weight edit correctly advanced the profile target and revision, staled the report, preserved the active-cycle snapshot, and paused the upcoming cycle, but the prior grocery ingredients remained visible until acknowledgment. `ProfileCycleAdaptationService.recordOrdinaryChange` now marks affected unshopped future-cycle grocery lists stale in the same transaction as the profile adaptation state. Grocery reads hide these lists and disallow shopping/export until rebuild.
- The acceptance verifies old-version report acknowledgment fails both before and after synthetic new-report publication, exact-version acknowledgment moves the upcoming cycle to `REBUILD_REQUIRED`, its old grocery list remains hidden, shopping-started cycles stay frozen, and a later safety edit immediately invalidates current uneaten slots. It passed against all 66 migrations on a fresh disposable PostgreSQL 16.4 database, then the container was removed. The fixture suppresses the separate asynchronous AI preparation worker; live report regeneration and rebuilt plan/grocery content remain open Batch 10 checks.

## 116. Prepared slot promotion across the Manila cycle boundary (2026-09-24)

**Change ID:** CHG-20260924-06
...
## 118. Observed recipe admission and fresh profile hydration (2026-09-24)

**Change ID:** CHG-20260924-08

- Extended the patient/RND browser journey through explicit deidentified reuse consent and RND admission of a reproducible observed recipe. The RND supplied a canonical name, quantified ingredients, preparation, and meal type; the patient saw `admitted recipe` on the original log. A disposable database read found the resulting `USER_OBSERVED` raw candidate available and no corresponding certified library row. The journey exposed that the ingredient classifier treated ordinary plural `carrots` as unknown; the narrow plural fix has a regression assertion.
- A combined run exposed an already verified patient being sent to email verification after a profile reload. The API's conditional profile response could return `304` without a body, leaving AuthContext's temporary unverified token placeholder visible to routing. Express ETags are disabled, `/api/user/profile` sends `private, no-store`, and a failed profile read presents a retry/sign-out state instead of claiming the email is unverified. A direct request with the former profile ETag returned `200` with the full body and no ETag. The route guard has a regression test for the failed-profile state.
- The complete seven-test role-browser file passed against a disposable PostgreSQL 17 restore after these changes. Fixture cleanup now deletes patient accounts before the fixture RND because outside-meal revisions restrict reviewer deletion. All five accounts and the disposable container were removed. Root `npm run check` passed: 483 backend tests passed / 1 existing TODO, 203 frontend tests passed, lint, format, architecture, and both builds; script typecheck also passed. Batch 10 remains open for the limits in [Batch 10 integrated journeys](BATCH_10_INTEGRATED_JOURNEYS.md).

## 119. Scheduled preparation coverage and Manila daily boundary (2026-09-24)

**Change ID:** CHG-20260924-09

- `UpcomingPlanPreparationService.runScheduled` now scans eligible users in bounded, ID-ordered pages, so an external daily call can reach accounts beyond the former first-100 cap. The production daily check-in now uses one clock across its aggregate date, safety retry, and upcoming preparation steps; its default remains the current time.
- A new disposable-loopback-only acceptance created 205 eligible accounts, confirmed each was visited once, then exercised daily aggregation at 23:59 and 00:00 Asia/Manila and an idempotent rerun. It passed after all 66 migrations on a fresh PostgreSQL 17 database. Root `npm run check` passed: 483 backend tests passed / 1 existing TODO, 203 frontend tests passed, lint, format, architecture, and both builds; acceptance-script typecheck passed. The fixture does not invoke real generation or an external scheduler; the repository still needs a deployed caller for the endpoint. `README.md` now states the schedule and dependency. Batch 10's remaining limits are in [Batch 10 integrated journeys](BATCH_10_INTEGRATED_JOURNEYS.md).
- The same daily job had an independent first-50-user safety-revalidation cap. Pending revalidation now scans unique affected users in bounded, ID-ordered pages. A second disposable-database run added 55 pending users and confirmed each was visited once; the recheck worker itself was stubbed. This closes the scheduler's two first-page starvation paths but does not prove live regeneration or external scheduler delivery.

## 120. Full certified-catalogue plan and catalogue preflight (2026-09-24)

**Change ID:** CHG-20260924-10

- A disposable PostgreSQL 17 full-plan run applied all 66 migrations, seeded the checked-in FNRI foods, and used fictional fixture RND accounts. The managed catalogue seed initially failed after five meals because the deterministic classifier interpreted FNRI's exact `Soybean cheese, soft curd` name as dairy. The classifier now recognizes only the three exact FNRI soybean-curd names as plant based; names with added milk and ordinary cheese still detect dairy. Its version advances to V2 and a focused regression test covers both sides. The catalogue seed now validates detected allergens against declared allergens before writes.
- The same rehearsal found all 51 managed meals had zero persisted applicability rows, making certified lookup return zero despite complete base safety evidence. The seed now writes and verifies a reviewed primary meal-type row under its existing fictional RND fixture. Its database preflight reports managed rows, missing reviewed meal-type facts, soybean-curd classifier refreshes, and new rows. A disposable no-condition 1,200-kcal profile then resolved 9 breakfasts, 7 lunches, and 9 dinners and generated 21 distinct approved library-backed slots with a grocery list and no Gemini event. At a 2,000-kcal target, this catalogue alone has no breakfast within the 510–690 kcal slot range; raw-corpus sourcing and pending review remain necessary. The fixture's RND identity establishes software flow only, not real clinical review.
- The authorized shared development preflight showed 51 managed meals, 51 missing reviewed slot facts, 15 soybean-curd classifier refreshes, and 0 new meals. A PostgreSQL 17 custom-format backup was saved at `C:\Users\chima\.codex\backups\Nutrimind\development-before-catalog-slot-repair-20260924.dump` (2,472,705 bytes; SHA-256 `C957E26A6094206BE96D03D4A3AAF864DFAB7AAEBB0CBFC4768C83CFFC34A63E`; 79 table-data entries). The seed created 0 rows, recertified 49 definitions, and skipped 2 current rows. A read-only rerun then reported 0 missing types, 0 classifier refreshes, 0 recertifications, and 0 new meals. The fixture RND's 63 verified meals had 0 condition-clearance rows. The preflight now reports total pending recertifications as well, since the earlier 15-classifier figure alone did not predict the 49 definition updates.
- The named full-library acceptance passed again on the disposable database after the shared apply; that container was removed. Root `npm run check` passed on the final code: 484 backend tests passed / 1 existing TODO, 203 frontend tests passed, lint, format, architecture, and both builds. Script typechecking passed separately. Batch 10 remains open for the other connected journeys and live-provider limits in [Batch 10 integrated journeys](BATCH_10_INTEGRATED_JOURNEYS.md).

## 121. Connected RND rejection and unavailable-slot outcome (2026-09-24)

**Change ID:** CHG-20260924-11

- The RND rejection path now says that a slot is unavailable while a safe replacement is sought. If certified, raw-corpus, and generated sourcing produce no replacement, it writes a separate unavailable notification and audit event instead of leaving the earlier “being generated” claim as the last user-visible state.
- A disposable-loopback-only acceptance used all 66 migrations, FNRI seed, fictional RND accounts, and the managed 51-meal catalogue. RND claim/rejection of a pending breakfast selected a certified fallback, restored 21 cleared slots, preserved one approved selection for that slot, and refreshed groceries to exactly the cleared meal ingredients and quantities without a Gemini event. A high-target breakfast with no available source remained rejected and unactionable; the cycle became `INCOMPLETE_AT_DEADLINE`. The provider was intentionally disabled for this run, and no real clinician reviewed the fixtures. Browser rejection and live provider behavior remain open in [Batch 10 integrated journeys](BATCH_10_INTEGRATED_JOURNEYS.md).
- Root `npm run check` passed: 484 backend tests passed / 1 existing TODO, 203 frontend tests passed, lint, format, architecture, and both builds. Acceptance-script typechecking passed separately. The disposable PostgreSQL 17 container was stopped and removed; the shared development database was not modified by this change.

## 122. Deterministic raw-corpus sourcing and batched FNRI reconciliation (2026-09-24)

**Change ID:** CHG-20260924-12

- A shared-development plan job remained at 25% while its ingredient resolver made 114 Gemini estimation attempts in ten minutes. Raw recipe IDs are now selected by deterministic eligibility checks and the existing preparation score, with exact content signatures used to avoid repeats. When a first-page shortlist leaves slots empty, bounded subsequent pages are checked before Gemini fallback. The generated-meal allergen/diet precheck still rejects definite conflicts; raw origin still grants no safety authority.
- Plan ingredient preparation loads one FNRI food and alias snapshot, resolves each distinct ingredient name once, and records unknown raw ingredients as `SOURCE_RECIPE` with no food ID. Missing quantities and incomplete composition remain uncertain, and raw plan rows remain `PENDING_REVIEW`. Progress now reports corpus lookup at 35% and FNRI reconciliation at 65%, before each step runs.
- The full repository gate passed before the adaptive-pagination follow-up (486 backend tests, one existing TODO, and 203 frontend tests). Final backend build, script typecheck, and 487 backend tests passed with one existing TODO. A disposable PostgreSQL 17 restore of the verified development backup yielded 21/21 raw slots, 231 ingredient entries, 192 unresolved entries, and zero AI calls in the sourcing/preparation path (185 ms local). A complete synthetic 21-slot weekly generation took under one second locally, producing one certified and 20 pending raw slots with zero AI calls. The fixture was deleted; the container was removed after verification. Remote database latency, true empty-slot Gemini fallback, RND turnaround, and a live browser retry remain separate checks.

## 123. Shared UI integration safeguards (2026-09-25)

**Change ID:** CHG-20260925-01

- Preserved Antigravity's recent UI layout while removing an added generation fallback that failed backend compilation and could clear unmatched slots without actually filling them. The normal certified-library match and raw-corpus lookup remain first; Gemini still has three bounded attempts for genuinely unmatched slots, and failure now leaves the plan unsaved rather than inventing a certified-library provenance for a pending meal.
- Generation status now selects the most recently started job instead of preferring any older job still marked `GENERATING`. Client polling ignores a job started before the current request even if that older job is still marked generating. Starter-plan notification failure is logged after plan persistence and cannot turn an already saved plan into a reported generation failure.
- The outside-meal modal retains its new presentation. It clears prior suggestion macros when the selected food changes, distinguishes unreviewed catalog recipes from eligible certified recipes, preserves the verified-library source when its values are unmodified, and asks for consumed grams before using an FNRI per-100 g reference. The outdated Premium AI label and fixed starter-cycle date fallback were removed.
- Local verification: backend build and 487 backend tests passed with one existing TODO; frontend production build, lint, and 233 frontend tests passed; source architecture check passed. These checks do not establish a live Gemini fallback, connected browser meal logging, or real RND review in this change.

## 124. Legacy review queue and managed fixture catalogue cleanup (2026-09-25)

**Change ID:** CHG-20260925-02

- A bounded dry run on the authorized shared development database identified 90 pending review rows represented by 84 coalesced queue cards across seven legacy or test patient accounts. The current `chimairelp@gmail.com` account was explicitly excluded and retained its 30 pending review rows. The transaction removed the 90 legacy rows; there were no associated review decisions. Postflight counts found zero remaining pending or disputed rows for the legacy accounts.
- The managed common-meal fixture catalogue was identified only through its `NUTRIMIND_COMMON_LIBRARY_V1` through `V4` safety-review provenance. Exactly 51 rows matched. Preflight found no favorites, outside-meal references, or condition clearances. Ten historical plan rows were detached from their fixture-library references so the user history remained intact, then 255 synthetic safety declarations, 357 synthetic safety reviews, and all 51 fixture meals were deleted. Nutritionist verified-meal counters were recalculated.
- Postflight found zero remaining managed fixture meals, 1,963 other library records, 1,960 raw corpus records, and zero library rows with complete approved base safety evidence. An append-only `LEGACY_DEVELOPMENT_DATA_CLEANUP` audit event records the applied counts. Cloudinary assets were not modified by this database cleanup.

## 125. Panlasang ingredient identity reconciliation (2026-09-25)

**Change ID:** CHG-20260925-03

- Added versioned, deterministic reconciliation for every ingredient occurrence in the 1,960-row Panlasang raw corpus. The policy removes quantities and preparation-only wording, accepts exact FNRI names and a narrow reviewed equivalence map, and permits only identity-neutral lexical additions. It explicitly rejects ambiguous foods and processing changes; examples include refusing chicken broth → instant chicken noodle broth, fresh malunggay leaves → dried powder, and chopped cashew → cashew fruit.
- The authorized development dry run evaluated 20,258 occurrences. The applied `PANLASANG_FNRI_IDENTITY_V1` pass linked 6,102 occurrences to one of 1,537 FNRI records, retained 13,829 as explicitly unresolved, and marked 327 scraper fragments as invalid and unavailable to future candidate projection. Unresolved does not mean unsafe; it means the local FNRI table lacks a defensible one-to-one identity or the source phrase combines/underspecifies foods.
- Stored raw ingredients retain their source wording and now carry match status, exact FNRI ID/name, match method, mapping version, and planning exclusion state. Re-indexing the source corpus applies the same policy, preventing a later import from erasing this evidence. Candidate projection preserves these identities, and generated raw-corpus plans attach them while keeping `SOURCE_RECIPE` provenance; a corpus match still grants no review or safety authority.
- Backfilled 96 FNRI identities into existing Panlasang plan ingredients and removed 11 preparation-only fragments from those plan rows. The reported example `leftover rice` now links to FNRI `Rice, well-milled, boiled` in both the raw recipe and every matching pending plan. Published source macros were not silently replaced: portion conversion and professional review remain separate requirements.
- The nutritionist review UI now distinguishes FNRI-linked identity, unmatched source-recipe evidence, and AI estimates. Conditions and allergies no longer display warning symbols when none are declared. Postflight dry-run reported zero recipe changes, zero plan links, and zero invalid plan rows remaining. Verification passed backend build/lint/script typecheck and **494 pass / 0 fail / 1 existing TODO** across 495 backend tests; frontend build/lint and **236/236** tests across 57 files.

## 126. Admin analytics null-data and error recovery (2026-09-25)

**Change ID:** CHG-20260925-04

- The admin analytics page crashed while rendering two historical AI usage groups whose nullable `purpose` value predated the required operation/purpose convention. The backend analytics response now exposes missing purposes as `UNSPECIFIED`, and the frontend normalizes both live responses and older session-cached analytics before rendering. Missing newer counters and row collections also receive safe empty or zero defaults, while a malformed non-object response still fails visibly.
- The shared page error boundary now derives its recovery destination from the authenticated account. An admin returns to `/admin/overview`, a nutritionist to `/nutritionist/reviews`, and a user to `/dashboard`; an unauthenticated visitor returns to login. This prevents an admin analytics failure from sending an admin through the user-only dashboard and then to `/unauthorized`.
- A direct service read against the authorized development database confirmed there are no null purposes in the normalized response. Verification passed backend lint/build and **494 pass / 0 fail / 1 existing TODO** across 495 tests; frontend lint/build and **241/241** tests across 59 files; the architecture check also passed.

## 127. Authentication viewport and scroll containment (2026-09-25)

**Change ID:** CHG-20260925-05

- Removed the authentication shell's nested vertical scroller and delayed mobile auto-scroll. The outer `overflow-x-hidden` rule had implicitly computed vertical overflow as `auto`, while the inner shell independently allowed vertical scrolling; together they produced two visible scrollbars and could move the page without user input.
- Desktop authentication pages now use one viewport-bound shell. Registration uses tighter card padding, field spacing, input height, and action sizing, with reduced desktop panel padding and responsive hero scale. At 1280×768 and 1280×1024, registration, its account links, and all other authentication cards fit fully in the viewport with no scrollable container. Narrow mobile layouts retain one natural document scrollbar so form fields are not clipped.
- Browser checks covered registration, login, forgot-password, reset-password, and verify-email at the laptop viewport, plus registration at 390×844. Focused authentication tests passed **3/3**, frontend lint and production build passed, and changed files passed Prettier and Git whitespace checks.

## 128. Report-first onboarding gate restoration (2026-09-25)

**Change ID:** CHG-20260925-06

- Removed the onboarding-completion shortcut that created an empty placeholder nutrition report and marked it acknowledged. Completing onboarding now leaves any pre-completion report stale and unacknowledged, returns the report workspace as the next destination, and cannot silently acknowledge a report when the completion request is replayed. The Terms page follows that server result; after the user acknowledges the real current report, the explicit first-time continuation returns to the dashboard.
- The existing backend meal prerequisite remains the enforcement boundary: a current report for the exact profile revision must exist, be non-stale, and have an acknowledgment timestamp before meal routes run. Raw-corpus and generated candidates retain `PENDING_REVIEW`; report acknowledgment does not grant meal safety approval.
- A bounded dry run on the authorized development account `maeannamparado5@gmail.com` found one auto-acknowledged placeholder report/version, two completed generation jobs, two cycles, 27 meals all in `PENDING_REVIEW`, three plan notifications, and no review decisions, meal logs, or grocery lists. The corrective transaction removed only those premature records and wrote audit event `PREMATURE_ONBOARDING_PLAN_REMEDIATED`. It preserved the account, profile, and all three structured safety entries: dairy allergy, lactose intolerance, and dairy avoidance. Postflight found no report, plan, cycle, job, or plan notification remaining for that account.
- Verification passed backend lint/build and **495 pass / 0 fail / 1 existing TODO** across 496 tests, including the focused onboarding policy test (**11/11**). Frontend lint/build and **242/242** tests passed, including the focused report workspace tests (**6/6**). A direct post-repair prerequisite check returned `409 REPORT_ACKNOWLEDGEMENT_REQUIRED` for meal access. A source regression assertion prevents the placeholder-report and auto-acknowledgment pattern from returning.

## 129. Nutrition guidance evidence communication (2026-09-25)

**Change ID:** CHG-20260925-07
...
## 131. Versioned clinical evidence registry and condition policy map (2026-09-25)

**Requirement ID:** REQ-033

**Architecture decision:** ADR-028

**Documentation ID:** DOC-069

**Verification ID:** TEST-223

**Change ID:** CHG-20260925-09

- Added `ClinicalEvidenceSource` as the normalized policy-evidence registry. Each immutable revision records its issuing organization, document identity and type, clinical domain, canonical location, source version/publication/retrieval dates, exact supporting section, population, jurisdiction, caveats, lifecycle state, and optional archive/checksum evidence. Corrected or newer evidence receives a new source code instead of silently changing a reviewed revision.
- Added explicit rule application metadata to nutrient and ingredient rules: bound evidence revision, exact claim locator, applicable population, required inputs, exclusions/caveats, evaluation scope, result authority, and a closed calculation-method code where applicable. Existing title and URL fields remain for compatibility but no longer constitute sufficient activation evidence.
- Policy impact generation and approval now fail closed unless every child rule has complete governance metadata and a `CURRENT` source revision. Runtime rule lookup also requires a current evidence source. Automatic ruleset clearance queries admit only rules explicitly marked `CLEARANCE_ELIGIBLE`; review-assistance rules and hard blocks cannot silently create positive clearance.
- Seeded 14 source revisions covering DOST-FNRI/PDRI, Mifflin-St Jeor, NHLBI DASH, ADA standards and consensus, KDIGO/NIDDK CKD guidance, AHA saturated-fat/sodium guidance, CDC/FDA/ACOG pregnancy guidance, U.S. FDA allergen taxonomy, and Philippine FDA prepackaged-food labeling. The existing seven nutrient and nine pregnancy ingredient rules now bind exact source/application metadata while remaining `DRAFT`, inactive, and unapproved.
- Added read-only `GET /api/evidence/sources`, returning only allowlisted public fields for `CURRENT` evidence revisions. Source presence explicitly grants no rule approval or runtime authority.
- Expanded `/sources` with six condition-domain explanations for hypertension, diabetes, kidney disease, heart conditions, pregnancy, and allergies/intolerances. Each entry separates deterministic checks, formula or aggregation method, linked sources, and the manual-review boundary. Source cards expose version, exact support, population, and limitation where available. Panlasang Pinoy remains recipe provenance with no nutrition or safety authority.
...
## 133. Planning-readiness feedback after report acknowledgment (2026-09-25)

**Change ID:** CHG-20260925-11
...
## 135. Prefer Panlasang Pinoy article photos for recipe candidates (2026-09-25)

**Change ID:** CHG-20260925-13

- Clarified the Generate action: when today falls before the next shopping-anchored weekly cycle, it requests a `STARTER` plan covering only the remaining days. The upcoming seven-day cycle can already be preparing and displayed as non-actionable previews before Generate is pressed.
- Pending-plan preview responses now carry display-image metadata from their linked raw recipe or certified library entry. For Panlasang Pinoy corpus meals, the stored article photo takes precedence over a YouTube thumbnail or representative image; a failed article-photo request falls back to the linked video thumbnail when present. Existing approved library entries are resolved to their raw corpus photo in one bounded batch by their explicit Panlasang source URL. A separately assigned exact image remains authoritative; a representative assignment does not displace the article photo.
- Recipe-photo URLs are restricted to HTTPS Panlasang Pinoy upload paths, and the recipe page is retained as attribution. The frontend explicitly permits that image host and links back to the source from meal details. Article/photo provenance has no safety authority and does not change review, clearance, or plan actionability.
- Initial read-only shared-development check found 1,905 raw recipes with article-photo URLs and 55 without. A dry run fetched the 55 source pages plus 17 older library source URLs absent from the raw index: 21 raw and all 17 legacy entries yielded working article photos. The applied pass filled only those 21 null raw-photo fields and stored the 17 verified legacy URL mappings in a checked-in manifest. Postflight: 1,926 of 1,960 indexed raw recipes have a photo URL; 1,915 of 1,949 older library entries now resolve to an article photo. The remaining 34 source pages did not yield a working photo (several advertise stale image URLs returning HTTP 404) and retain the video/representative fallback. No meal certification, condition clearance, or review state changed.
- Verification: backend and frontend production builds, scripts TypeScript check, targeted lint checks, 12 backend image/preview tests, and 16 frontend image tests passed. Two screenshot recipe-photo URLs returned HTTP 200 with image content; the built Next.js image optimizer also returned HTTP 200 with `image/jpeg` for the pictured breakfast recipe. A read-only service lookup resolved a legacy manifest photo. Authenticated browser rendering remains unverified.

## 136. Nutritionist application journey audit and activation gate (2026-09-25)

**Change ID:** CHG-20260925-14

- Audited public application validation/submission and private reference-plus-email tracking, admin credential stages and call scheduling, post-call approval/rejection, invitation delivery/acceptance, and access to the nutritionist workspace. The isolated PostgreSQL journey used synthetic applicants and test-only captured emails; it did not contact real applicants or the shared development database.
- Found and reproduced an activation bypass: approving an application created a verified nutritionist profile before the invitation was accepted. The invited user could request a password-reset link, set a password, and enter nutritionist routes while the application remained `APPROVED`. Pending invitations now cannot receive or use reset links, cannot sign in through password or Google login, and cannot enter nutritionist routes even with an otherwise valid role token. Existing activated and legacy nutritionist accounts retain the normal password-reset path.
- Application stage changes, scheduling, final decisions, invitation resends, and activation now use conditional state updates. An outdated or simultaneous request fails instead of overwriting a final state; concurrent approvals create only one account, and concurrent invitation acceptances activate only once. A failed resend restores the prior invitation token and expiry if the application is still awaiting activation.
- The repeatable API acceptance checks submission, duplicate denial, status privacy, admin-only access, stage/call timing, approval and rejection, pre-activation denial, invitation replay denial, concurrent approval/activation, and post-activation role access. A browser journey checks public wizard validation, status tracking, invalid invitation handling, admin application loading, and the admin's review/call-stage actions against the isolated API.
- These checks verify application behavior, not PRC credential authenticity, whether a human call actually occurred, real SMTP delivery, or deployment configuration. Those remain operational review responsibilities.

## 137. Full applicant media submission and admin receipt (2026-09-25)

**Change ID:** CHG-20260925-15
...
- Verification: Prisma schema validation, backend production build, frontend TypeScript check, targeted lint, 516 passing backend tests with one existing TODO, and 249 passing frontend tests. Temporary reservation fixtures confirmed the configured two-per-minute overall gate and the one-per-minute background cap while leaving foreground capacity; all fixtures were deleted (zero reservation rows afterward). An empty-queue scan completed against shared development. Live generation with a real Gemini response, nutritionist approval of a newly queued batch, multi-instance concurrency, and browser interaction with the retry control were not exercised in this change.

## 139. Original cooking links for Panlasang sourced meals (2026-09-25)

**Change ID:** CHG-20260925-17

- Plan meals, pending previews, and certified-library cards now expose a direct cooking link from stored recipe provenance. A Panlasang Pinoy corpus candidate opens its exact article; an independently recorded YouTube video opens its exact video only when no Panlasang article applies. Meals without either link retain a clearly labeled tutorial search. Photo attribution remains separate from cooking provenance, and recipe origin grants no meal safety authority.
- Library entries published from raw-corpus plans recover their Panlasang article through the original plan-to-corpus relation when the library description has no source line. Swapping to a certified library meal now clears the replaced candidate's raw-source ID and records certified-library provenance; display handling also ignores old raw-source IDs on preexisting user-swapped rows.
- Read-only checks found 1,960 indexed Panlasang candidate records with 1,960 syntactically valid direct article URLs; the pictured Chicken Adobo Fried Rice and Tortang Corned Beef plan row resolved to its exact Panlasang article. Backend/frontend builds or typechecks, targeted lint, six backend tests, and seven MealCard tests passed. The external pages were not individually HTTP checked, and authenticated browser clicking was not exercised.

## 140. Owner-scoped approved meals in Library and fixed review claim expiry (2026-09-25)

**Change ID:** CHG-20260925-18

- A plan approval notification describes approval for that user's plan; it does not certify reusable `MealLibrary` safety evidence. Approved, currently actionable meals from the user's current or upcoming plan now appear in the Library tab as “Approved for you” and “In your plan.” They remain owner-scoped, are not offered as globally certified favorites or swap replacements, and disappear when the actionable workspace no longer includes them. Reusable catalog entries in the plan also receive an “In your plan” label.
- The user-facing certified catalog admits meals that pass the same base evidence, allergen, condition-clearance, and reviewer checks even when they fall outside a voluntary dietary preference. Such cards explicitly say “Outside your dietary preference.” Planner and swap selection keep the existing dietary and slot-calorie requirements. The catalog already did not impose a goal or calorie filter.
- A nutritionist's 30-minute meal-review claim now has a fixed expiry: reopening an active card cannot extend it. After expiry, another RND may claim the item immediately; the prior claimant has a five-minute cooldown and sees it at the bottom of their queue. The service enforces the cooldown in the atomic claim path. Queue cards show completed/required approvals such as `1/2`; the existing Lead and distinct-reviewer enforcement for independent second review remains active.
- No schema or shared-data mutation was needed. Backend and frontend production builds passed. Backend suite: 518 passed, zero failed, one existing TODO. Frontend suite: 251 passed. Focused tests cover the claim cooldown, owner-scoped plan meal visibility, and browse-only dietary relaxation while preserving condition/allergen exclusion. An authenticated browser journey with two simultaneous RND sessions remains unverified.

## 141. Automatic first current plan preparation (2026-09-25)

**Change ID:** CHG-20260925-19

- After current consent, onboarding, a current acknowledged nutrition report, and clinical planning readiness, the existing report-acknowledgment trigger and scheduled/page recovery path now prepare the first current window before the upcoming week. The window remains the shopping-day anchored `STARTER` bridge when appropriate, or a full `WEEKLY` cycle when today is the cycle start. It uses the existing idempotent generation job and candidate sourcing; every newly sourced meal still requires its usual review and is only a preview until cleared.
- Existing current cycles and in-progress jobs are reused. A failed first-window job is not restarted by scheduled scans or page polling; the user can explicitly retry it. Dashboard and Meals now poll while the first current cycle is being created, explain the automatic preparation, and show retry only for a failed job. Once a cycle exists, the existing missing-slot queue and review UI continue unchanged. Automatic preparation does not bypass report version, consent, suspension, account-role, or clinical-context gates.
- No schema change or shared-data backfill was needed. Backend build and frontend TypeScript check passed. Backend suite: 520 passed, zero failed, one existing TODO; frontend suite: 253 passed. Two focused tests cover report/consent/profile revision and account prerequisites. A live onboarding-to-review browser journey and actual Gemini quota usage were not exercised in this change.

## 142. Administrator-authored meal drafts (2026-09-25)

**Change ID:** CHG-20260925-20

- Administrators can enter a complete named recipe, meal type, serving description, preparation method, per-serving calories/macros and optional extended nutrients, FNRI-linked ingredients with gram amounts, and notes explaining the nutrient calculation/source. The admin workspace lists its submissions, permits edits only before RND evidence review, and links to the existing meal-image manager for a separately assigned photo. No Gemini call is made to author these recipes.
- Submission creates a `MealLibrary` row with `safetyEvidenceStatus=INCOMPLETE`, no condition/allergen clearance, no certifying nutritionist, and a versioned `ADMIN_AUTHORED_DRAFT` safety-review snapshot plus an admin audit event. The existing `APPROVED` library status is operational and does not make the meal eligible; matching still requires independently certified evidence, current reviewer eligibility, and restriction coverage. Admin-entered nutrient values have `nutritionEvidenceSource=UNKNOWN` until reviewed. `LEGACY_UNREVIEWED` is the closest current enum value for the unreviewed origin; the exact admin authorship is preserved in the safety-review and audit records. No schema migration or shared data change was needed.
- The nutritionist Meal library has an admin-drafts filter. Its detail and certification views show the cooking method, portion, all nutrient values, ingredient amounts, and the admin's calculation notes. The existing RND certification action supplies reviewed base/allergen facts and sets the certifying RND as verifier when the author was an admin; condition clearance remains a separate governed action. Optimistic revision checks prevent certification against an edited recipe, and duplicate recipe signatures are rejected.
- Verification: backend build, frontend TypeScript check and production build, targeted lint, backend suite **522 passed, 0 failed, 1 existing TODO**, frontend suite **253 passed**. A task-owned PostgreSQL 16.4 database on loopback port 55479 applied all 69 migrations and exercised FNRI-only create, unreviewed exclusion, nutritionist visibility, draft edit, revision conflict, duplicate prevention, and audit/provenance persistence. The isolated database was discarded. A live authenticated browser journey and real RND judgment were not exercised; nutritional values remain admin claims until RND certification.

## 143. Explicit nutritionist review claim and release (2026-09-25)

**Change ID:** CHG-20260925-21

- Opening a meal-plan review card now loads a preview without acquiring its 30-minute claim. A separate Claim review action uses the existing atomic compare-and-set lock, while Release claim conditionally clears only the current RND's active claim and records an audit event. Release leaves the meal pending and immediately available to peers; the five-minute cooldown remains limited to expired claims. Review decisions and clinical-file downloads still require an active claim.
- The nutritionist page labels preview versus claimed state, exposes the two actions, and hides editing and decision controls until the current RND owns the claim. The queue refreshes every 15 seconds while visible so another RND sees a released meal without manual refresh. Existing acceptance callers that intentionally acquire a claim now request it explicitly.
- Verification: backend build and frontend production build passed; backend suite **522 passed, 0 failed, 1 existing TODO**, frontend suite **254 passed**. A fresh task-owned PostgreSQL 16.4 database on loopback port 55479 applied all 69 migrations and exercised preview without mutation, exclusive claim, non-owner release rejection, owner release, peer reclaim without cooldown, and the release audit event. The database was discarded. An authenticated browser journey with two live RND sessions was not run.

## 144. Deterministic nutrition guidance and profile document (2026-09-26)

**Change ID:** CHG-20260926-01

- New nutrition guidance versions use calculated, source-labeled references instead of a Gemini-generated assessment. For ages 19 and above, the existing FNRI PDRI adult AMDR policy computes gram ranges from the user's saved daily energy target. Diabetes fiber uses the ADA per-1,000-kcal reference. The document marks sodium patterns as general references and marks diabetes carbohydrate, kidney nutrients, broad heart conditions, pregnancy preparation, custom conditions, and allergen exposure for individualized review rather than assigning unsupported personal limits. The underlying meal eligibility and RND gates are unchanged.
- The current report and its immutable version retain profile revision, source codes, URLs, calculations, and acknowledgment state. Existing AI-written versions remain in history. A legacy current version is considered stale at the report endpoint and is replaced with a new deterministic version when the user opens the guidance page; the acknowledgment endpoint rejects legacy policy versions. No schema change or shared data backfill was needed.
- Onboarding and post-login routing now lead to Profile → Nutrition Guidance. The former `/nutrition-report` URL redirects there with an allowlisted continuation. The current guidance page and PDF use a plain white document layout with source links/URLs and a clear separation between acknowledgment and meal clearance. The report endpoint no longer uses the Gemini-specific limiter or calls Gemini.
- Verification: backend production build and **526 passing backend tests with one existing TODO**; frontend production build, TypeScript check, and **256 passing frontend tests**. Focused policy tests cover age scope, calculated ranges, source URLs, and refusal to invent diabetes/CKD/allergen limits; UI tests cover acknowledgment and routing. A live authenticated onboarding-to-PDF browser journey and independent clinical approval of the source application were not performed.

## 145. Separate local development and production Next.js caches (2026-09-26)

**Change ID:** CHG-20260926-02

- A production `next build` run in the shared frontend folder while `next dev` was serving port 3000 left the development `.next` directory without `server/vendor-chunks/lucide-react.js`. The subsequent report-acknowledgment redirect to `/profile` surfaced a Next.js ENOENT overlay. The page source and acknowledgment API were not the cause.
- Stopped the identified frontend development process, moved the broken cache aside, restarted the frontend server, and confirmed `/profile` and `/profile/nutrition-report` return HTTP 200. Production builds now use `.next-production`; development retains `.next`, and the existing isolated repair mode retains `.next-repair`. Docker copies production standalone and static output from `.next-production`; TypeScript and Git ignore rules include the separate output. The old moved cache remains locally ignored because automatic command review rejected recursive deletion of it.
- Verification: frontend production build passed while the development server remained live. Afterward, both profile routes returned HTTP 200 and the development `lucide-react.js` chunk existed. Authenticated browser interaction and a Docker image build were not run.

## 146. Curated Panlasang ingredient aliases against FNRI (2026-09-26)

**Change ID:** CHG-20260926-03

- Audited all 20,258 ingredient occurrences in 1,960 indexed Panlasang Pinoy recipes against the local FNRI catalogue. Version 1 linked 6,102 occurrences, left 13,829 unresolved, and excluded 327 scraper fragments. Recipe labels that do not identify one FNRI composition row remain unresolved; examples include unspecified cooking oil, water, mixed seasonings, olive oil, and cooked-versus-raw brown rice.
- The source-ingredient matcher now accepts verified `FoodAlias` rows while rejecting unverified or conflicting aliases. Corpus indexing and reconciliation load the same verified alias snapshot, and the read-only coverage script reports every unresolved normalized name with examples when run with `--all`.
...
## 148. Reconcile independent Panlasang library ingredient snapshots (2026-09-26)

**Change ID:** CHG-20260926-05

- The earlier FNRI and USDA reconciliation updated `RawRecipeCandidate.ingredients` and a subset of scheduled meal ingredients. It did not update the separate `MealLibraryIngredient` rows created when an approved plan was published as reusable evidence. This left the nutritionist library modal showing every Chao Fan ingredient as composition identity unresolved even though the indexed source recipe had eight defensible links.
- Added a dry-run-first, auditable library snapshot reconciliation. It covers Panlasang-sourced meals whose base safety evidence is not complete, resolves only unique FNRI names/curated equivalents/verified aliases or unique exact USDA names/verified aliases, and skips recipe-signature collisions. Each meal update is transactional, checks the prior evidence revision and signature, recomputes the recipe signature from the linked ingredient identities, and increments the evidence revision. It does not change ingredient `dataSource`, nutrient totals, safety status, declarations, or certification. The `cold leftover rice` recipe label now maps to FNRI's boiled well-milled rice record; the source preparation and serving amount still need separate evidence.
- Three additional narrow Panlasang-to-USDA aliases were curated with an audit record: `garlic powder` → USDA FDC 171325, `ground white pepper` → USDA FDC 170933, and `salt` → USDA FDC 173468. The `salt` alias corrected an unverified legacy entry that pointed to an FNRI cracker. This linked 998 additional ingredient occurrences across 787 indexed source recipes; the repeat source dry run found zero more candidates. The aliases identify food records only and do not invent quantities or nutrient totals.
- In the pictured Chao Fan entry, six names now map to FNRI and five to USDA. Ground pork, green peas, and unspecified cooking oil remain unlinked because their composition depends on fat content, preparation, or oil type. The two `to taste` entries have no measurable amount even though their food names are now linked. The meal remains `STALE` from its prior invalidated batch certification and cannot be reused on the strength of these name links.
- Verification: the focused FNRI matcher tests, backend script type check, lint, production build, and full backend suite passed (**531 passed, 0 failed, 1 existing TODO**). The first shared-development database dry run proposed 7,083 links across 1,849 Panlasang library meals. After both identity passes, the final dry run has just 10 possible links in two colliding, incomplete Buffalo Chicken Wings rows with identical ingredient and nutrient content but different Panlasang source URLs; both were skipped rather than breaking recipe-signature uniqueness. Chao Fan was queried after apply: 6 FNRI links, 5 USDA links, 3 unresolved, `STALE` status, no certified revision. The same `cold leftover rice` FNRI link was applied to its indexed source recipe; the USDA source-recipe reconciliation repeat dry run had zero pending updates. An authenticated browser refresh and professional review of the unresolved identity and quantity choices were not run.

## 149. Plain-language reusable recipe review and measured USDA fallback (2026-09-26)

**Change ID:** CHG-20260926-06

- The nutritionist library shows one recipe status and a specific next step. Ingredient source details and proposed classifications remain available in expandable sections. A recipe awaiting work can be prepared in one screen: the RND chooses a specific FNRI or USDA composition record for every saved ingredient, enters edible grams per serving, and documents the serving and measurement basis. No cup, piece, or “to taste” conversion is inferred by the system. Exact record identity and source links are shown for inspection.
- The backend calculates calories, protein, carbohydrate, and fat from the chosen per-100-g records. Fiber, sodium, and potassium are shown only when every selected record supplies that nutrient; unavailable sugar, phosphorus, and saturated fat are cleared rather than carried forward from unverified source claims. Preparation increments the recipe revision, records an audit snapshot with composition revisions and prior values, and invalidates any earlier reusable certification and dependent condition clearance.
- New V2 reusable certification requires a preparation audit for the exact current recipe revision, unchanged composition records, matching stored calculations, reviewed allergen facts, and the existing nutritionist eligibility and cross-contact checks. A USDA fallback additionally requires explicit RND acceptance and a written rationale. Ingredient classification considers both the recipe label and selected food-record name. Per-user allergy, condition, and other eligibility checks still apply after recipe certification. Prior V1 FNRI certifications remain supported under their original policy; they are not retroactively represented as having passed the V2 calculation workflow.
- The downstream restriction adapter distinguishes the new certified-library USDA evidence from an ordinary USDA name match. Only a current, base-complete reusable certification can pass the former to the user's restriction evaluation; an unreviewed USDA match still requires review. A focused test exercises both sides of this handoff.
- This does not auto-select clinically equivalent food records or certify existing Panlasang recipes in bulk. An RND must still judge preparation equivalence, edible amounts, and source applicability. No shared database rows were changed by this code update. Verification: backend and frontend production builds, backend suite **536 passed, 0 failed, 1 existing TODO**, frontend suite **257 passed**, lint, and source checks passed. An authenticated end-to-end RND review and independent clinical validation were not performed.

## 150. Narrow Library reuse from a finalized meal approval (2026-09-27)

**Change ID:** CHG-20260927-01

- A final patient-specific nutritionist approval can publish an additional recipe approval keyed to an exact hash of recorded conditions, allergens, and structured safety entries. Goal, calorie target, and voluntary diet preference do not enter that hash. Only an unchanged recipe, current profile/cycle revisions, current plan safety policy, eligible reviewer, and non-flagged library record can satisfy it. Unrecognized restrictions, condition cases, high-risk reviews, clinical-document cases, observed private recipes, and incomplete ingredient classification for allergy cases are excluded. A nutritionist flag increments the evidence revision so dismissing the flag cannot silently reactivate the earlier approval.
- The user's Library may display these recipes as “Reviewed for a matching health profile” after rechecking the current profile and ingredient allergen facts. This is narrower than reusable recipe certification. Planner selection and swap replacement still require their existing certification, portion, and clinical gates; browsing a recipe does not promise plan eligibility. Existing certified recipes remain available under their original rules. The new approval row stores a scope hash and reviewer reference, with no user foreign key. Existing library review audit snapshots may still refer to source plan IDs; this change does not claim the entire historical library record is deidentified.
- Review-work coalescing now requires the same safety scope as well as recipe/portion identity, and rejects stale profile or safety revisions, preventing one user's final review from being copied to a different restriction context. The additive migration `20260927010000_profile_matched_meal_approvals` was applied to the configured shared-development database. A dry-run-first backfill identified 11 qualifying historical plan approvals and applied all 11 without error; deduplication produced six distinct library approval rows. A live read through `MealSwapService.getCompatibleLibraryMeals` for an approved source profile returned six profile-matched cards and included its source recipe.
- Verification: backend production build, script TypeScript check, lint, and full suite (**540 passed, 0 failed, 1 existing TODO**); frontend production build and **258 passing frontend tests** before the final backend-only eligibility tightening. Focused tests cover exact scope identity, different goals, recipe/evidence changes, expired reviewers, allergen conflicts, and condition exclusion. The shared database read is a service-level check, not an authenticated browser journey or independent clinical validation. This conservative boundary follows the need for individualized nutrition planning in the [ADA Standards of Care](https://diabetesjournals.org/care/article/49/Supplement_1/S89/163932) and ingredient/cross-contact vigilance in [FDA food-allergy guidance](https://www.fda.gov/food/nutrition-food-labeling-and-critical-foods/food-allergies).
