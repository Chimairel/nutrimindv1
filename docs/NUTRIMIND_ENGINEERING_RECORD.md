...
- Before any fixture write, the 49 managed usage-counter baselines are read so the eventual selected replacement has a known baseline. In `finally`, the fixture resolves that selected row before deletion when necessary and restores only its fixture-owned `+1` through an exact compare-and-set decrement. If the row has any concurrent or unexpected value, the fixture refuses to overwrite it and reports cleanup failure. Counter restoration and deletion of the exact reserved user (cascading its owned rows) are attempted independently, then verification asserts zero reserved user, plan, grocery, structured-entry, revision, and meal-log rows plus zero selected-counter drift. The intended assertions cover invalidation, certified replacement or safe block, grocery refresh, revision/replacement evidence, and idempotent resubmission/recheck.
