...
| --- | --- | --- |
| REQ | Functional or non-functional requirement | REQ-012 |
| ADR | Architecture/design decision | ADR-010 |
| RISK | Technical, project, security, clinical, privacy, or operational risk | RISK-020 |
| DEF | Defect, inconsistency, or documentation mismatch | DEF-028 |
| CHG | Implemented change set, formatted CHG-YYYYMMDD-## | CHG-20260820-06 |
| TEST | Test case or verification procedure | TEST-034 |
| UNC | Unresolved uncertainty | UNC-016 |
| DOC | Documentation correction or addition | DOC-020 |
...
| --- | --- | --- |
| Meal generation | profile/calorie target -> approved library matching -> Gemini fill -> FNRI resolution -> `MealPlan`/ingredients -> review state | Implemented but externally unverified; pending rows are preserved for review but excluded from actionable generation/current responses |
...
| REQ-002 | Only approved actionable meals may be shown, logged, totaled, or included in groceries | Clinical/data integrity | Codex guide and owner-approved Batch 3 policy | Must | Non-approved/expired statuses are filtered from current/grocery/totals and rejected before user mutations/swaps | TEST-013/014/022/024 | Implemented and unit/closest-boundary verified; authenticated live exclusion and non-mutating invalid-state rejection verified, while the live positive approved-current path and stored-grocery provenance remain unverified | ADR-007, DEF-008/020/021/022, RISK-001/015/016 |
...
| --- | --- | --- | --- | --- | --- |
| Credentials/Google auth/OTP/reset/refresh | All | auth routes/controller/service; frontend auth pages/context | Existing USER/NUTRITIONIST/ADMIN credential login runtime-verified; owner manually verified USER traditional and Google login/dashboard against the same profile | revocation, full auth-page browser matrix, OTP/reset/refresh, external services | Address auth lifecycle in a separately approved batch |
| User onboarding/profile/ToS | USER | user routes/controller; onboarding pages | Restriction vocabulary and pure NONE/normalization policy unit-verified; no caller behavior change | array validation, non-atomic enum/custom updates, AI normalization, caller integration, bounds | Later validated DTO/policy adapter only after separate approval |
...
| Meal generation/FNRI | USER | generation service, Gemini/FNRI libs, actionability policy, restriction policy, generation library adapter | Approved-only library query plus adapter filtering/fallback seam unit-verified; authoritative lifecycle snapshot migrated; live AI/FNRI generation unverified | legacy rows remain incomplete/ineligible; no first-class library evidence or adapter mapping implemented; broad ingredient profile includes 384 FNRI-labelled unlinked and 126 estimated/unlinked ingredients; prompt rules remain unreviewed | Keep fallback; separately design first-class evidence persistence before certification |
...
4. Full frontend browser/hydration interaction, pending-state/`409` defense-in-depth, backend prerequisite/session policies, five TODO specifications, and absent CI remain open.
