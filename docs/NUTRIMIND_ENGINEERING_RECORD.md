...
| ADR-018 | Keep retail-price evidence separate from FNRI nutrition; use append-only source/publication/geography/commodity/mapping/observation records and conservative partial-range policies before any public budget feature | Accepted; additive foundation implemented with migration unapplied | 2026-09-06 | Prevents timeless/invented prices and unsafe substitutions; requires a later local migration rehearsal, licensed bounded source basket, explicit geography, repository, API, and UI gates | REQ-022, RISK-023, UNC-017, TEST-090 through TEST-095 |
...
| `MealPlan`, `MealIngredient` | per-slot plan and ingredient records | meal ingredients have optional FNRI link, data source, quantity, and free-text unit; plan indexes include migration-defined objects absent from the datamodel | older/AI rows may lack exact quantities or FNRI links; actionability policy remains separate |
...
| `IngredientPriceSource`, `IngredientPricePublication`, `IngredientPriceGeography`, `IngredientPriceCommodity`, `IngredientPriceCommodityMapping`, `IngredientPriceObservation` | official source provenance, immutable snapshots, geographic scope, original/normalized commodity units, exact-or-uncertain FNRI mapping, and dated PHP centavo ranges | additive unapplied migration; restrictive FKs; append-only triggers; range/date/normalization/mapping checks; explicit supersession | empty foundation only; no accepted geography, data import, repository, endpoint, UI, or runtime estimate |

Schema changes are authorized only in their recorded phase. The ingredient-price migration added by ADR-018 is generated but unapplied; it requires a separate disposable rehearsal before any shared-database decision. Before future uniqueness migrations over existing data, inspect duplicate counts without exposing personal data, approve deterministic remediation, and define recovery.
