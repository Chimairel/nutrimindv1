...
| ADR-016 | Persist structured safety entries additively while synchronizing existing enum/custom fields as a compatibility projection; backend catalogues alone assign canonical codes and support states | Accepted; source implemented, migration pending approval | 2026-09-05 | Preserves current consumers and legacy profiles; unsupported and unmapped entries remain review-gated | REQ-018, REQ-003/010/011, RISK-002/019, TEST-073 |
...
| TEST-073 | Structured multi-value safety intake policy and source boundary | Backend pure-policy/schema tests; Prisma validation/client generation; backend/frontend no-emit/build; frontend lint; guarded route-shell browser inspection | Documented separators preserve multi-word terms; controlled aliases/deduplication/domain separation; invalid/vague/forged inputs deny; unsupported/custom entries remain review-gated; full legacy projection retains all domains; source compiles | Pass for unit/static/build scope: 262 registered, 261 pass, 0 fail, 1 clinical TODO; Prisma schema valid; both production builds pass; frontend lint has zero warnings; authenticated database/browser acceptance pending migration approval | CHG-20260905-01; REQ-018; ADR-016; planned INT-009/E2E-007 |
...
## 35. Structured multi-value safety intake — source implementation pending migration acceptance (2026-09-05)
...
**Verification ID:** TEST-073; planned INT-009 and E2E-007
...
- Prepared additive migration `backend/prisma/migrations/20260905120000_structured_safety_intake/migration.sql`. It creates three enums, the structured entry table, indexes/foreign key, and one additive revision enum value. The migration has **not** been applied to the configured shared database.
- Prisma format, schema validation, and client generation pass locally. Backend no-emit TypeScript and production build pass.
- TEST-073 expands the deterministic backend suite to **262 registered: 261 pass, 0 fail, 1 pre-existing clinical-policy TODO**. New cases cover separators, multi-word values, mixed sources, aliases and controlled misspellings, case-insensitive duplicates, domain separation, invalid/vague/unsupported/unknown states, oversized and separator-only values, `NONE` contradictions, forged client fields/codes, catalogue evidence metadata, full legacy intersection projection, and deterministic input preservation.
- Frontend no-emit TypeScript passes. Frontend lint passes with zero warnings. The production build passes and generates all 37 routes.
- A guarded `INT-009` acceptance script is prepared. After explicit migration-target approval, it will create one reserved-domain fixture, verify structured persistence/profile reload/history/idempotence/legacy projection, and remove that exact fixture in `finally` cleanup. It refuses to run unless `ALLOW_STRUCTURED_SAFETY_ACCEPTANCE=true`.
- Browser route-shell inspection compiled `/onboarding/conditions`, returned HTTP 200, and recorded no console error. The unauthenticated browser remained at the route guard; authenticated chip interaction, mobile inspection, save/reload, and active-plan/grocery recheck are pending the database gate and are not claimed as E2E evidence.
...
- The configured shared PostgreSQL/Neon database and external services were not changed or called by this change. No catalogue population, Gemini request, email, OAuth, payment, merge, push, or deployment occurred.
- Owner approval is required before applying the migration to the configured shared database. After approval: deploy the migration, restart the backend, run guarded INT-009, execute authenticated desktop/mobile E2E-007 across onboarding and Health profile, verify a real safety change refreshes the active plan and derived grocery list without stale claims, then update this record with exact results.
