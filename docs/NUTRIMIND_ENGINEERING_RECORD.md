...
| CHG | Implemented change set, formatted CHG-YYYYMMDD-## | CHG-20260906-04 |
| TEST | Test case or verification procedure | TEST-098 |
| UNC | Unresolved uncertainty | UNC-018 |
| DOC | Documentation correction or addition | DOC-038 |
...
| DEF-031 | Prisma declares `MealPlan(highRiskReviewRequired, reviewApprovalCount, status)` and `MealPlan(requiresSafetyRevalidation, status)` indexes that are absent from the exact 17-migration baseline and shared-development schema | Medium | Shared price-migration preflight datamodel diff; exact 17-migration local PostgreSQL reconstruction; live-to-reconstruction diff | Decide whether both indexes remain required, then add one separately reviewed additive migration or remove stale datamodel declarations | Open; did not block CHG-20260906-03 because live shared development matched the exact migration-derived baseline before and after the two price migrations |
...
| `MealPlan`, `MealIngredient` | per-slot plan and ingredient records | meal ingredients have optional FNRI link, data source, quantity, and free-text unit; plan index inventory has a documented datamodel/migration mismatch in DEF-031 | older/AI rows may lack exact quantities or FNRI links; actionability policy remains separate |
...
- Direct comparison to the Prisma datamodel reported two `MealPlan` indexes that the datamodel declares but no checked-in migration creates: `(highRiskReviewRequired, reviewApprovalCount, status)` and `(requiresSafetyRevalidation, status)`. DEF-031 records this pre-existing repository mismatch. It requires a separate product/schema decision and additive migration if the indexes remain desired.
- The bounded migration stop condition was cleared through the authorized migration-derived method. A task-owned PostgreSQL 16.4 database reconstructed all 17 canonical migrations, and read-only database-to-database comparison against shared development returned `No difference detected`. The price deployment therefore started from the exact schema produced by accepted migration history; DEF-031 was neither changed nor silently folded into this phase.
...
- DEF-031 should be resolved independently through a reviewed additive index migration or removal of stale datamodel declarations; it must not be bundled into source-data ingestion.
