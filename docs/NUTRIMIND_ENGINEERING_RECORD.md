...
| CHG | Implemented change set, formatted CHG-YYYYMMDD-## | CHG-20260905-05 |
| TEST | Test case or verification procedure | TEST-079 |
| UNC | Unresolved uncertainty | UNC-017 |
| DOC | Documentation correction or addition | DOC-028 |
...
| ADR-017 | Use PayMongo first as a sandbox collection adapter; keep collection, durable entitlement grants, and nutritionist compensation as separate auditable domains; defer provider disbursements and all live-money use behind explicit readiness gates | Accepted for architecture and phased planning; implementation not started | 2026-09-05 | Enables a bounded monthly Premium experiment with a 3-versus-6 swap entitlement while preserving common safety; requires additive finance/webhook/compensation models, reconciliation, provider approval, and legal/tax decisions | REQ-021, RISK-022, UNC-016, TEST-078 |
