...
| RISK-024 | PSA retail observations price purchased raw commodities, while managed-meal quantities often represent cooked edible grams; treating them as the same basis would understate cost and overstate coverage | Product/data integrity | High if observations are connected directly to meal quantities | High | High | Keep raw and cooked FNRI identities distinct; report identity and available-cell coverage separately; retain original/normalized source unit; add no runtime adapter; require reviewed edible-yield and cooking-conversion evidence before estimation | Open; bounded ingestion preserves the distinction and exposes no runtime estimate, but conversion evidence is not yet defined |
...
| UNC-017 | Which user geography, freshness windows, source precedence, high-coverage basket, package/edible-quantity mappings, DA reuse terms, and minimum coverage thresholds should govern a public budget estimate? | These choices determine whether an estimate is locally meaningful and honest enough to show or rank | Owner product decision; bounded PSA coverage measurement; RND safety review; DA terms confirmation; reviewed exact mappings and user-facing copy | Owner/engineer/RND/legal reviewer | Open; blocks data import, runtime ranking, endpoint/UI exposure, and any Premium claim, but not the empty additive foundation |
