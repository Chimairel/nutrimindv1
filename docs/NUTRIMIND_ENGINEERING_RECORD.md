...
- Read-only shared-development data check: 1,905 available raw recipes have a Panlasang article-photo URL. Of 1,949 older library entries carrying an explicit Panlasang source URL, 1,932 match a raw corpus URL and 1,877 of those have an article photo; the other entries retain the video/representative fallback. No image scrape, database mutation, or certification change was performed.
- Verification: backend and frontend production builds, TypeScript and targeted lint checks, 12 backend image/preview tests, and 16 frontend image tests passed. Two screenshot recipe-photo URLs returned HTTP 200 with image content; the built Next.js image optimizer also returned HTTP 200 with `image/jpeg` for the pictured breakfast recipe. Authenticated browser rendering remains unverified.
