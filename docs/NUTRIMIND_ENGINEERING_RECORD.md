...
| --- | --- | --- |
| REQ | Functional or non-functional requirement | REQ-032 |
| ADR | Architecture/design decision | ADR-026 |
| RISK | Technical, project, security, clinical, privacy, or operational risk | RISK-027 |
| DEF | Defect, inconsistency, or documentation mismatch | DEF-037 |
| CHG | Implemented change set, formatted CHG-YYYYMMDD-## | CHG-20260906-16 |
| TEST | Test case or verification procedure | TEST-149 |
| UNC | Unresolved uncertainty | UNC-019 |
| DOC | Documentation correction or addition | DOC-050 |
...
| ADR-024 | Represent ingredient conversion as append-only source and evidence edges between explicit purchased-as-sold, raw-edible, and cooked-edible nodes; use exact identity/preparation matching and rational bounded evaluation | Accepted; empty schema and local policy/coverage verification complete | 2026-09-06 | Prevents raw-price/cooked-quantity conflation and arbitrary yield transfer; keeps nearby authoritative tables as metadata until licensing and exact row identity are defensible | REQ-022/030, RISK-024, UNC-017, TEST-130 through TEST-136 |
| ADR-025 | Calculate compensation from immutable completed-action credits using one active versioned policy, period-locked base retainer and capped workload bands, independently approved signed adjustments, and manual off-platform payout evidence | Accepted; source and disposable-local acceptance complete | 2026-09-06 | Keeps subscription collection and reviewer compensation separate; avoids approval bounties; requires distinct workflow actors and retains correction/payment evidence without storing payout credentials or moving money | REQ-021/031, RISK-022/026, TEST-137 through TEST-142 |
...
Schema changes are authorized only in their recorded phase. The two ingredient-price migrations added by ADR-018 passed disposable rehearsal and shared-development acceptance; they do not authorize source-data ingestion. Before future uniqueness migrations over existing data, inspect duplicate counts without exposing personal data, approve deterministic remediation, and define recovery.
...
- The additive migration is not applied to shared development. Compensation policy amounts in acceptance are synthetic and do not establish a contract, salary, employee/contractor status, tax treatment, payout schedule, dispute procedure, retention requirement, or production approval.
- Shared migration/application, administrator staffing for every maker-checker stage, accounting/legal approval, operational dispute and reversal procedures, deployment monitoring, and any future provider disbursement require separate authorization and evidence. Existing PayMongo capabilities remain disabled and unrelated to this manual record flow.
