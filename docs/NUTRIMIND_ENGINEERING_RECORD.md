...
| RISK-025 | A created checkout session could be mistaken for payment, duplicated after an uncertain response, or leak credentials/provider detail through persistence and errors | Financial/security/privacy/data integrity | Critical without server authority and durable idempotency | Critical | Critical | TEST-only config; fixed origins; bounded HTTPS; stable provider key with 23-hour reclaim limit; state constraints; allow-listed response; sanitized failure code; append-only audit; zero entitlement until verified webhook/reconciliation | Reduced for checkout creation; webhook delivery, payment projection, reconciliation, live mode, and production remain blocked |
...
| UNC-018 | When will a secure public TEST webhook endpoint, signing secret, and provider registration be available, and does the sandbox account support Maya and the later subscription/refund lifecycle? | Checkout creation alone cannot establish paid state or Premium and does not prove broader account capabilities | Owner/provider-controlled HTTPS endpoint and signing secret; TEST delivery/replay acceptance; explicit Maya/subscription/refund capability checks | Owner/provider/engineer | Open; card checkout creation is proven, while webhook delivery, Maya, payment completion, subscriptions, refunds, and all live capabilities remain unverified |
