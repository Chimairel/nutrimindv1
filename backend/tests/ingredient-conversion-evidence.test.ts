...
const purchased: ConversionNode = {
  basis: 'PURCHASED_AS_SOLD', unit: 'KILOGRAM', foodIdentity: 'Carrot, 1 KG',
  preparationCode: 'FRESH_WHOLE_AS_SOLD', priceCommodityId: 'psa-carrot',
};
const raw: ConversionNode = {
  basis: 'RAW_EDIBLE', unit: 'GRAM', foodIdentity: 'Carrot',
  preparationCode: 'TRIMMED_PEELED', foodItemId: 'fnri-carrot-raw',
};
const cooked: ConversionNode = {
  basis: 'COOKED_EDIBLE', unit: 'GRAM', foodIdentity: 'Carrot, boiled',
  preparationCode: 'BOILED_DRAINED', foodItemId: 'fnri-carrot-boiled',
};

function evidence(overrides: Partial<ConversionEvidenceCandidate> = {}): ConversionEvidenceCandidate {
  return {
    id: 'ap-raw', sourceCode: 'SYNTHETIC_TEST', externalEvidenceKey: 'carrot-ap-raw', evidenceVersion: '1',
    kind: 'PURCHASED_TO_RAW_YIELD', direction: 'FORWARD_ONLY', from: purchased, to: raw,
    factorMin: { numerator: 4, denominator: 5 }, factorMax: { numerator: 4, denominator: 5 },
    reviewStatus: 'REVIEWED', effectiveFrom: new Date('2026-01-01Z'), ...overrides,
  };
}

const rawCooked = (): ConversionEvidenceCandidate => evidence({
  id: 'raw-cooked', externalEvidenceKey: 'carrot-raw-cooked', kind: 'RAW_TO_COOKED_YIELD',
  from: raw, to: cooked, factorMin: { numerator: 9, denominator: 10 }, factorMax: { numerator: 9, denominator: 10 },
});

test('[TEST-130] reviewed exact evidence composes rational AP-to-raw and raw-to-cooked factors', () => {
  const result = evaluateConversionChain({ clinicalCompatibility: 'ALLOW', from: purchased, to: cooked, evidence: [evidence(), rawCooked()], asOf: AS_OF });
  assert.deepEqual(result, {
    status: 'AVAILABLE', coverage: 'COMPLETE', confidence: 'MEDIUM',
    factorMin: { numerator: 720, denominator: 1 }, factorMax: { numerator: 720, denominator: 1 },
    evidenceIds: ['ap-raw', 'raw-cooked'], reasons: [],
  });
});
...
test('[TEST-130] clinical exclusion happens before malformed conversion evidence is inspected', () => {
  ...
  const result = evaluateConversionChain({ clinicalCompatibility: 'BLOCK', from: purchased, to: cooked, evidence: [malformed], asOf: AS_OF });
  assert.deepEqual(result, { status: 'UNAVAILABLE', coverage: 'UNAVAILABLE', confidence: 'NONE', evidenceIds: [], reasons: ['CLINICAL_EXCLUDED'] });
});
...
test('[TEST-130] zero, negative, inverted, and implausible factors fail closed', () => {
  for (const bad of [
    ...
    const result = evaluateConversionChain({ clinicalCompatibility: 'ALLOW', from: purchased, to: raw, evidence: [bad], asOf: AS_OF });
    assert.equal(result.status, 'UNAVAILABLE');
    if (result.status === 'UNAVAILABLE') assert.ok(result.reasons.includes(bad.factorMin.numerator <= 0 || bad.factorMin.denominator <= 0 || bad.factorMin.numerator / bad.factorMin.denominator > bad.factorMax.numerator / bad.factorMax.denominator ? 'INVALID_FACTOR' : 'IMPLAUSIBLE_FACTOR'));
  }
});
...
test('[TEST-131] basis, identity, preparation, and units must match exactly', () => {
  ...
  for (const target of mismatchTargets) {
    const result = evaluateConversionChain({ clinicalCompatibility: 'ALLOW', from: purchased, to: target, evidence: [evidence(), rawCooked()], asOf: AS_OF });
    assert.equal(result.status, 'UNAVAILABLE');
  }
});

test('[TEST-131] reverse traversal is unavailable unless the reviewed source explicitly authorizes it', () => {
  const denied = evaluateConversionChain({ clinicalCompatibility: 'ALLOW', from: raw, to: purchased, evidence: [evidence()], asOf: AS_OF });
  assert.equal(denied.status, 'UNAVAILABLE');
  if (denied.status === 'UNAVAILABLE') assert.ok(denied.reasons.includes('UNAUTHORIZED_REVERSE'));
  const allowed = evaluateConversionChain({ clinicalCompatibility: 'ALLOW', from: raw, to: purchased, evidence: [evidence({ direction: 'BIDIRECTIONAL' })], asOf: AS_OF });
...
test('[TEST-131] draft, rejected, future, stale, and superseded evidence cannot be used', () => {
  const cases: Array<[ConversionEvidenceCandidate[], string]> = [
    ...
    [[evidence({ id: 'old' }), evidence({ id: 'new', externalEvidenceKey: 'replacement', supersedesEvidenceId: 'old' })], 'SUPERSEDED_EVIDENCE'],
  ];
  for (const [items, reason] of cases) {
    const result = evaluateConversionChain({ clinicalCompatibility: 'ALLOW', from: purchased, to: raw, evidence: items, asOf: AS_OF });
...
test('[TEST-131] conflicting duplicates, multiple chains, cycles, and double application are detected', () => {
  const duplicate = evaluateConversionChain({
    clinicalCompatibility: 'ALLOW', from: purchased, to: raw, asOf: AS_OF,
    evidence: [evidence(), evidence({ factorMin: { numerator: 3, denominator: 4 }, factorMax: { numerator: 3, denominator: 4 } })],
  });
  ...
  const cyclic = evaluateConversionChain({
    clinicalCompatibility: 'ALLOW', from: purchased, to: unreachable, asOf: AS_OF,
    evidence: [evidence({ direction: 'BIDIRECTIONAL' }), evidence({ id: 'parallel', externalEvidenceKey: 'parallel', direction: 'BIDIRECTIONAL' })],
  });
...
test('[TEST-132] factor ranges propagate and conservative centavo rounding goes outward', () => {
  ...
  assert.deepEqual([conversion.factorMin, conversion.factorMax], [
    { numerator: 700, denominator: 1 }, { numerator: 800, denominator: 1 },
  ]);
  assert.deepEqual(estimateConservativeConvertedCost({
    targetQuantity: 101, priceAmountMinCentavos: 5_001, priceAmountMaxCentavos: 6_001,
    pricePurchasedQuantity: 1, conversion,
  }), {
    status: 'AVAILABLE', amountMinCentavos: 631, amountMaxCentavos: 866,
    confidence: 'MEDIUM', evidenceIds: ['ap-raw'],
  });
});
...
test('[TEST-132] an exact household measure-to-mass row is food and preparation specific', () => {
  ...
  const conversion = evaluateConversionChain({
    clinicalCompatibility: 'ALLOW', from: cup, to: grams, asOf: AS_OF,
    evidence: [evidence({
      id: 'cup-mass', externalEvidenceKey: 'carrot-cup-mass', kind: 'HOUSEHOLD_MEASURE_TO_MASS',
      from: cup, to: grams, factorMin: { numerator: 120, denominator: 1 }, factorMax: { numerator: 130, denominator: 1 },
    })],
  });
  assert.equal(conversion.status, 'AVAILABLE');
  ...
  assert.equal(evaluateConversionChain({
    clinicalCompatibility: 'ALLOW', from: { ...cup, preparationCode: 'RAW_SHREDDED' }, to: grams, asOf: AS_OF,
    evidence: [],
  }).status, 'UNAVAILABLE');
});

test('[TEST-132] unsafe money or quantity arithmetic never produces a price', () => {
  const conversion = evaluateConversionChain({ clinicalCompatibility: 'ALLOW', from: purchased, to: raw, evidence: [evidence()], asOf: AS_OF });
  for (const targetQuantity of [0, -1, 1.5, Number.MAX_SAFE_INTEGER + 1]) {
    assert.deepEqual(estimateConservativeConvertedCost({ targetQuantity, priceAmountMinCentavos: 100, priceAmountMaxCentavos: 200, pricePurchasedQuantity: 1, conversion }), {
      status: 'UNAVAILABLE', reasons: ['UNSAFE_ARITHMETIC'],
    });
  }
});

test('[TEST-133] authoritative source review validates and intentionally contains no pilot factor rows', () => {
  const review = loadIngredientConversionSourceReview(resolve(process.cwd(), 'data/ingredient-conversion-evidence/source-review-2026-09-06.json'));
...
test('[TEST-134] all 51 meals expose every missing price or conversion layer', () => {
  const snapshot = loadPsaOpenStatSnapshot(resolve(process.cwd(), 'data/psa-openstat/2026-09-06-cebu-city/manifest.json'));
  const review = loadIngredientConversionSourceReview(resolve(process.cwd(), 'data/ingredient-conversion-evidence/source-review-2026-09-06.json'));
  ...
  assert.equal(report.meals.reduce((sum, meal) => sum + meal.missing.length, 0), 195);
