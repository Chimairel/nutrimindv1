...
test('[TEST-124] admin billing status is aggregate, bounded, and privacy allowlisted', async () => {
  ...
  const repository: BillingOperationsRepository = {
    async readCounts(since) {
      ...
      return {
        pending: 4, processing: 2, failed: 5, retryable: 3, deadLetter: 2,
        recentSucceeded: 7, recentFailed: 1, openReconciliationIssues: 2,
        oldestPendingCreatedAt: new Date('2026-09-06T11:57:30.000Z'),
      };
    },
  };
  const service = new BillingOperationsStatusService(repository, () => ({
    enabled: true,
    lifecycle: 'IDLE',
    lastRunStartedAt: '2026-09-06T11:59:00.000Z',
    lastRunCompletedAt: '2026-09-06T11:59:01.000Z',
    lastRun: {
      outcome: 'COMPLETED', attempted: 3, succeeded: 2, replayed: 0,
      retryScheduled: 1, quarantined: 0, providerCallBudgetUpperBound: 3,
    },
  }), () => NOW);
  ...
  assert.deepEqual(Object.keys(status).sort(), [
    'environment', 'observedAt', 'openReconciliationIssues', 'queue', 'recent', 'worker',
  ]);
  assert.deepEqual(Object.keys(status.queue).sort(), [
    'deadLetter', 'failed', 'oldestPendingAgeSeconds', 'pending', 'processing', 'retryable',
  ]);
...
test('[TEST-124] future-skewed or absent pending timestamps never produce unsafe ages', async () => {
  for (const oldestPendingCreatedAt of [new Date('2026-09-07T00:00:00Z'), null]) {
    const service = new BillingOperationsStatusService({
      async readCounts() {
        return {
          pending: 0, processing: 0, failed: 0, retryable: 0, deadLetter: 0,
          recentSucceeded: 0, recentFailed: 0, openReconciliationIssues: 0,
          oldestPendingCreatedAt,
        };
      },
    }, () => ({
      enabled: false, lifecycle: 'STOPPED', lastRunStartedAt: null, lastRunCompletedAt: null, lastRun: null,
    }), () => NOW);
    const status = await service.getStatus();
