...
const base = {
  ...
  name: 'Chicken vegetable stew', servingGrams: 300,
...
test('[BATCH-9] exact observations deduplicate, while serving and preparation variants survive', () => {
  ...
  assert.equal(first, observedContentSignature({ ...base,
    ingredients: [...base.ingredients].reverse(), name: ' CHICKEN vegetable stew ' }));
...
test('[BATCH-9] recipe consent and image rights are independent, explicit choices', () => {
  assert.equal(observedMealConsentSchema.safeParse({ detailsConsent: false,
    imageReuseConsent: false, imageRightsConfirmed: false }).success, false);
  assert.equal(observedMealConsentSchema.safeParse({ detailsConsent: true,
    imageReuseConsent: true, imageRightsConfirmed: false }).success, false);
  assert.equal(observedMealConsentSchema.safeParse({ detailsConsent: true,
    imageReuseConsent: false, imageRightsConfirmed: false }).success, true);
  assert.equal(observedMealConsentSchema.safeParse({ detailsConsent: true,
    imageReuseConsent: true, imageRightsConfirmed: true }).success, true);
});

test('[BATCH-9] admission rejects unstructured fields and obvious contact details', () => {
  assert.equal(observedMealAdmissionSchema.safeParse({ kind: 'RECIPE_CANDIDATE',
    canonicalName: 'Chicken stew', secretUserId: 'not allowed' }).success, false);
