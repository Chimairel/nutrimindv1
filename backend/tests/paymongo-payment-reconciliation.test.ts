...
function binding(overrides: Partial<PaymentProjectionBinding> = {}): PaymentProjectionBinding {
  return {
    checkoutFound: true,
    processingId: 'processing_12345678',
    claimToken: 'claim_12345678',
    webhookEventId: 'webhook_12345678',
    providerEventId: 'evt_12345678',
    eventType: 'checkout_session.payment.paid',
    eventLivemode: false,
    eventProviderCreatedAt: new Date('2026-09-06T13:01:00.000Z'),
    eventReceivedAt: new Date('2026-09-06T13:01:01.000Z'),
    providerSessionId: 'cs_123456789',
    checkoutRequestId: 'checkout_12345678',
    checkoutUserId: 'user_12345678',
    userEligible: true,
    billingSubjectKey: 'subject_12345678',
    billingPriceId: 'price_12345678',
    productCode: 'PREMIUM',
    ...
    referenceNumber: 'nmco_12345678',
    checkoutCreatedAt: new Date('2026-09-06T12:00:00.000Z'),
    checkoutCompletedAt: new Date('2026-09-06T12:00:01.000Z'),
...
function evidence(overrides: Partial<ReconciledPaymongoCheckout> = {}): ReconciledPaymongoCheckout {
  return {
    ...
    providerPaymentIntentId: 'pi_123456789',
    providerPaymentId: 'pay_12345678',
    amountMinor: 19900,
    currency: 'PHP',
    paidAt: new Date('2026-09-06T13:00:00.000Z'),
    providerUpdatedAt: new Date('2026-09-06T13:00:01.000Z'),
    ...overrides,
  };
}
...
test('[TEST-102] local ownership, immutable request, and TEST price binding fail closed before retrieval', () => {
  assert.equal(
    failureCode(() => validatePaymongoPayment(binding({ checkoutFound: false }), evidence(), NOW)),
    'UNKNOWN_CHECKOUT_SESSION'
  );
  assert.equal(
    failureCode(() => validatePaymongoPayment(binding({ checkoutUserId: null }), evidence(), NOW)),
    'CHECKOUT_OWNERSHIP_MISSING'
  );
  assert.equal(
    failureCode(() => validatePaymongoPayment(binding({ requestHash: 'bad' }), evidence(), NOW)),
    'CHECKOUT_REQUEST_INVALID'
  );
  assert.equal(
    failureCode(() => validatePaymongoPayment(binding({ checkoutStatus: 'FAILED' }), evidence(), NOW)),
    'CHECKOUT_NOT_COMPLETED'
  );
  assert.equal(
    failureCode(() => validatePaymongoPayment(binding({ productCode: 'FREE' }), evidence(), NOW)),
...
test('[TEST-102] provider session, reference, amount, currency, mode, and paid state must match exactly', () => {
  ...
  for (const [change, code] of cases)
    assert.equal(
      failureCode(() => validatePaymongoPayment(binding(), evidence(change), NOW)),
      code
    );
});
...
test('[TEST-102] impossible and stale provider timestamps are quarantined', () => {
  assert.equal(
    failureCode(() => validatePaymongoPayment(binding(), evidence({ paidAt: new Date('2026-09-07T00:00:00Z') }), NOW)),
    'TIMESTAMP_INVALID'
  );
  assert.equal(
    failureCode(() =>
      validatePaymongoPayment(binding({ latestProviderUpdatedAt: new Date('2026-09-06T13:30:00Z') }), evidence(), NOW)
    ),
...
class FakeRepository implements PaymentProjectionRepository {
  ...
  async claimNext() {
    return { decision: 'CLAIMED' as const, binding: this.bound };
  }
  async project(
    _binding: PaymentProjectionBinding,
    _evidence: ReconciledPaymongoCheckout,
    ...
    return {
      ...
      paymentAttemptId: 'attempt_12345678',
      transactionId: 'transaction_12345678',
      entitlementGrantId: 'grant_12345678',
      ...period,
      replayed: this.projections > 1,
    };
  }
  async fail(_binding: PaymentProjectionBinding, failure: { code: string; retryable: boolean }) {
...
test('[TEST-126] lifecycle cancellation reaches retrieval and releases the durable claim for retry', async () => {
  ...
  const processing = new PaymongoPaymentProjectionService(repository, gateway, () => NOW).processNext(
    controller.signal
  );
  await started;
  controller.abort();
