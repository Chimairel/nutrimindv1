...
test('[TEST-137] compensation mutation bodies are strict, bounded, and PHP-scoped', () => {
  const policy = createCompensationPolicySchema.parse({
    version: '2026-Q4-demo',
    currency: 'PHP',
    baseRetainerMinor: 500_000,
  ...
  assert.throws(() =>
    createCompensationPolicySchema.parse({
      ...policy,
      workloadBands: [
  ...
  assert.doesNotThrow(() =>
    createCompensationPeriodSchema.parse({
      policyId: 'policy-1',
  ...
  assert.doesNotThrow(() =>
    createCompensationAdjustmentSchema.parse({
      amountMinor: -5_000,
      currency: 'PHP',
  ...
  assert.deepEqual(decideCompensationAdjustmentSchema.parse({ decision: 'REJECT', reason: 'Unsupported evidence' }), {
    decision: 'REJECT',
    reason: 'Unsupported evidence',
  });
  assert.deepEqual(prepareCompensationPayoutSchema.parse({ idempotencyKey: 'payout:1' }), {
  ...
  assert.doesNotThrow(() =>
    reverseWorkCreditSchema.parse({ reversalActionKey: 'review:1:reversal', reasonCode: 'INVALID_SOURCE' })
  );
});
...
test('[TEST-138] compensation routes are role-scoped and payout amount is server-authoritative', () => {
  ...
  assert.doesNotMatch(
    readFileSync(join(process.cwd(), 'src', 'validation', 'compensation.schemas.ts'), 'utf8').match(
      /prepareCompensationPayoutSchema[\s\S]*?strict\(\)/
    )?.[0] ?? '',
    /amountMinor/
  );
  ...
  assert.ok(
    nutritionistAuth >= 0 &&
      nutritionistRole > nutritionistAuth &&
      eligibility > nutritionistRole &&
      ownCompensation > eligibility
  );
...
test('[TEST-139] completed review decisions create keyed credit in the same transaction and claims do not', () => {
  ...
  assert.match(
    creditService,
    /sourceActionKey = `meal-plan-review:\$\{input\.mealPlanId\}:\$\{input\.stage\.toLowerCase\(\)\}`/
  );
...
test('[TEST-140] migration adds actor evidence and immutable compensation records without altering accepted migrations', () => {
  const migration = readFileSync(
    join(process.cwd(), 'prisma', 'migrations', '20260906235900_compensation_admin_workflow', 'migration.sql'),
    'utf8'
  );
...
test('[TEST-141] role UIs disclose separation, own-only scope, outcome neutrality, and manual evidence', () => {
  ...
  assert.match(own, /Claims and expired or abandoned work do\s+not count/);
});
