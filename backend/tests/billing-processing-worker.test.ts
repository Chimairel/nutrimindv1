...
import { BillingProcessingWorker, PaymentProjectionProcessor } from '../src/services/billing-processing-worker.service';
...
function projection(replayed = false) {
  return {
    ...
    entitlementGrantId: 'grant',
    effectiveFrom: new Date('2026-09-06T00:00:00Z'),
    effectiveUntil: new Date('2026-10-06T00:00:00Z'),
    replayed,
  };
}
...
test('[TEST-118] worker tuning is bounded and internally consistent', () => {
  ...
  assert.throws(
    () =>
      loadBillingProcessingWorkerConfig(
        {
          BILLING_PROCESSING_WORKER_ENABLED: 'true',
          PAYMONGO_ENVIRONMENT: 'TEST',
          NODE_ENV: 'test',
    ...
    /BILLING_PROCESSING_WORKER_CONCURRENCY/
  );
  assert.throws(
    () =>
      loadBillingProcessingWorkerConfig(
        {
          ...
          PAYMONGO_ENVIRONMENT: 'TEST',
          NODE_ENV: 'test',
          BILLING_PROCESSING_WORKER_BATCH_SIZE: '2',
          BILLING_PROCESSING_WORKER_PROVIDER_CALL_BUDGET: '3',
        },
        paymongo
      ),
...
test('[TEST-121] outcomes distinguish retry, terminal quarantine, and replay', async () => {
  ...
  const result = await new BillingProcessingWorker(enabledConfig, {
    async processNext() {
      return results.shift() || { decision: 'NO_WORK' };
    },
  }).runOnce();
...
test('[TEST-123] worker logs contain fixed summaries and discard thrown private details', async () => {
  ...
  const worker = new BillingProcessingWorker(
    ...
    {
      async processNext() {
        throw new Error('sk_test_private card health payload');
      },
    },
    undefined,
    {
