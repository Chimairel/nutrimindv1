...
import { BillingHttpRequest, BillingHttpTransport, CheckoutReconciliationGatewayError } from '../src/billing/contracts';
    ...
    JSON.stringify({
      data: {
        ...
        attributes: {
          livemode: false,
          status: 'active',
          reference_number: 'nmco_reconcile_12345678',
          ignored_private_shape: { discard: true },
          payments: [
            {
              id: 'pay_reconcile_12345678',
              type: 'payment',
              attributes: {
                amount: 19_900,
                currency: 'PHP',
                livemode: false,
                status: 'paid',
                payment_intent_id: 'pi_reconcile_12345678',
...
test('[TEST-104] parser requires exact session, TEST mode, one paid payment, and succeeded matching intent', () => {
  assert.throws(
    () => parsePaymongoCheckoutReconciliation(response(), 'cs_different_12345678'),
    (error: unknown) => errorCode(error, 'PROVIDER_SESSION_MISMATCH', false)
  );
  assert.throws(
    () => parsePaymongoCheckoutReconciliation(response({ livemode: true }), SESSION),
    (error: unknown) => errorCode(error, 'PROVIDER_LIVE_MODE_REJECTED', false)
  );
  assert.throws(
    () => parsePaymongoCheckoutReconciliation(response({ payments: [] }), SESSION),
...
test('[TEST-104] parser rejects malformed, hostile, contradictory, and oversized success responses', async () => {
  assert.throws(
    () => parsePaymongoCheckoutReconciliation(Buffer.from('not-json'), SESSION),
    /PROVIDER_RESPONSE_INVALID/
  );
  assert.throws(
    () => parsePaymongoCheckoutReconciliation(response({ reference_number: 'bad value' }), SESSION),
    /PROVIDER_RESPONSE_INVALID/
  );
  assert.throws(
    () =>
  ...
  await assert.rejects(
    () => new PaymongoReconciliationGateway(config(), oversized.adapter).retrieveCheckoutSession(SESSION),
...
test('[TEST-104] authentication, unknown session, rejection, and transient failures have stable sanitized classes', async () => {
  for (const [status, code, retryable] of [
    ...
    [404, 'PROVIDER_CHECKOUT_NOT_FOUND', false],
    [422, 'PROVIDER_REQUEST_REJECTED', false],
    [408, 'PROVIDER_TEMPORARILY_UNAVAILABLE', true],
    [429, 'PROVIDER_TEMPORARILY_UNAVAILABLE', true],
  ...
  const failedTransport: BillingHttpTransport = {
    async send() {
  ...
  const oversizedTransport: BillingHttpTransport = {
    async send() {
      throw new BillingHttpResponseTooLargeError();
    },
  };
  await assert.rejects(
...
test('[TEST-104] unsafe origin, malformed ID, and non-JSON content fail before usable evidence', async () => {
  let calls = 0;
  const fake: BillingHttpTransport = {
    async send() {
      calls += 1;
  ...
  } as unknown as EnabledPaymongoReconciliationConfig;
  await assert.rejects(
    () => new PaymongoReconciliationGateway(unsafe, fake).retrieveCheckoutSession(SESSION),
    (error: unknown) => errorCode(error, 'PROVIDER_REQUEST_REJECTED', false)
  );
  await assert.rejects(
    () => new PaymongoReconciliationGateway(config(), fake).retrieveCheckoutSession('../escape'),
    (error: unknown) => errorCode(error, 'PROVIDER_REQUEST_REJECTED', false)
  );
  assert.equal(calls, 0);
  const wrongType = transport(response(), 200, 'text/html');
  await assert.rejects(
    () => new PaymongoReconciliationGateway(config(), wrongType.adapter).retrieveCheckoutSession(SESSION),
    (error: unknown) => errorCode(error, 'PROVIDER_RESPONSE_INVALID', false)
  );
});
