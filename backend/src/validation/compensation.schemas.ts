...
export const createCompensationPolicySchema = z
  .object({
  ...
  .superRefine((value, context) => {
    ...
    sorted.forEach((band, index) => {
      if (band.minimumUnitsMillis > value.workloadUnitCapMillis) {
        context.addIssue({
          code: 'custom',
          path: ['workloadBands', index, 'minimumUnitsMillis'],
          message: 'A workload threshold cannot exceed the cap.',
        });
      }
      ...
      if (index > 0 && sorted[index - 1].allowanceMinor > band.allowanceMinor) {
        context.addIssue({
          code: 'custom',
          path: ['workloadBands'],
          message: 'Workload allowances must not decrease.',
        });
      }
    });
  });

export const createCompensationPeriodSchema = z
  .object({
...
export const createCompensationAdjustmentSchema = z
  .object({
    amountMinor: z
      .number()
      .int()
...
export const recordManualPayoutSchema = z
  .object({
    externalReference: z
      .string()
      .trim()
      .min(1)
      .max(191)
      .regex(/^[A-Za-z0-9][A-Za-z0-9 ./_:#-]*$/, 'The evidence reference contains unsupported characters.'),
  })
  .strict();

export const reverseWorkCreditSchema = z
  .object({
