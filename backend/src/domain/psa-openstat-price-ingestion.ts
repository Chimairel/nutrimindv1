...
const ambiguousMappingSchema = z
  .object({
...
export const psaOpenStatManifestSchema = z
  .object({
...
function parsePeriodHeader(header: string): { label: string; from: string; to: string } {
  const match =
    /^(\d{4}) (January|February|March|April|May|June|July|August|September|October|November|December)$/.exec(header);
...
export function parsePsaOpenStatCsv(
  ...
  if (configuredByLabel.size !== file.commodities.length)
    throw new Error(`${file.dataFile}: duplicate configured commodity label`);
  ...
  if (missingRows.length > 0) {
    throw new Error(
      `${file.dataFile}: missing configured rows: ${missingRows.map((item) => item.sourceLabel).join(', ')}`
    );
  }
...
export function validatePsaOpenStatRequest(
  ...
  if (
    new Set(commodityCodes).size !== commodityCodes.length ||
...
export function loadPsaOpenStatSnapshot(manifestPath: string): LoadedPsaSnapshot {
  ...
  const selectedNames = new Set(
    manifest.files.flatMap((file) => file.commodities.map((item) => item.catalogueTargetName))
  );
  const selectedRows = catalogueRows.filter((item) => selectedNames.has(item.foodName));
  const selectedMeals = COMMON_MEAL_CATALOGUE.filter((meal) =>
    meal.ingredients.some((item) => selectedNames.has(item.foodName))
  );
  if (
    manifest.catalogue.mealCount !== COMMON_MEAL_CATALOGUE.length ||
...
export function buildPsaCoverageReport(snapshot: LoadedPsaSnapshot): PsaCoverageReport {
  const allCommodities = snapshot.manifest.files.flatMap((file) =>
  ...
  const exactIdentityNames = new Set(
    allCommodities
      .filter(
        ({ commodity }) =>
          commodity.mapping.state === 'EXACT' && commodity.mapping.foodName === commodity.catalogueTargetName
      )
      .map(({ commodity }) => commodity.catalogueTargetName)
  );
  const availableIdentityNames = new Set(
    allCommodities
      .filter(
        ({ file, commodity }) =>
  ...
  const countMeals = (names: Set<string>) =>
    COMMON_MEAL_CATALOGUE.filter((meal) => meal.ingredients.some((item) => names.has(item.foodName))).length;
