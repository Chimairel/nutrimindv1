...
const sourceReviewSchema = z.object({
  ...
  redistributionStatus: z.enum(['REDISTRIBUTABLE', 'AUTHORIZED_INTERNAL_USE', 'METADATA_ONLY', 'LICENSE_REVIEW_REQUIRED']),
  ...
  if ((source.redistributionStatus === 'REDISTRIBUTABLE' || source.redistributionStatus === 'AUTHORIZED_INTERNAL_USE') &&
      (!source.licenseCode || !source.contentSha256)) {
    context.addIssue({ code: 'custom', message: 'Usable evidence requires an explicit license code and artifact SHA-256' });
  }
});

export const ingredientConversionSourceReviewSchema = z.object({
  ...
  for (const source of review.sources) {
    if (codes.has(source.code)) context.addIssue({ code: 'custom', message: `Duplicate source code: ${source.code}` });
...
function countMeals(names: Set<string>): number {
  return COMMON_MEAL_CATALOGUE.filter((meal) => meal.ingredients.some((ingredient) => names.has(ingredient.foodName))).length;
}
...
export function buildIngredientConversionCoverageReport(
  ...
  const configured = snapshot.manifest.files.flatMap((file) => file.commodities.map((commodity) => ({ file, commodity })));
  ...
  const availableKeys = new Set(snapshot.parsedFiles.flatMap((file) => file.cells)
    .filter((cell) => cell.status === 'AVAILABLE' && new Date(cell.observedTo).getTime() <= asOf.getTime() &&
  ...
  const exact = new Set(configured.filter(({ commodity }) => commodity.mapping.state === 'EXACT')
    .map(({ commodity }) => commodity.catalogueTargetName));
  const current = new Set(configured.filter(({ file, commodity }) => commodity.mapping.state === 'EXACT' &&
    availableKeys.has(`${file.matrixId}:${commodity.externalCode}`)).map(({ commodity }) => commodity.catalogueTargetName));
  const quantity = new Set([...current].filter((name) => COMMON_MEAL_CATALOGUE.some((meal) =>
    meal.ingredients.some((ingredient) => ingredient.foodName === name && Number.isFinite(ingredient.grams) && ingredient.grams > 0))));
  const sameIdentity = new Set(configured.filter(({ file, commodity }) => commodity.mapping.state === 'EXACT' &&
    commodity.mapping.foodName === commodity.catalogueTargetName &&
    availableKeys.has(`${file.matrixId}:${commodity.externalCode}`)).map(({ commodity }) => commodity.catalogueTargetName));
  const reviewedConversion = new Set<string>();
  if (review.pilotEvidence.length !== 0) throw new Error('Coverage implementation must be updated before pilot evidence is enabled');
  ...
  const meals = COMMON_MEAL_CATALOGUE.map((meal) => {
    ...
    const status: 'COMPLETE' | 'PARTIAL' | 'UNPRICED' = pricedIngredientRows === meal.ingredients.length
      ? 'COMPLETE'
      : pricedIngredientRows > 0 ? 'PARTIAL' : 'UNPRICED';
    return { mealName: meal.mealName, status, pricedIngredientRows, totalIngredientRows: meal.ingredients.length, missing };
  });
  ...
  return {
    ...
    missingByReason: Object.fromEntries(Object.entries(missingByReason).sort(([left], [right]) => left.localeCompare(right))),
    meals,
  };
}
