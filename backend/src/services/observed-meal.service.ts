...
import {
  ...
  type ObservedIngredient,
} from '@/domain/observed-meal.policy';
...
function confirmedAtCurrentRevision(item: {
  ...
  return (
    ['VERIFIED', 'CORRECTED'].includes(item.nutritionStatus) &&
...
export class ObservedMealService {
  static async invalidateSource(tx: Prisma.TransactionClient, itemId: string) {
    const submissions = await tx.observedMealSubmission.findMany({
      where: {
        sourceOutsideMealItemId: itemId,
        status: { not: 'WITHDRAWN' },
      },
      select: { id: true, rawRecipeCandidateId: true },
    });
    if (!submissions.length) return;
    await tx.observedMealSubmission.updateMany({
      where: { id: { in: submissions.map((row) => row.id) } },
      data: { status: 'WITHDRAWN', withdrawnAt: new Date() },
    });
    for (const candidateId of new Set(
      submissions.flatMap((row) => (row.rawRecipeCandidateId ? [row.rawRecipeCandidateId] : []))
    )) {
      const remaining = await tx.observedMealSubmission.count({
        where: {
          rawRecipeCandidateId: candidateId,
  ...
  static async consent(
    ...
    itemId: string,
    consent: {
      detailsConsent: boolean;
      imageReuseConsent: boolean;
    ...
    return prisma.$transaction(
      async (tx) => {
        ...
        const now = new Date();
        const existing = await tx.observedMealSubmission.findUnique({
          where: {
            sourceOutsideMealItemId_sourceRevision: {
              sourceOutsideMealItemId: item.id,
              sourceRevision: item.currentRevision,
            },
          },
        });
        const submission = existing
          ? await tx.observedMealSubmission.update({
              ...
              data: {
                ...
                ...(existing.status === 'WITHDRAWN'
                  ? {
                      ...
                      rawRecipeCandidateId: null,
                      classifiedAt: null,
                      classifiedByNutritionistId: null,
                    }
                  : {}),
              },
            })
          : await tx.observedMealSubmission.create({
              data: {
                sourceUserId: userId,
                sourceOutsideMealItemId: item.id,
                sourceRevision: item.currentRevision,
                detailsConsentedAt: now,
                imageReuseConsentedAt: imageReuse ? now : null,
              },
            });
        await tx.auditEvent.create({
          data: {
            actorUserId: userId,
            action: 'OBSERVED_MEAL_REUSE_CONSENT',
            entityType: 'ObservedMealSubmission',
    ...
    return prisma.$transaction(
      async (tx) => {
        ...
        if (row.rawRecipeCandidateId) {
          const remaining = await tx.observedMealSubmission.count({
            where: {
              rawRecipeCandidateId: row.rawRecipeCandidateId,
              status: 'ADMITTED_RECIPE',
            },
          });
          if (!remaining)
        ...
        await tx.auditEvent.create({
          data: {
            actorUserId: userId,
            action: 'OBSERVED_MEAL_REUSE_WITHDRAWN',
            entityType: 'ObservedMealSubmission',
            entityId: row.id,
          },
        });
  ...
  static async pending() {
    const rows = await prisma.observedMealSubmission.findMany({
      where: {
        status: 'SUBMITTED',
        sourceOutsideMealItemId: { not: null },
        sourceUserId: { not: null },
      },
      select: {
        id: true,
        sourceRevision: true,
        imageReuseConsentedAt: true,
        createdAt: true,
    ...
    return rows
      .filter(
        (row) =>
          row.sourceOutsideMealItem?.mealLog.status === 'DONE' &&
          row.sourceOutsideMealItem.currentRevision === row.sourceRevision &&
          confirmedAtCurrentRevision(row.sourceOutsideMealItem)
      )
      .slice(0, 50);
  }
  ...
  static async admit(nutritionistProfileId: string, submissionId: string, input: Admission) {
    ...
    return prisma.$transaction(
      async (tx) => {
        const submission = await tx.observedMealSubmission.findFirst({
          where: {
            id: submissionId,
            status: 'SUBMITTED',
            sourceOutsideMealItemId: { not: null },
        ...
        if (
          !submission ||
          !item ||
          item.mealLog.status !== 'DONE' ||
          item.currentRevision !== submission.sourceRevision ||
          !confirmedAtCurrentRevision(item)
        )
        ...
        if (input.kind === 'FOOD_REFERENCE') {
          const signature = observedContentSignature({
            kind: input.kind,
            name,
          ...
          const reference =
            existing ??
            (await tx.observedFoodReference.create({
              data: {
                signature,
                name,
                normalizedName: normalizeObservedName(name),
                servingGrams: item.portionGrams,
                ...macros,
              },
            }));
          ...
          await tx.observedMealSubmission.update({
            ...
            data: {
              ...
              classifiedByNutritionistId: nutritionistProfileId,
          ...
          if (
            ...
            input.preparation.trim().length < 20 ||
            ingredients.length < 2 ||
            !mealTypes.length ||
            mealTypes.some(
              (type) => !([MealType.BREAKFAST, MealType.LUNCH, MealType.DINNER] as MealType[]).includes(type)
            ) ||
            ingredients.some(
              (row) =>
                !row.name.trim() ||
                !assertShareableText(row.name) ||
                !row.unit.trim() ||
            ...
            throw new AppError(
              'A reproducible recipe needs preparation, quantified ingredients, and meal types.',
              422,
          ...
          if (classification.status !== 'COMPLETE')
            throw new AppError(
              'Resolve or clarify unknown ingredients before admission.',
              422,
          ...
          const candidate =
            existing ??
            (await tx.rawRecipeCandidate.create({
              data: {
                sourceRecordId: `observed:${signature}`,
                sourceName: 'USER_OBSERVED',
                sourceUrl: `urn:nutrimind:observed:${signature}`,
                recipeName: name,
                normalizedName: normalizeObservedName(name),
                contentSignature: signature,
                cuisines: [],
                description: input.preparation.trim(),
                ...
                publishedNutrition: Prisma.JsonNull,
                ...macros,
                originalServings: 1,
                applicableMealTypes: {
                  create: mealTypes.map((mealType) => ({
                    mealType,
                    source: 'NUTRITIONIST_REVIEW' as const,
          ...
          if (existing?.status === 'RETIRED')
            await tx.rawRecipeCandidate.update({
