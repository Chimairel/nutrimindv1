...
const recordReplacementReviewCredit = recordCompletedMealPlanReviewCredit;
...
export class NutritionistReviewService {
  ...
  static async generateReplacementCandidate(nutritionistProfileId: string, mealPlanId: string, reason: string) {
    ...
    if (plan.claimedByNutritionistId !== nutritionistProfileId || !plan.claimedAt || plan.claimedAt < claimCutoff) {
      throw new Error('You must hold an active claim before generating a replacement. Please reopen it from the queue.');
    }
    ...
    const candidate = candidateMealSchema.parse({
      ...
      proteinG: Number(raw.proteinG) || 20,
      carbsG: Number(raw.carbsG) || 50,
      fatG: Number(raw.fatG) || 15,
  ...
  /**
   * Atomically replaces a rejected meal with an approved candidate, certifying the replacement
   * and providing immediate zero-pending delivery to the patient.
   */
  static async replaceAndApproveMealPlan(
    ...
    const isFirstHighRiskApproval = plan.highRiskReviewRequired && plan.reviewApprovalCount === 0;
      ...
      async (tx) => {
        ...
        const createdReplacement = await tx.mealPlan.create({
          data: {
            ...
            reviewApprovalCount: isFirstHighRiskApproval ? 1 : plan.highRiskReviewRequired ? 2 : 1,
            ...
            ingredients: {
              create: candidate.ingredients.map((ing) => ({
                ingredientName: ing.name,
                category: ing.category || 'PANTRY',
                dataSource: (ing.dataSource as MealIngredientDataSource) || MealIngredientDataSource.GEMINI_ESTIMATED,
              })),
            },
          },
        });
