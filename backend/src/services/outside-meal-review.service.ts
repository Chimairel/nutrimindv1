...
export class OutsideMealReviewService {
      ...
      async (tx) => {
        ...
        const claimed = await tx.outsideMealReview.updateMany({
          ...
          data: {
            ...
            claimedByNutritionistId: nutritionistProfileId,
            claimedAt: now,
            claimedRevision: review.outsideMealLogItem.currentRevision,
          },
        });
        if (claimed.count !== 1)
  ...
  static async replyByUser(userId: string, logId: string, itemId: string, content: string) {
    return prisma.$transaction(
      async (tx) => {
        ...
        if (review._count.messages >= 12)
          throw new AppError('This clarification thread has reached its limit.', 409, 'CLARIFICATION_LIMIT');
        ...
        if (changed.count !== 1)
          throw new AppError('This item changed. Reload before replying.', 409, 'ITEM_REVISION_CHANGED');
        ...
        await tx.outsideMealReviewMessage.create({
          data: {
            ...
            sender: OutsideMealMessageSender.USER,
            authorUserId: userId,
        ...
        await tx.auditEvent.create({
          data: {
            actorUserId: userId,
            action: 'OUTSIDE_MEAL_CLARIFICATION_REPLIED',
            entityType: 'OutsideMealLogItem',
            entityId: item.id,
            metadata: { revision, reviewId: review.id },
          },
        });
  ...
  static async imageForClaimedReview(nutritionistProfileId: string, reviewId: string) {
    ...
    const review = await prisma.outsideMealReview.findFirst({
      ...
      select: {
        outsideMealLogItem: { select: { mealLog: { select: { outsideImage: true, outsideImageMime: true } } } },
      },
    });
    ...
    return prisma.$transaction(
      async (tx) => {
        ...
        if (review.claimedRevision === null || review.claimedRevision !== item.currentRevision)
          throw new AppError(
            'This meal changed after it was claimed. Claim the new revision.',
            409,
            'STALE_REVIEW_REVISION'
          );
        if (['VERIFY', 'CORRECT'].includes(action.action) && !item.includedInTotals && action.action !== 'CORRECT')
          throw new AppError(
            'An unresolved item needs values before it can be confirmed.',
            422,
            'NUTRITION_VALUES_REQUIRED'
          );
        ...
        await tx.outsideMealReview.update({
          ...
          data: {
            status: reviewStatus,
            reviewedAt: now,
            reviewedRevision: review.claimedRevision,
            claimedByNutritionistId: null,
            claimedAt: null,
            claimedRevision: null,
          },
        ...
        if (needsInfo) {
          ...
          await tx.outsideMealReviewMessage.create({
            data: {
              outsideMealReviewId: review.id,
              sender: OutsideMealMessageSender.NUTRITIONIST,
        ...
        await tx.notification.create({
          data: {
            ...
            title: needsInfo
              ? 'Outside meal needs more information'
              : unverifiable
                ? 'Outside meal estimate could not be confirmed'
                : 'Outside meal nutrition updated',
            message: needsInfo
