...
function aggregateSource(items: Array<{ source: OutsideMealItemSource; includedInTotals: boolean }>): MealLogDataSource {
...
async function summarizeAndPersist(tx: Prisma.TransactionClient, logId: string) {
  ...
  const summary = summarizeOutsideMealNutrition(items.map((item) => ({
    source: item.source,
  ...
  const compatibility = items.some((item) => item.compatibilityStatus === OutsideMealCompatibilityStatus.CONFLICT_DETECTED)
    ? 'CONFLICT_DETECTED'
  ...
  const log = await tx.mealLog.update({
    ...
    data: {
      ...
      outsideSafetyFollowUp: { status: compatibility, messages: compatibility === 'NO_KNOWN_CONFLICT' ? [] : [
...
export class OutsideMealCaptureService {
  static async suggestions(userId: string, search: string) {
    ...
    const otherKnown = [
      ...catalog.map((meal) => {
        const flaggedAllergen = meal.safetyDeclarations.find((declaration) =>
          declaration.declarationType === 'ALLERGEN_PRESENT' && allergens.includes(declaration.canonicalKey as typeof allergens[number])
        );
        const tags = Array.isArray(meal.dietaryTags) ? meal.dietaryTags : [];
        const dietaryConflict = user.userProfile?.dietaryPreference && !tags.includes(user.userProfile.dietaryPreference);
        return {
          kind: 'KNOWN_CATALOG', id: meal.id, name: meal.mealName,
          label: flaggedAllergen?.canonicalKey ? `Possible ${flaggedAllergen.canonicalKey.toLowerCase()} conflict` :
            dietaryConflict ? 'Does not match your diet preference' : 'Safety or serving evidence is incomplete',
          compatibility: flaggedAllergen || dietaryConflict ? 'CONFLICT' : 'UNKNOWN',
        };
      }),
      ...raw.map((meal) => ({ kind: 'RAW_RECIPE', id: meal.id, name: meal.recipeName,
  ...
  static async editItem(userId: string, logId: string, itemId: string, input: EditInput) {
    ...
    return prisma.$transaction(async (tx) => {
      const item = await tx.outsideMealLogItem.findFirst({
        where: { id: itemId, mealLogId: logId, mealLog: { userId, source: MealLogSource.USER_LOGGED, status: MealLogStatus.DONE } },
      ...
      const ingredients = input.name === item.name && Array.isArray(item.ingredients)
      ...
      const compatibilityStatus = evaluateOutsideMealCompatibility({
        name: input.name, ingredients, baselineStatus: OutsideMealCompatibilityStatus.INSUFFICIENT_EVIDENCE,
        restrictions,
      }).status;
      const includedInTotals = Boolean(input.reportedNutrition);
      const source = includedInTotals && item.mealLibraryId
        ? OutsideMealItemSource.USER_ADJUSTED_LIBRARY
        : includedInTotals ? OutsideMealItemSource.USER_REPORTED : OutsideMealItemSource.UNRESOLVED;
      const nutritionStatus = includedInTotals ? OutsideMealNutritionStatus.USER_REPORTED : OutsideMealNutritionStatus.UNRESOLVED;
      const next = {
        name: input.name,
      ...
      await tx.auditEvent.create({ data: {
        actorUserId: userId, action: 'OUTSIDE_MEAL_ITEM_USER_REVISED', entityType: 'OutsideMealLogItem', entityId: item.id,
  ...
  static async voidLog(userId: string, logId: string, reason: string) {
    return prisma.$transaction(async (tx) => {
      ...
      await tx.auditEvent.create({ data: {
        actorUserId: userId, action: 'OUTSIDE_MEAL_VOIDED', entityType: 'MealLog', entityId: logId,
  ...
  static async image(userId: string, logId: string) {
    ...
    if (!log?.outsideImage || !log.outsideImageMime) throw new AppError('Image not found.', 404, 'OUTSIDE_IMAGE_NOT_FOUND');
