...
export async function importPsaOpenStatSnapshot(
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      const existingSource = await tx.ingredientPriceSource.findUnique({
        where: { code: snapshot.manifest.source.code },
      });
      if (existingSource) {
        ...
        await tx.ingredientPriceSource.create({
          data: {
            id: sourceId,
      ...
      if (existingGeography) {
        ...
        await tx.ingredientPriceGeography.create({
          data: {
            id: geographyId,
            scheme: snapshot.manifest.geography.scheme,
            externalCode: snapshot.manifest.geography.externalCode,
      ...
      for (const file of snapshot.manifest.files) {
        for (const commodity of file.commodities) {
          ...
          if (existingCommodity) {
            ...
            await tx.ingredientPriceCommodity.create({
              data: {
                id: commodityId,
                sourceId,
                sourceCommodityKey,
          ...
          if (existingMapping) {
            ...
            await tx.ingredientPriceCommodityMapping.create({
              data: {
                id: mappingId,
                commodityId,
                state: mapping.state,
      ...
      for (const file of snapshot.manifest.files) {
        ...
        if (existingPublication) {
          ...
          await tx.ingredientPricePublication.create({
            data: {
              id: publicationId,
              sourceId,
              externalPublicationId,
        ...
        for (const cell of available) {
          ...
          const commodity = file.commodities.find(
            (candidate) => `${file.matrixId}:${candidate.externalCode}` === cell.sourceCommodityKey
          );
          ...
          if (existingObservation) {
            assertSame(`IngredientPriceObservation ${sourceObservationKey}`, [
              ...
              [
                existingObservation.originalCommodityDescription === commodity.sourceLabel,
                'originalCommodityDescription',
              ],
              ...
              [
                existingObservation.normalizedQuantity?.toString() === commodity.normalizedQuantity,
                'normalizedQuantity',
              ],
          ...
          await tx.ingredientPriceObservation.create({
            data: {
              id: observationId,
              publicationId,
              commodityId,
