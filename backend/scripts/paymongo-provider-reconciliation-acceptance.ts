...
async function seedAcceptanceBinding() {
  ...
  const product = await prisma.billingProduct.create({ data: {
    code: 'PREMIUM', displayName: 'Premium', status: 'ACTIVE', featureSetVersion: 'provider-acceptance-v1',
  } });
  const price = await prisma.billingPrice.create({ data: {
    productId: product.id, provider: 'PAYMONGO', environment: 'TEST', currency: 'PHP',
    amountMinor: EXPECTED_AMOUNT_MINOR, interval: 'MONTH', intervalCount: 1, version: 1, isActive: true,
  } });
  const requestHash = hash(JSON.stringify({
    fixture: 'reconstructed-phase-3b-binding', provider: 'PAYMONGO', environment: 'TEST',
    priceId: price.id, productCode: 'PREMIUM', amountMinor: EXPECTED_AMOUNT_MINOR,
    currency: 'PHP', paymentMethods: ['card'], referenceNumber: REFERENCE,
  }));
  await prisma.billingCheckoutRequest.create({ data: {
    userId: user.id, billingSubjectKey: 'subject_provider_acceptance_12345678', billingPriceId: price.id,
    provider: 'PAYMONGO', environment: 'TEST', requestIdempotencyKey: 'provider-acceptance-request-12345678',
    requestHash, providerIdempotencyKey: 'provider-acceptance-key-12345678', referenceNumber: REFERENCE,
    status: 'SUCCEEDED', attemptCount: 1, providerSessionId: SESSION_ID,
    checkoutUrl: `https://checkout.paymongo.com/${SESSION_ID}`,
    completedAt: new Date('2026-09-06T05:01:00.000Z'), createdAt: new Date('2026-09-06T05:00:00.000Z'),
  } });
  const fixtureTime = new Date();
  await prisma.providerWebhookEvent.create({ data: {
    provider: 'PAYMONGO', environment: 'TEST', providerEventId: EVENT_ID,
    eventType: 'checkout_session.payment.paid', livemode: false,
    payloadHash: hash(JSON.stringify({ fixture: 'sanitized-accepted-event-reconstruction', eventId: EVENT_ID, sessionId: SESSION_ID })),
    sanitizedPayload: {
      schemaVersion: 1,
    ...
    signatureKeyVersion: 'acceptance-reconstruction-v1', providerCreatedAt: fixtureTime, receivedAt: fixtureTime,
...
async function main() {
  ...
  const processor = new PaymongoPaymentProjectionService(
    new PrismaPaymentProjectionRepository(prisma), gateway,
  );
  const first = await processor.processNext();
  if (first.decision !== 'SUCCEEDED') {
    const financeRows = await Promise.all([
      prisma.userSubscription.count(), prisma.billingInvoice.count(), prisma.paymentAttempt.count(),
      prisma.billingTransaction.count(), prisma.financialLedgerEntry.count(), prisma.entitlementGrant.count(),
    ]);
    assert.deepEqual(financeRows, [0, 0, 0, 0, 0, 0]);
    process.stdout.write(JSON.stringify({ acceptance: 'SANITIZED_GAP', code: first.code, providerRequests: transport.count }));
    process.exitCode = 2;
    return;
  }
  assert.ok(captured);
  ...
  assert.equal(ledger.reduce((sum, entry) => sum + (entry.direction === 'DEBIT' ? entry.amountMinor : -entry.amountMinor), 0), 0);
  ...
  assert.equal(resolveBillingEntitlement({
    at: new Date(grant.effectiveUntil.getTime() - 1), grants: grantEvidence, subscriptions: subscriptionEvidence,
  }).tier, 'PREMIUM');
  assert.equal(resolveBillingEntitlement({
    at: grant.effectiveUntil, grants: grantEvidence, subscriptions: subscriptionEvidence,
  }).tier, 'FREE');
  assert.equal(resolveBillingEntitlement({ at: new Date(), grants: [], subscriptions: [] }).tier, 'FREE');

  const unrelatedCounts = await Promise.all([
    prisma.providerCustomer.count(), prisma.billingSubscriptionCancellation.count(), prisma.billingRefund.count(),
    prisma.nutritionistWorkCredit.count(), prisma.compensationStatement.count(), prisma.compensationPayout.count(),
  ]);
  assert.deepEqual(unrelatedCounts, [0, 0, 0, 0, 0, 0]);

  process.stdout.write(JSON.stringify({
    acceptance: 'SUCCEEDED', providerRequests: transport.count,
    evidence: {
      sessionId: captured.providerSessionId, referenceNumber: captured.referenceNumber,
      paymentId: captured.providerPaymentId, paymentIntentId: captured.providerPaymentIntentId,
      paymentStatus: captured.paymentStatus, paymentIntentStatus: captured.paymentIntentStatus,
      amountMinor: captured.amountMinor, currency: captured.currency, livemode: captured.livemode,
      paidAt: captured.paidAt.toISOString(), providerUpdatedAt: captured.providerUpdatedAt.toISOString(),
    },
