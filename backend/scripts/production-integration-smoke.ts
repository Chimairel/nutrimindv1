...
async function main() {
  ...
  const aggregateMutation = () =>
    prisma.dailyNutritionLog.upsert({
  ...
  console.log(
    '[Integration] Exact preview persistence, write idempotency, atomic claims, refresh rotation, certification, invalidation, check-in idempotency, and generation contention passed.'
  );
}
