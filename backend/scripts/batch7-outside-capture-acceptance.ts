...
async function main() {
  ...
  try {
    const user = await prisma.user.create({
      data: {
        name: 'Batch 7 Capture Fixture',
        email: `batch7-${run}@example.invalid`,
    ...
    const food = await prisma.foodItem.create({
      data: {
        name: `Batch 7 measured rice ${run}`,
        category: 'Grains',
        source: 'FNRI',
        calories: 130,
        proteinG: 2.7,
    ...
    assert.ok(
      suggestions.eligible.some((meal) => meal.id === known.id),
      'Certified recipe should be first-party eligible suggestion.'
    );
    ...
    const libraryCommit = await MealLogService.logOutsideMeal({
      ...
      requestKey: libraryInput.requestKey,
    });
    ...
    assert.equal(
      (
        await prisma.outsideMealItemRevision.findFirstOrThrow({
    ...
    const adjusted = await MealLogService.logOutsideMeal({
      ...
      items: [
        {
          name: known.mealName,
          mealLibraryId: known.id,
          reportedNutrition: {
            calories: known.calories + 25,
    ...
    assert.deepEqual(
      mixed.items.map((item) => item.source),
      ['FNRI', 'USER_REPORTED', 'UNRESOLVED']
    );
    ...
    assert.equal(
      (await prisma.dailyNutritionLog.findFirstOrThrow({ where: { userId: user.id } })).totalCalories,
    ...
    assert.equal(
      (await OutsideMealCaptureService.voidLog(user.id, mixedCommit.log.id, 'Accidental duplicate')).replayed,
      true
    );
    ...
    const catalogMeal = await prisma.mealLibrary.create({
      data: {
        mealName: `Batch 7 seafood bowl ${run}`,
        mealType: MealType.SNACK,
        calories: 260,
        proteinG: 20,
        carbsG: 20,
...
main().catch((error) => {
  console.error('[Batch 7 outside capture acceptance] FAIL', error);
  process.exitCode = 1;
});
