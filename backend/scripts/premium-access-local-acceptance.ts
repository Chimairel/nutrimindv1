...
async function readyUser(id: string, email: string) {
  return prisma.user.upsert({
    ...
    create: {
      id,
      name: id === 'accept_free_user' ? 'Free Acceptance' : 'Premium Acceptance',
      email,
      passwordHash,
      role: 'USER',
      emailVerified: true,
...
async function seedPlan(userId: string, planGroupId: string, swapsUsed: number) {
  ...
  await prisma.mealPlan.create({
    data: {
      id: `${planGroupId}_meal`,
      planGroupId,
      userId,
      status: 'APPROVED',
      mealType: 'BREAKFAST',
      mealName: 'Synthetic acceptance breakfast',
      calories: 420,
      proteinG: 24,
      carbsG: 48,
      fatG: 14,
  ...
  try {
    ...
    for (const user of [freeUser, premiumUser]) {
      await prisma.userProfile.upsert({
        ...
        update: {},
        create: { userId: user.id, shoppingDayOfWeek: 6 },
      });
      await prisma.nutritionReport.upsert({
        ...
        create: {
          ...
          foodsRecommended: [],
          drinksGuidance: [],
          generalSummary: 'Synthetic local acceptance record.',
          basedOnConditions: [],
          basedOnAllergies: [],
        },
      });
    ...
    const product = await prisma.billingProduct.upsert({
      ...
      create: {
        id: 'accept_premium_product',
        code: 'PREMIUM',
        displayName: 'Premium',
        status: 'ACTIVE',
        featureSetVersion: 'meal-swaps-v1',
        description: 'Synthetic local acceptance product.',
      },
    });
    ...
    const checkout = await prisma.billingCheckoutRequest.upsert({
      ...
      update: {},
      create: {
        id: 'accept_premium_checkout',
        userId: premiumUser.id,
        billingSubjectKey: 'accept-premium-subject',
        billingPriceId: price.id,
        provider: 'PAYMONGO',
        environment: 'TEST',
        requestIdempotencyKey: 'acceptance-request-key',
        requestHash: 'a'.repeat(64),
        providerIdempotencyKey: 'acceptance-provider-key',
        referenceNumber: 'acceptance-reference',
        status: 'SUCCEEDED',
        providerSessionId: 'cs_acceptance_synthetic',
        checkoutUrl: 'https://checkout.paymongo.com/acceptance-synthetic',
        completedAt: at,
      },
    });
    ...
    const subscription = await prisma.userSubscription.upsert({
      where: {
        provider_environment_creationIdempotencyKey: {
          provider: 'PAYMONGO',
          environment: 'TEST',
          creationIdempotencyKey: 'acceptance-subscription-key',
        },
      },
      update: {
        userId: premiumUser.id,
      ...
      create: {
        id: 'accept_premium_subscription',
        userId: premiumUser.id,
        billingSubjectKey: 'accept-premium-subject',
        billingPriceId: price.id,
        sourceCheckoutRequestId: checkout.id,
    ...
    const invoice = await prisma.billingInvoice.upsert({
      ...
      update: {
        status: 'PAID',
        amountPaidMinor: 19_900,
        servicePeriodStart: accessStartsAt,
        servicePeriodEnd: accessExpiresAt,
      },
      create: {
        id: 'accept_premium_invoice',
    ...
    await prisma.entitlementGrant.upsert({
      ...
      update: {
        ...
        effectiveFrom: accessStartsAt,
        effectiveUntil: accessExpiresAt,
        revokedAt: null,
      },
      create: {
        id: 'accept_premium_grant',
        userId: premiumUser.id,
        billingSubjectKey: 'accept-premium-subject',
        subscriptionId: subscription.id,
        invoiceId: invoice.id,
        ...
        source: 'PAID_INVOICE',
        sourceKey: 'acceptance-paid-premium-grant',
        effectiveFrom: accessStartsAt,
        effectiveUntil: accessExpiresAt,
      },
    });
    ...
    const [freeReservations, premiumReservations] = await Promise.all([
      Promise.allSettled(
        Array.from({ length: 12 }, () =>
          reserveWeeklySwap(prisma, {
            trackerId: freeTracker.id,
            userId: freeUser.id,
            cap: 3,
          })
        )
    ...
    const [free, premium] = await Promise.all([service.getForUser(freeUser.id), service.getForUser(premiumUser.id)]);
