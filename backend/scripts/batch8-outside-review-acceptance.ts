...
async function main() {
  ...
  try {
    const user = await prisma.user.create({ data: {
      name: 'Batch 8 User', email: `batch8-user-${run}@example.invalid`, passwordHash: 'disabled',
    ...
    for (const index of [1, 2]) {
      const account = await prisma.user.create({ data: {
        name: `Batch 8 RND ${index}`, email: `batch8-rnd${index}-${run}@example.invalid`,
        passwordHash: 'disabled', role: 'NUTRITIONIST', emailVerified: true,
      } });
      accountIds.push(account.id);
      reviewers.push(await prisma.nutritionistProfile.create({ data: {
        userId: account.id, prcLicenseNumber: `BATCH8-${index}-${run}`,
        prcLicenseExpiry: new Date('2030-12-31T00:00:00Z'), isVerified: true, verifiedAt: new Date(),
      } }));
    }
    const consumedAt = new Date(Date.now() - 10 * 60_000).toISOString();
    const preview = await MealLogService.logOutsideMeal({ userId: user.id, mealType: MealType.SNACK, consumedAt,
      items: [{ name: `Batch 8 packaged snack ${run}`,
        reportedNutrition: { calories: 200, proteinG: 10, carbsG: 25, fatG: 7 } }] });
    assert.ok('previewRequired' in preview);
    const committed = await MealLogService.logOutsideMeal({ userId: user.id, mealType: MealType.SNACK,
      warningAcknowledged: true, confirmationId: preview.confirmationId });
    assert.ok('log' in committed);
    const logId = committed.log.id;
    const itemId = committed.log.outsideItems[0].id;
    assert.equal(await prisma.outsideMealReview.count({ where: { outsideMealLogItemId: itemId } }), 0,
      'An ordinary manual estimate does not automatically enter the queue.');
    await prisma.dailyNutritionLog.create({ data: {
      userId: user.id, logDate: getStartOfManilaBusinessDay(new Date(consumedAt)),
      totalCalories: 200, totalProteinG: 10, totalCarbsG: 25, totalFatG: 7,
      targetCalories: 2000, adherencePct: 10,
    } });
    await OutsideMealCaptureService.attachImage(user.id, logId,
      { buffer: Buffer.from('private fixture image'), mimetype: 'image/png' });
    await rejectsCode(() => OutsideMealReviewService.requestByUser(reviewers[0].userId, logId, itemId),
    ...
    assert.equal('outsideImage' in firstClaim.outsideMealLogItem.mealLog, false,
    ...
    assert.deepEqual((await OutsideMealReviewService.imageForClaimedReview(reviewers[0].id, requested.id)).buffer,
      Buffer.from('private fixture image'));
    await rejectsCode(() => OutsideMealReviewService.imageForClaimedReview(reviewers[1].id, requested.id),
      'OUTSIDE_IMAGE_NOT_FOUND');
    const question = await OutsideMealReviewService.resolve(reviewers[0].id, requested.id, {
      action: 'NEEDS_MORE_INFO', reason: 'How large was the portion, and was sauce included?',
    });
    ...
    const reply = await OutsideMealReviewService.replyByUser(user.id, logId, itemId,
      'One packet, 50 grams; the packet included the sauce.');
    assert.equal(reply.revision, 2);
    await rejectsCode(() => OutsideMealReviewService.replyByUser(user.id, logId, itemId, 'Another reply'),
      'NO_OPEN_CLARIFICATION');
    await rejectsCode(() => OutsideMealReviewService.resolve(reviewers[0].id, requested.id,
      { action: 'VERIFY', reason: 'Stale decision' }), 'ACTIVE_CLAIM_REQUIRED');
    ...
    const correction = await OutsideMealReviewService.resolve(reviewers[1].id, requested.id, {
      action: 'CORRECT', calories: 240, proteinG: 11, carbsG: 27, fatG: 9,
    ...
    const changed = await OutsideMealCaptureService.editItem(user.id, logId, itemId, {
      name: `Batch 8 packaged snack ${run}`, portionGrams: 50,
    ...
    const unverifiable = await OutsideMealReviewService.resolve(reviewers[0].id, requested.id, {
      action: 'UNVERIFIABLE', reason: 'The package values conflict and the portion cannot be confirmed.',
    });
...
main().catch((error) => { console.error('[Batch 8 outside review acceptance] FAIL', error); process.exitCode = 1; });
