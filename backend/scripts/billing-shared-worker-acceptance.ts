  ...
  await prisma.$transaction(
    async (transaction) => {
      ...
      const inventory = await transaction.$queryRawUnsafe<Array<{ table_name: string; row_count: number }>>(
        BILLING_TABLES.map(
          (tableName) => `SELECT '${tableName}' AS table_name, COUNT(*)::int AS row_count FROM "public"."${tableName}"`
        ).join(' UNION ALL ')
      );
      ...
      await prisma.$transaction(
        async (transaction) => {
          ...
          await transaction.user.create({
            data: {
              id: userId,
              name: 'Shared Billing Acceptance',
              email: EMAIL,
          ...
          await transaction.billingProduct.create({
            data: {
              id: productId,
              code: 'PREMIUM',
              displayName: 'Synthetic Premium',
              status: 'ACTIVE',
              featureSetVersion: `${PREFIX}_v1`,
            },
          });
          stage = 'CREATE_SYNTHETIC_PRICE';
          await transaction.billingPrice.create({
            data: {
              id: priceId,
              productId,
              provider: 'PAYMONGO',
          ...
          const createCheckout = async (suffix: string, sessionId: string, referenceNumber: string) => {
            await transaction.billingCheckoutRequest.create({
              data: {
                id: `${PREFIX}_checkout_${suffix}`,
                userId,
                billingSubjectKey: subjectKey,
                billingPriceId: priceId,
                provider: 'PAYMONGO',
                environment: 'TEST',
                requestIdempotencyKey: `${PREFIX}_request_${suffix}`,
                requestHash: sha256(`${PREFIX}:checkout:${suffix}`),
                providerIdempotencyKey: `${PREFIX}_provider_${suffix}`,
                referenceNumber,
                status: 'SUCCEEDED',
                attemptCount: 1,
          ...
          const createEvent = async (
            ...
            transaction.providerWebhookEvent.create({
              data: {
                id: `${PREFIX}_event_${suffix}`,
                provider: 'PAYMONGO',
                environment: 'TEST',
                ...
                signatureKeyVersion: `${PREFIX}_key_v1`,
                providerCreatedAt,
                receivedAt: new Date(providerCreatedAt.getTime() + 1_000),
                processing: { create: processing },
              },
            });
          const makeWorker = () => {
            const processor = new PaymongoPaymentProjectionService(
              new PrismaPaymentProjectionRepository(client, () => NOW),
              gateway,
              () => NOW
            );
          ...
          await createEvent('expired', sessionOne, {
            id: `${PREFIX}_processing_expired`,
            handlerVersion: `${PREFIX}_stale_worker`,
            status: 'PROCESSING',
            attemptCount: 1,
            lockedAt: new Date('2026-12-09T23:00:00Z'),
            claimTokenHash: 'a'.repeat(64),
            claimExpiresAt: new Date('2026-12-09T23:01:00Z'),
          });
          evidenceBySession.set(sessionOne, checkoutEvidence(sessionOne, referenceOne, '2026-09-06T13:00:00Z', 'one'));

          stage = 'READ_PREPROCESSING_OPERATIONS';
          const operationsBefore = await new BillingOperationsStatusService(
            new PrismaBillingOperationsRepository(client),
            () => makeWorker().snapshot(),
            () => NOW
          ).getStatus();
          assert.equal(operationsBefore.queue.processing, 1);
          assert.doesNotMatch(
            JSON.stringify(operationsBefore),
            /sanitizedPayload|providerResourceId|billingSubjectKey|userId/
          );
          ...
          assert.equal(
            (
              await transaction.webhookEventProcessing.findUniqueOrThrow({
          ...
          await createEvent('replay', sessionOne, {
            id: `${PREFIX}_processing_replay`,
            handlerVersion: `${PREFIX}_restart`,
            status: 'PENDING',
          });
          ...
          await createEvent('unknown', `cs_${PREFIX}_missing_session`, {
            id: `${PREFIX}_processing_unknown`,
            handlerVersion: `${PREFIX}_dead_letter`,
            status: 'PENDING',
          });
          ...
          await createEvent(
            ...
            sessionRetry,
            {
              id: `${PREFIX}_processing_retry`,
              handlerVersion: `${PREFIX}_retry`,
              status: 'PENDING',
            },
          ...
          evidenceBySession.set(
            sessionRetry,
            checkoutEvidence(sessionRetry, referenceRetry, '2026-10-07T13:00:00Z', 'retry')
          );
          ...
          for (const batchKey of new Set(postings.map((row) => row.batchKey))) {
            ...
            assert.equal(
              batch.reduce((sum, row) => sum + (row.direction === 'DEBIT' ? row.amountMinor : -row.amountMinor), 0),
              0
            );
          }
          ...
          assert.equal(
            grants[0].effectiveUntil.getTime() - grants[0].effectiveFrom.getTime(),
            30 * 24 * 60 * 60 * 1_000
          );
          ...
          assert.doesNotMatch(
            JSON.stringify(operationsAfter),
