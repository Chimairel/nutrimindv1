...
function canonicalMigrationHash(name: string): string {
  const bytes = execFileSync('git', [
    'show', `HEAD:backend/prisma/migrations/${name}/migration.sql`,
  ], { cwd: resolve(process.cwd(), '..'), encoding: 'buffer', stdio: ['ignore', 'pipe', 'ignore'] });
  return sha256(bytes);
}
...
function validateTarget(): { url: string; fingerprint: string } {
  ...
  if (parsed.protocol !== 'postgresql:' || !parsed.hostname.endsWith('.neon.tech') ||
      parsed.searchParams.get('sslmode') !== 'require' || fingerprint !== EXPECTED_TARGET_FINGERPRINT) {
...
async function main(): Promise<void> {
  ...
  const repositoryMigrations = readdirSync(migrationRoot, { withFileTypes: true })
    ...
    .sort();
  if (repositoryMigrations.length !== 21 ||
      !AUTHORIZED_PENDING.every((name) => repositoryMigrations.includes(name))) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (transaction) => {
      ...
      const identity = await transaction.$queryRaw<Array<{
      ...
      const expectedBillingTables = mode === 'preflight' ? foundationTables : [...foundationTables, ...checkoutTables];
    ...
    if (mode === 'postflight') {
      ...
      if (baseline.version !== 1 || baseline.targetFingerprint !== fingerprint || baseline.migrationNames.length !== 19) {
