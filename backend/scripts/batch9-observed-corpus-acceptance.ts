  ...
  try {
    for (const suffix of ['a', 'b']) {
      const user = await prisma.user.create({ data: {
        email: `batch9-${suffix}-${marker}@example.invalid`, name: `Batch 9 ${suffix}`,
        passwordHash: 'disabled', emailVerified: true,
      } });
      users.push(user.id);
    }
    const rndUser = await prisma.user.create({ data: {
      email: `batch9-rnd-${marker}@example.invalid`, name: 'Batch 9 RND',
      passwordHash: 'disabled', role: 'NUTRITIONIST', emailVerified: true,
    } });
    users.push(rndUser.id);
    const rnd = await prisma.nutritionistProfile.create({ data: {
      userId: rndUser.id, prcLicenseNumber: `BATCH9-${marker}`,
      prcLicenseExpiry: new Date('2030-12-31T00:00:00Z'), isVerified: true,
    } });

    async function confirmed(userId: string, name: string, portionGrams: number) {
      const log = await prisma.mealLog.create({ data: {
        userId, source: 'USER_LOGGED', status: 'DONE', mealName: name,
        mealType: MealType.LUNCH, calories: 400, proteinG: 30, carbsG: 45, fatG: 12,
        dataSource: 'NUTRITIONIST_REVIEWED', notes: `Private note ${marker}`,
        estimationContext: 'Simmer chicken, carrots, and water until cooked.',
        outsideImage: Buffer.from('private photo'), outsideImageMime: 'image/png',
        outsideItems: { create: { position: 0, name, portionGrams,
          source: 'NUTRITIONIST_REVIEWED', nutritionStatus: 'VERIFIED',
          compatibilityStatus: 'INSUFFICIENT_EVIDENCE', includedInTotals: true,
          calories: 400, proteinG: 30, carbsG: 45, fatG: 12, currentRevision: 1,
    ...
    await rejectsCode(() => ObservedMealService.consent(users[0], refA.logId, refA.itemId,
      { ...withImage, imageRightsConfirmed: false }), 'REUSE_CONSENT_REQUIRED');
    await rejectsCode(() => ObservedMealService.consent(users[1], refA.logId, refA.itemId, detailsOnly), 'OUTSIDE_ITEM_NOT_FOUND');
    ...
    const admittedA = await ObservedMealService.admit(rnd.id, consentA.id, {
      kind: 'FOOD_REFERENCE', canonicalName: sharedName,
    });
    ...
    assert.equal(await prisma.observedMealSubmission.count({ where: {
      foodReferenceId: admittedA.id, status: 'ADMITTED_REFERENCE',
    } }), 2);
    ...
    const recipeInput = { kind: 'RECIPE_CANDIDATE' as const,
      canonicalName: `Batch9 chicken stew ${marker}`, ingredients,
      preparation: 'Simmer the chicken, carrot, and water until completely cooked.',
      mealTypes: [MealType.LUNCH, MealType.DINNER] };
    await rejectsCode(() => ObservedMealService.admit(rnd.id, recipeConsentA.id, {
      ...recipeInput, ingredients: [{ name: 'chicken', quantity: 0, unit: 'g' }],
    ...
    const candidate = await prisma.rawRecipeCandidate.findUniqueOrThrow({ where: { id: admittedRecipeA.id },
    ...
    const page = await databaseRecipeCandidateProvider.list({ mealType: MealType.LUNCH,
      dietaryPreference: 'OMNIVORE', sourceKind: 'USER_OBSERVED',
      search: candidate.recipeName, limit: 10 });
    ...
    assert.equal(await prisma.observedMealSubmission.count({ where: {
      rawRecipeCandidateId: candidate.id, status: 'ADMITTED_RECIPE',
    } }), 2);
    ...
    const variantAdmission = await ObservedMealService.admit(rnd.id, variantConsent.id, {
      ...recipeInput, preparation: 'Roast the chicken and carrot first, then add water and simmer until cooked.',
    });
    ...
    const voidAdmission = await ObservedMealService.admit(rnd.id, voidConsent.id, {
      ...recipeInput, preparation: 'Steam the chicken and carrot, then simmer briefly in water.',
    });
    candidates.push(voidAdmission.id);
    await OutsideMealCaptureService.voidLog(users[1], toVoid.logId, 'Recorded by mistake');
    assert.equal((await prisma.observedMealSubmission.findUniqueOrThrow({ where: { id: voidConsent.id } })).status,
      'WITHDRAWN');
    assert.equal((await prisma.rawRecipeCandidate.findUniqueOrThrow({ where: { id: voidAdmission.id } })).status,
      'RETIRED');

    await ObservedMealService.withdraw(users[0], recipeConsentA.id);
    assert.equal((await prisma.rawRecipeCandidate.findUniqueOrThrow({ where: { id: candidate.id } })).status, 'AVAILABLE');
    await ObservedMealService.withdraw(users[1], recipeConsentB.id);
    assert.equal((await prisma.rawRecipeCandidate.findUniqueOrThrow({ where: { id: candidate.id } })).status, 'RETIRED');
    await assert.rejects(() => prisma.$transaction((tx) => assertObservedSourceStillAvailable(tx, candidate.id)),
    ...
    assert.equal((await prisma.rawRecipeCandidate.findUniqueOrThrow({ where: { id: variantAdmission.id } })).status,
      'AVAILABLE', 'A deidentified admitted candidate survives source account deletion.');
...
main().catch((error) => { console.error('[Batch 9 observed corpus acceptance] FAIL', error); process.exitCode = 1; });
