...
export default function StructuredSafetyIntake({
  ...
  const review = async () => {
    ...
    try {
      ...
      setInputs(
        response.data.data.entries.map((entry: SafetyProfileEntry) => ({
          domain: entry.domain,
    ...
    <div className="space-y-5">
      ...
      <div>
        <label
          ...
          className="mb-2 block text-xs font-bold uppercase tracking-wide text-brand-muted"
        >
      ...
      <div aria-live="polite" className="space-y-2">
        {(preview?.entries || []).map((entry) => (
          <div
            ...
            className="flex items-start justify-between gap-3 rounded-xl border-2 border-brand-border bg-brand-bgAlt p-3"
          >
            <div>
              <p className="text-sm font-bold text-brand-text">{entry.displayName}</p>
              <p className="text-xs text-brand-muted">
                {labels[entry.domain]} · {stateLabel[entry.supportState]} · entered as “{entry.originalText}”
              </p>
            </div>
                    ...
                    current.filter(
                      (candidate) =>
                        !(
                          ...
                          (candidate.value.toLocaleLowerCase() === entry.originalText.toLocaleLowerCase() ||
                            candidate.value === entry.canonicalCode)
                        )
                    )
                  );
                  invalidateReview();
      ...
      {preview?.requiresReview && (
        <div className="flex gap-2 rounded-xl border border-status-pending-text/30 bg-status-pending-bg/10 p-3 text-xs text-status-pending-text">
          <AlertTriangle className="h-4 w-4 shrink-0" /> Some entries require individual review. They will remain active
          and will prevent automatic verified-library claims.
        </div>
      )}
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
        ...
        <Button
          ...
          variant="primary"
          onClick={save}
          disabled={isBusy || !preview?.canSave || !confirmed}
          isLoading={isBusy && confirmed}
        >
          {submitLabel}
        </Button>
      </div>
