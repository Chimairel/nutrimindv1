      ...
      <div className="rounded-xl border border-brand-border bg-brand-bgAlt/50 p-4">
        ...
        {summary.provisionalItemCount > 0 && (
          <p className="mt-3 text-xs font-semibold text-status-pending-text">
            {Math.round(summary.provisionalCalories)} kcal is estimated or based on an unconfirmed reference. It will
            change only if this entry is corrected or reviewed.
          </p>
        )}
      </div>
...
function OutsideMealForm(props: Props) {
  ...
  useEffect(() => {
    if (
      !props.isOpen ||
      props.mealName.includes(',') ||
      props.mealName.trim().length < 2 ||
      selectedSuggestion?.name === props.mealName.trim()
    ) {
      setSuggestions(null);
      return;
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await api.get('/user/meals/outside-suggestions', {
          params: { search: props.mealName.trim() },
        });
  ...
  const submit = (forceAi = false) => {
    ...
    if (
      ...
      !submittedItems[0].portionGrams &&
      Number(portionInput) > 0
    ) {
    ...
    if (useManualValues && canUseManual && !forceAi) {
      ...
      if (
        selectedSuggestion?.kind === 'OBSERVED_REFERENCE' ||
        !reference ||
  ...
  return (
    ...
    >
      <Input
        ...
        required
      />
      {suggestions && (suggestions.eligible.length > 0 || suggestions.otherKnown.length > 0) && (
        <div className="-mt-3 max-h-44 space-y-1 overflow-y-auto rounded-xl border border-brand-border p-2 text-xs">
          {suggestions.eligible.map((suggestion) => (
            <button
              type="button"
              key={`eligible-${suggestion.id}`}
              className="block w-full rounded-lg p-2 text-left hover:bg-brand-green/10"
              onClick={() => {
            ...
            >
              <strong>{suggestion.name}</strong> · {suggestion.label} · {suggestion.serving}
            </button>
          ))}
          {suggestions.otherKnown.map((suggestion) => (
            <button
              type="button"
              key={`${suggestion.kind}-${suggestion.id}`}
              className="block w-full rounded-lg p-2 text-left hover:bg-brand-border/30"
              onClick={() => {
                setSelectedSuggestion(suggestion);
                props.onMealNameChange(suggestion.name);
                setSuggestions(null);
      ...
      {canUseManual && selectedSuggestion?.kind !== 'ELIGIBLE_LIBRARY' && (
        <Input
          id="portionGrams"
          type="number"
          min="1"
          max="5000"
          step="0.1"
          label="Approximate portion in grams (if known)"
          value={portionInput}
      ...
      <Input
        id="consumedAt"
        type="datetime-local"
        label="When you ate it"
        value={consumedLocal}
        onChange={(event) => setConsumedLocal(event.target.value)}
        required
      />
      ...
      <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-brand-border p-3 text-xs">
        ...
        <span>
          ...
          <span className="text-brand-muted">
            Add a portion and preparation details below. AI values count immediately as estimates and may enter
            nutritionist review. Limits: 5 items/day and 30/30 days.
          </span>
        </span>
      </label>
      ...
      >
        ...
        <span>
          <strong className="block text-brand-text">
            {selectedSuggestion?.kind === 'ELIGIBLE_LIBRARY'
              ? 'Library serving macros (editable)'
              : 'I have nutrition-label or menu values'}
          </strong>
          <span className="text-brand-muted">
            {selectedSuggestion?.kind === 'ELIGIBLE_LIBRARY'
              ? 'Changing these values records a user-adjusted estimate.'
