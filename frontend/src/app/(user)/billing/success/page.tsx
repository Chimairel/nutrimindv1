...
const formatExpiry = (value: string) => new Intl.DateTimeFormat('en-PH', {
...
export default function BillingSuccessPage() {
  ...
  if (isLoading && !data) {
    return <div className="flex min-h-[60vh] items-center justify-center" aria-label="Checking payment verification"><LoadingSpinner /></div>;
  }
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-2xl items-center px-5 py-10">
      <Card className="w-full p-8 text-center sm:p-10">
        <span className={`mx-auto flex h-16 w-16 items-center justify-center rounded-3xl ${active ? 'bg-brand-accent text-[#07100d]' : 'bg-status-pending-bg text-status-pending-text'}`}>
          {active ? <CheckCircle2 className="h-8 w-8" /> : <Clock3 className="h-8 w-8" />}
        </span>
        <p className="mt-6 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-brand-green">Checkout return</p>
        <h1 className="mt-3 font-display text-3xl font-extrabold">{active ? 'Premium access is active' : 'Payment received — verification pending'}</h1>
        ...
        {error && <p className="mt-4 text-sm text-status-error-text" role="alert">{error}</p>}
        <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
          {!active && <Button variant="secondary" onClick={() => void refresh()}><RefreshCw className="h-4 w-4" />Check status</Button>}
          <Link href="/billing" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-brand-accent/70 bg-brand-accent px-5 py-2.5 font-display text-sm font-extrabold text-brand-black shadow-neon outline-none focus:ring-2 focus:ring-brand-green/60">
            <Crown className="h-4 w-4" />View access
          </Link>
        </div>
