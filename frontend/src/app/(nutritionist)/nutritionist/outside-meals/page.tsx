                ...
                <Card className="p-4 transition hover:border-brand-green">
                  <div className="flex justify-between gap-3">
                    ...
                    <span className="text-xs font-bold">
                      {row.queueReason?.replaceAll('_', ' ').toLowerCase() ?? 'Review'} · priority {row.priority}
                    </span>
                  </div>
          ...
          {!selected ? (
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-xs text-brand-muted">
                  Reviewing revision {selected.claimedRevision ?? selected.outsideMealLogItem.currentRevision} ·{' '}
                  {selected.queueReason?.replaceAll('_', ' ').toLowerCase()}
                </p>
                <p className="text-xs text-brand-muted">
                ...
                {selected.outsideMealLogItem.mealLog.estimationContext && (
                  <p className="mt-2 rounded-lg bg-brand-bgAlt p-2 text-xs">
                    Preparation context: {selected.outsideMealLogItem.mealLog.estimationContext}
                  </p>
                )}
                {selected.outsideMealLogItem.ingredients?.length ? (
                  <p className="mt-2 text-xs text-brand-muted">
                    Recorded ingredients: {selected.outsideMealLogItem.ingredients.join(', ')}
                  </p>
                ) : null}
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="space-y-2 p-4">
            {submissions.length === 0 && (
              <p className="text-sm text-brand-muted">No observations awaiting classification.</p>
            )}
            {submissions.map((row) => (
              <button
                type="button"
                ...
                className="block w-full rounded-xl border border-brand-border p-3 text-left text-sm hover:border-brand-green"
              >
          ...
          <Card className="space-y-3 p-4">
            {!observed ? (
              <p className="text-sm text-brand-muted">Select a consented observation to classify.</p>
            ) : (
              <>
                <p className="text-xs text-brand-muted">
                  {observed.sourceOutsideMealItem?.calories} kcal · P {observed.sourceOutsideMealItem?.proteinG} g · C{' '}
                  {observed.sourceOutsideMealItem?.carbsG} g · F {observed.sourceOutsideMealItem?.fatG} g. Source
                  identity and private notes are excluded.
                </p>
                  ...
                  <>
                    <label className="block text-sm">
                      ...
                      <textarea
                        ...
                        className="mt-1 min-h-24 w-full rounded-xl border border-brand-border bg-brand-surface p-2"
                      />
                    </label>
                    <label className="block text-sm">
                      Preparation method
