...
export default function AdminCompensationPage() {
  ...
  const act = async (key: string, request: () => Promise<unknown>, success: string) => {
    ...
    try {
      ...
      setNotice({ tone: 'error', text: apiError(error, 'The action could not be completed.') });
    } finally {
      setBusy(null);
    }
  };
          ...
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-brand-muted">
                Amounts are inactive until another admin activates this version.
              </p>
            </div>
          </div>
          <form onSubmit={createPolicy} className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="text-xs text-brand-muted">
              ...
              <input
                ...
                type="number"
                className={`${field} mt-1`}
                value={policyForm.baseRetainer}
                onChange={(e) => setPolicyForm({ ...policyForm, baseRetainer: e.target.value })}
              />
            </label>
            <label className="text-xs text-brand-muted">
              Workload cap · units
              <input
                ...
                type="number"
                className={`${field} mt-1`}
                value={policyForm.capUnits}
                onChange={(e) => setPolicyForm({ ...policyForm, capUnits: e.target.value })}
              />
            </label>
            <label className="text-xs text-brand-muted">
              Effective from
            ...
            <label className="text-xs text-brand-muted sm:col-span-2">
              ...
              <input
                required
                className={`${field} mt-1`}
                value={policyForm.bands}
                onChange={(e) => setPolicyForm({ ...policyForm, bands: e.target.value })}
              />
            </label>
            <Button className="sm:col-span-2" isLoading={busy === 'policy-new'}>
          ...
          <form onSubmit={createPeriod} className="grid gap-3 sm:grid-cols-2">
            <label className="text-xs text-brand-muted sm:col-span-2">
              ...
              <select
                required
                className={`${field} mt-1`}
                value={periodForm.policyId}
                onChange={(e) => setPeriodForm({ ...periodForm, policyId: e.target.value })}
              >
            ...
            <Button className="sm:col-span-2" isLoading={busy === 'period-new'}>
              ...
              >
                <div>
                  ...
                  <p className="mt-1 text-xs text-brand-muted">
                    ...
                    · effective {date(policy.effectiveFrom)}
                  </p>
                </div>
                {policy.status === 'DRAFT' && (
                  <Button
                ...
                <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-brand-muted">
                      ...
                      units · allowance {peso(statement.workloadAllowanceMinor)} · adjustments{' '}
                      {peso(statement.adjustmentMinor)}
                    </p>
                  ...
                  <div className="flex max-w-xl flex-wrap gap-2">
                    {statement.status === 'CALCULATED' && (
                      <>
                        <Button size="sm" variant="secondary" onClick={() => proposeAdjustment(statement)}>
                          Propose adjustment
                        </Button>
                        <Button
                          size="sm"
                    ...
                    <Button
                      ...
                      onClick={() =>
                        void act(
                          ...
                          () =>
                            api.post(`/admin/compensation/adjustments/${adjustment.id}/decision`, {
                              decision: 'APPROVE',
                            }),
                          'Adjustment approved.'
                        )
                  ...
                  <div className="mt-3">
                    ...
                    {payout.status === 'APPROVED' && (
                      <Button
                        size="sm"
                        onClick={() => {
                          const ref = window.prompt('Off-platform evidence reference (no account details):');
                          if (ref)
                            void act(
                              `payout-record-${payout.id}`,
                              () =>
                                api.post(`/admin/compensation/payouts/${payout.id}/record`, { externalReference: ref }),
