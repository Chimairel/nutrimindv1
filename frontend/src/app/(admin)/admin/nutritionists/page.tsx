...
export default function AdminNutritionistsPage() {
    ...
    try {
      ...
      setError(
        axios.isAxiosError(caught)
          ? caught.response?.data?.error || 'Professional records could not be loaded.'
  ...
  const act = async (
    ...
    try {
      ...
      setError(
        axios.isAxiosError(caught)
          ? caught.response?.data?.error || 'The application could not be updated.'
        ...
        <div className="bg-brand-bgAlt/35 p-5 sm:p-6">
          ...
          {application.status === 'CALL_REQUIRED' && (
            <div className="space-y-4">
              <div>
                <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-brand-muted">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setScheduleDrafts((current) => ({
                          ...current,
                          [application.id]: { ...draft, scheduledCallAt: toLocalInput(slot) },
                        }))
                      }
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Input
                  ...
                  onChange={(event) =>
                    setScheduleDrafts((current) => ({
                      ...current,
                      [application.id]: { ...draft, scheduledCallAt: event.target.value },
                    }))
                  }
                />
                <Input
                  ...
                  onChange={(event) =>
                    setScheduleDrafts((current) => ({
                      ...current,
                      [application.id]: { ...draft, meetingUrl: event.target.value },
                    }))
                  }
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                ...
                {!callOccurred && (
                  <p className="self-center text-[10px] text-brand-muted">
                    Approval unlocks after the scheduled call time.
                  </p>
                )}
              </div>
            </div>
          ...
          {!['APPROVED', 'REJECTED', 'ACTIVATED'].includes(application.status) && (
            <div className="mt-5 flex flex-col gap-3 border-t border-brand-border/60 pt-5 sm:flex-row">
              <Input
                ...
                onChange={(event) =>
                  setRejectionReasons((current) => ({ ...current, [application.id]: event.target.value }))
                }
              ...
              <Button
                variant="danger"
                className="self-end"
                disabled={!rejectionReasons[application.id]?.trim()}
                onClick={() =>
                  void act(
                    application.id,
                    () =>
                      api.patch(`/admin/nutritionist-applications/${application.id}/decision`, {
                        decision: 'reject',
                        reason: rejectionReasons[application.id],
                      }),
    ...
    <div className="portal-page space-y-7">
      ...
      <div className="flex rounded-2xl border border-brand-border bg-brand-surface/60 p-1">
        <button
          ...
          className={`flex-1 rounded-xl px-4 py-3 text-sm font-bold ${tab === 'applications' ? 'bg-brand-accent text-[#07100d]' : 'text-brand-muted'}`}
        >
        ...
        <button
          ...
          className={`flex-1 rounded-xl px-4 py-3 text-sm font-bold ${tab === 'professionals' ? 'bg-brand-accent text-[#07100d]' : 'text-brand-muted'}`}
        >
          Active professionals ({verified.length})
        </button>
      </div>
      {tab === 'applications' ? (
        <>
          <section>
            <p className="portal-section-label mb-4">Active pipeline · {activeApplications.length}</p>
            {activeApplications.length ? (
              <div className="grid gap-5 xl:grid-cols-2">{activeApplications.map(renderApplication)}</div>
            ) : (
          ...
          {completedApplications.length > 0 && (
            <section>
              <p className="portal-section-label mb-4">Completed applications · {completedApplications.length}</p>
              <div className="grid gap-5 xl:grid-cols-2">{completedApplications.map(renderApplication)}</div>
            </section>
          )}
        </>
      ) : (
        <section>
          {verified.length ? (
            ...
            <Card className="p-10 text-center text-sm text-brand-muted">No activated nutritionists yet.</Card>
          )}
        </section>
      )}
    </div>
