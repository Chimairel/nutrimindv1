...
export default function NutritionistApplyPage() {
    ...
    try {
      ...
      setError(
        axios.isAxiosError(caught)
          ? caught.response?.data?.error || 'Application could not be submitted.'
  ...
  const lookupStatus = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        axios.isAxiosError(caught)
          ? caught.response?.data?.error || 'Application was not found.'
        ...
        <div className="grid gap-10 lg:grid-cols-[0.7fr_1.3fr]">
          <aside className="futuristic-grid relative overflow-hidden rounded-[32px] bg-[#07100d] p-7 text-white lg:sticky lg:top-24 lg:h-fit lg:p-9">
            ...
            <p className="mt-5 text-sm leading-7 text-white/50">
              Registered nutritionist-dietitians from anywhere in the Philippines can apply online. Access is granted
              only after credential review and a one-on-one verification call.
            </p>
            ...
            <p className="mt-7 text-[11px] leading-5 text-white/35">
              Submitting an application does not grant access or guarantee employment. NutriMind administrators review
              every applicant.
            </p>
          </aside>
          ...
          <section>
            <div className="mb-6 flex rounded-2xl border border-brand-border bg-brand-surface/60 p-1">
              <button
                ...
                className={`flex-1 rounded-xl px-4 py-3 text-sm font-bold transition ${mode === 'apply' ? 'bg-brand-accent text-[#07100d]' : 'text-brand-muted'}`}
              >
                Apply online
              </button>
              <button
                ...
                className={`flex-1 rounded-xl px-4 py-3 text-sm font-bold transition ${mode === 'track' ? 'bg-brand-accent text-[#07100d]' : 'text-brand-muted'}`}
              >
                Track application
              </button>
            </div>
              ...
              <div className="surface-panel rounded-[30px] p-6 sm:p-8">
                  ...
                  {step === 1 && (
                    <>
                      <Input
                        ...
                        onChange={(event) => setField('prcLicenseNumber', event.target.value.toUpperCase())}
                      ...
                      <Input
                        ...
                        onChange={(event) => setField('prcLicenseExpiry', event.target.value)}
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(event) => setField('professionalBio', event.target.value)}
                  ...
                  {step === 3 && (
                    <>
                      <p className="rounded-2xl border border-brand-cyan/20 bg-brand-cyan/[0.06] p-4 text-sm leading-6 text-brand-muted">
                        Provide at least two schedules when you can join a short online call. An administrator will
                        select and confirm one.
                      </p>
                      <Input
                        id="callSlotOne"
                        label="Preferred schedule"
                        type="datetime-local"
                        value={form.callSlotOne}
                        onChange={(event) => setField('callSlotOne', event.target.value)}
                        error={errors.callSlotOne}
                      />
                      <Input
                        id="callSlotTwo"
                        label="Alternative schedule"
                        type="datetime-local"
                        value={form.callSlotTwo}
                        onChange={(event) => setField('callSlotTwo', event.target.value)}
                        error={errors.callSlotTwo}
                      />
                      <Input
                        id="callSlotThree"
                        label="Third option (optional)"
                        type="datetime-local"
                        value={form.callSlotThree}
                        onChange={(event) => setField('callSlotThree', event.target.value)}
                        error={errors.callSlotThree}
                      />
                      <label
                      ...
                      >
                        ...
                        <span className="text-xs leading-5 text-brand-muted">
                          I confirm that the professional information provided is accurate and consent to credential
                          verification and an online identity call.
                        </span>
                      </label>
                      {errors.consent && (
                        <p className="text-xs font-semibold text-status-error-text">{errors.consent}</p>
                      )}
                    </>
                  )}
                  {step === 4 && (
                    <div className="space-y-3">
                      {[
                        ...
                        <div
                          key={label}
                          className="flex items-start justify-between gap-5 rounded-2xl bg-brand-bgAlt/60 px-4 py-3"
                        >
                      ...
                      <div className="rounded-2xl border border-brand-green/20 bg-brand-green/[0.06] p-4 text-xs leading-5 text-brand-muted">
                        ...
                        Submitting creates an application only. It does not create a privileged account. An
                        administrator must complete the review and verification call first.
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-8 flex items-center justify-between gap-3">
                  ...
                  {step < 4 ? (
                    ...
                    <Button type="button" onClick={submitApplication} isLoading={isLoading}>
