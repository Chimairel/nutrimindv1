...
function InvitationForm() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!validatedPassword.success)
      return setError(validatedPassword.error.issues[0]?.message || 'Choose a valid password.');
    ...
    return (
      <div className="text-center">
        ...
        <p className="mt-3 text-sm leading-6 text-brand-muted">
          Sign in with your application email and the password you just created.
        </p>
        <Link
          href="/login"
          className="mt-6 inline-flex min-h-12 items-center rounded-2xl bg-brand-accent px-6 text-sm font-extrabold text-[#07100d]"
        >
          Continue to sign in
        </Link>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      {error && (
        <div
          role="alert"
          className="flex gap-3 rounded-2xl border border-status-error-text/25 bg-status-error-bg/10 p-4 text-sm font-semibold text-status-error-text"
        >
      ...
      <PasswordInput
        ...
        maxLength={128}
        disabled={isLoading}
        helperText="Use at least 8 characters with one uppercase letter and one number."
      />
      <PasswordInput
        id="nutritionist-confirm-password"
...
export default function NutritionistInvitationPage() {
  return (
    ...
    >
      <Suspense fallback={<p className="py-8 text-center text-sm text-brand-muted">Checking invitation...</p>}>
        <InvitationForm />
      </Suspense>
    </AuthShell>
  );
