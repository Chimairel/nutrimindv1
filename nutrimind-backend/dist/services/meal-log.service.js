...
class MealLogService {
     ...
     */
    static async logOutsideMeal(input) {
        ...
        const systemInstruction = "You are a clinical database dietitian specialized in estimating nutritional statistics for restaurant and home-cooked dishes in the Philippines.";
        const prompt = `Estimate the nutritional values and main ingredients of one standard serving of: "${mealName}".\n` +
        ...
        const estimate = await (0, gemini_1.generateGenerativeJSON)(prompt, systemInstruction);
        ...
        const loggedToday = await prisma_1.default.mealLog.findMany({
            where: {
                userId,
                loggedAt: {
                    gte: startOfDay,
        ...
        if (projectedTotal > dailyCalorieTarget) {
            ...
            conflictReasons.push(`Logging this meal puts you at ${Math.round(projectedTotal)} kcal, exceeding your daily target of ${dailyCalorieTarget} kcal by ${Math.round(projectedTotal - dailyCalorieTarget)} kcal.`);
        }
        ...
        const savedLog = await prisma_1.default.mealLog.create({
            data: {
                userId,
                source: client_1.MealLogSource.USER_LOGGED,
                ...
                status: client_1.MealLogStatus.DONE,
