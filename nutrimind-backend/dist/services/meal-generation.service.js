...
class MealGenerationService {
     ...
     */
    static async generate7DayPlan(userId) {
        ...
        const breakfasts = matchedLibraryMeals.filter((m) => m.mealType === client_1.MealType.BREAKFAST);
        const lunches = matchedLibraryMeals.filter((m) => m.mealType === client_1.MealType.LUNCH);
        const dinners = matchedLibraryMeals.filter((m) => m.mealType === client_1.MealType.DINNER);
        ...
        if (breakfasts.length >= 7 && lunches.length >= 7 && dinners.length >= 7) {
            ...
            await prisma_1.default.mealPlan.updateMany({
            ...
            for (let day = 0; day < 7; day++) {
                ...
                const dailyMeals = [
                    { type: client_1.MealType.BREAKFAST, source: breakfasts[day % breakfasts.length] },
                    { type: client_1.MealType.LUNCH, source: lunches[day % lunches.length] },
                    { type: client_1.MealType.DINNER, source: dinners[day % dinners.length] },
                ];
                for (const m of dailyMeals) {
                    planMealsToCreate.push({
                        ...
                        status: client_1.MealPlanStatus.APPROVED, // Pre-verified items are automatically approved!
                        mealType: m.type,
            ...
            await prisma_1.default.$transaction([
                prisma_1.default.mealPlan.createMany({ data: planMealsToCreate }),
                ...matchedLibraryMeals.map((m) => prisma_1.default.mealLibrary.update({
                    where: { id: m.id },
        ...
        const prompt = `Compile a highly customized, culturally appropriate 7-day meal plan (3 meals/day: BREAKFAST, LUNCH, DINNER = exactly 21 meals in total) matching these specs:\n` +
            `\n` +
        ...
        const aiResponse = await (0, gemini_1.generateGenerativeJSON)(prompt, systemInstruction);
        ...
        for (const rawMeal of aiResponse.meals) {
            ...
            for (const ing of rawMeal.ingredients) {
                try {
                    const lookup = await (0, fnri_1.lookupIngredient)(ing.name);
            ...
            // Newly generated AI plans default to PENDING_REVIEW unless completely SAFE
            // Wait, let's keep all AI plans under review for medical safety to satisfy the 5 legal protections
            const status = (flag === client_1.AIConfidenceFlag.SAFE) ? client_1.MealPlanStatus.APPROVED : client_1.MealPlanStatus.PENDING_REVIEW;
            // Save plan item and ingredients atomically
            const createdPlan = await prisma_1.default.mealPlan.create({
                data: {
                    planGroupId: newPlanGroupId,
                    userId,
                    status,
        ...
        if (needsReview) {
            ...
            const activeAssignment = await prisma_1.default.nutritionistAssignment.findFirst({
