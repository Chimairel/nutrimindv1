...
exports.getFNRISubset = getFNRISubset;
...
async function lookupIngredient(ingredientName) {
    ...
    const exactMatch = await prisma_1.default.foodItem.findFirst({
        where: {
            name: {
                equals: cleanName,
                mode: 'insensitive',
    ...
    const aliasMatch = await prisma_1.default.foodAlias.findFirst({
        where: {
            alias: {
                equals: cleanName,
                mode: 'insensitive',
    ...
    const fuzzyMatch = await prisma_1.default.foodItem.findFirst({
        where: {
            name: {
                contains: cleanName,
                mode: 'insensitive',
    ...
    if (fuzzyMatch) {
        ...
        try {
            await prisma_1.default.foodAlias.create({
                data: {
    ...
    const prompt = `Estimate the nutritional values per 100g portion of: "${cleanName}".\n` +
    ...
    try {
        const estimated = await (0, gemini_1.generateGenerativeJSON)(prompt, systemInstruction);
        const formattedFood = {
