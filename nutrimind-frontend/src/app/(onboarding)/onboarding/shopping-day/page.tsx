...
const options: { value: ShoppingDayGroup; icon: string; title: string; desc: string; days: string }[] = [
  {
    value: 'WEEKEND',
    icon: '🛒',
              ...
              {options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      flex items-center gap-4 px-5 py-5 rounded-xl border text-left transition-all duration-200 outline-none
                      ${isSelected
                    ...
                    `}
                  >
                    <span className="text-3xl bg-brand-border/30 p-3 rounded-xl flex items-center justify-center">
                      {opt.icon}
                    </span>
                    <div className="flex-1">
                      ...
                      <p className={`text-[11px] font-semibold mt-1 ${isSelected ? 'text-brand-green/80' : 'text-brand-border'}`}>
                        {opt.days}
                      </p>
                    </div>
                    {isSelected && (
                      <span className="text-brand-green text-sm font-bold bg-brand-green/10 h-7 w-7 rounded-full flex items-center justify-center flex-shrink-0">
                        ✓
                      </span>
                    )}
                  </button>
